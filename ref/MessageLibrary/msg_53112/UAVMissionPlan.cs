        ...
        public byte[] Serialize()
        {
            ...
            foreach (var missionSegment in missionSegmentList)
            {
                ...
                foreach (var individualMission in missionSegment.individualMissionList)
                {
                    ...
                    bw.Write(individualMission.allocatedArea.coordinateListN ?? 0);
                    if (individualMission.allocatedArea.coordinateListN > 0)
                    {
                        foreach (var coordinate in individualMission.allocatedArea.coordinateList)
                        {
                    ...
                    switch (individualMission.flightType)
                    {
                        case 1:
                            ...
                            foreach (var waypoint in individualMission.waypointList)
                            {
                                ...
                                {
                                    bw.Write(waypoint.filmingProperty.fieldOfView ?? 0);
                                    bw.Write(waypoint.filmingProperty.sensorType ?? 0);
                                    bw.Write(waypoint.filmingProperty.operationMode ?? 0);

                                    if (waypoint.filmingProperty.operationMode == 1 || waypoint.filmingProperty.operationMode == 2)
                                    {
                                        bw.Write(waypoint.filmingProperty.coordinateListN ?? 0);
                                        if (waypoint.filmingProperty.coordinateListN > 0)
                                        {
                                            foreach (var coordinate in waypoint.filmingProperty.coordinateList)
                                            {
                                                bw.Write(coordinate.latitude ?? 0);
                                                bw.Write(coordinate.longitude ?? 0);
                                                bw.Write(coordinate.altitude ?? 0);
                                    ...
                                    {
                                        bw.Write(waypoint.filmingProperty.searchSpeed ?? 0);
                                    }

                                    if (waypoint.filmingProperty.operationMode == 3)
                                    {
                                        bw.Write(waypoint.filmingProperty.targetID ?? 0);
                                    }

                                    if (waypoint.filmingProperty.operationMode == 4)
                                    {
                                        bw.Write(waypoint.filmingProperty.sensorYaw ?? 0);
                                    }

                                    if (waypoint.filmingProperty.operationMode == 4 || waypoint.filmingProperty.operationMode == 5)
                                    {
                                        bw.Write(waypoint.filmingProperty.sensorPitch ?? 0);
                                    }

                                    if (waypoint.filmingProperty.operationMode == 5)
                                    {
                                        bw.Write(waypoint.filmingProperty.sensorYawAngularSpeed.leftLimit ?? 0);
                                        bw.Write(waypoint.filmingProperty.sensorYawAngularSpeed.rightLimit ?? 0);
                                        bw.Write(waypoint.filmingProperty.sensorYawAngularSpeed.angularRate ?? 0);
                                    }
                                }
                            }
                            break;
