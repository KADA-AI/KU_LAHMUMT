    ...
    for uav_id in uav_ids:
        ...
        for mission in mission_list:
            if not isinstance(mission, dict):
                continue
            if not (mission.get("postAttackBoundaryHold") or mission.get("post_attack_boundary_hold")):
                continue
        ...
        for mission in mission_list:
            ...
            post_attack_boundary_hold = bool(
                mission.get("postAttackBoundaryHold")
                or mission.get("post_attack_boundary_hold")
            )
            if path_id is not None:
                path_data = load_db_json("FlightPath", path_id, db_root=db_root)
                post_attack_boundary_hold = bool(
                    post_attack_boundary_hold
                    or path_data.get("postAttackBoundaryHold")
                    or path_data.get("post_attack_boundary_hold")
                )
