...
def _target_destroyed_seen(
    ...
    for entry in target_map.values():
        ...
        if _to_int(entry.get("targetID")) != int(target_id):
            continue
        if _to_bool(entry.get("isDestroyed")) is True:
            return True
    return False


def _propagate_destroyed_state_for_target(
    target_map: Dict[str, Dict[str, Any]],
    target_id: int,
    *,
    ...
    for entry in target_map.values():
        ...
        if _to_int(entry.get("targetID")) != int(target_id):
            continue
        if _to_bool(entry.get("isDestroyed")) is not True:
            entry["isDestroyed"] = True
            changed = True
        if _to_bool(entry.get("targetInFrame")) is not False:
            entry["targetInFrame"] = False
            changed = True
        if _to_float(entry.get("threat")) != 0.0:
            entry["threat"] = 0.0
            changed = True
        if timestamp is not None:
            prev_ts = _to_int(entry.get("lastUpdated"))
            if prev_ts is None or int(timestamp) > prev_ts:
                entry["lastUpdated"] = int(timestamp)
                changed = True
    return changed
...
def load_target_info() -> Dict[str, Any]:
    ...
    destroyed_ids = sorted(
        {
            int(_to_int(entry.get("targetID")))
            for entry in normalized.values()
            if isinstance(entry, dict)
            and _to_int(entry.get("targetID")) is not None
            and _to_bool(entry.get("isDestroyed")) is True
        }
    )
    for target_id in destroyed_ids:
        _propagate_destroyed_state_for_target(normalized, int(target_id))
...
def reset_target_info(*, clear_prior_rediscovery: bool = True) -> None:
    """Reset persistent target detection state for a fresh simulation run."""
    save_target_info(dict(DEFAULT_TARGET_INFO))
    if clear_prior_rediscovery:
        try:
            prior_target_rediscovery_store.reset_state()
        except Exception:
            pass
...
def update_target_info_from_0402(message: Any) -> Tuple[Dict[str, Any], List[Dict[str, Any]]]:
    ...
    for entry in target_entries:
        ...
        if serialized:
            ...
            # Destroyed targets are terminal per targetID. A later watcher-specific
            # alive row for the same target should not resurrect the target on the map.
            if target_id_int is not None and (
                _to_bool(serialized.get("isDestroyed")) is True
                or _target_destroyed_seen(tracking_map, int(target_id_int))
            ):
                serialized["isDestroyed"] = True
                serialized["targetInFrame"] = False
                serialized["threat"] = 0.0
            ...
            if target_id is not None and not bool(serialized.get("isDestroyed")) and prior_rediscovery_entries:
                replan_candidate = {
                    "key": key,
                    "targetID": target_id,
                ...
                and not handled_elsewhere
                and not bool(serialized.get("isDestroyed"))
            ):
                new_targets.append(
                    {
                        "key": key,
            ...
            if target_id_int is not None and _to_bool(serialized.get("isDestroyed")) is True:
                _propagate_destroyed_state_for_target(target_map, int(target_id_int), timestamp=timestamp)
                _propagate_destroyed_state_for_target(tracking_map, int(target_id_int), timestamp=timestamp)
