...
def _extract_unmanned_entry_rows(
    ...
    *,
    allowed_ids: Set[int],
) -> tuple[Dict[int, Dict[str, Any]], Dict[int, float], Dict[int, Dict[str, Any]]]:
    entry_coord_map: Dict[int, Dict[str, Any]] = {}
    heading_map: Dict[int, float] = {}
    context_map: Dict[int, Dict[str, Any]] = {}
    if not isinstance(snapshot_payload, dict):
        return entry_coord_map, heading_map, context_map
    ...
    for entry in states:
        ...
        context_row = build_line_entry_context_row(
            ...
            state=entry,
            entry_coordinate=coord,
            heading_deg=heading,
            current_coordinate=coord,
        )
        if context_row is not None:
            context_map[int(aircraft_id)] = context_row
    return entry_coord_map, heading_map, context_map
...
def prepare_current_remaining_hybrid_replacements(
    *,
    ...
    entry_coord_map, heading_map, entry_aircraft_context_map = _extract_unmanned_entry_rows(
        agent_snapshot,
