...
def _terrain_data_signature_uncached() -> Tuple[Tuple[str, int, int], ...]:
    ...
    try:
        candidates.extend(sorted(_DEM_DIR.glob("*.tif")))
    except Exception:
        pass
...
def _read_dem_pixels_cached(
    ...
    cache_keys = [(str(path), int(row), int(col)) for row, col in zip(rows, cols)]
...
def _apply_dem_tile_to_indices(
    ...
    try:
        ...
        before_resolved = sum(1 for idx in chosen_indices if resolved[idx])
        ...
        for global_idx, native_x, native_y, value in zip(chosen_indices, chosen_xs, chosen_ys, values):
            ...
            if not usage_logged and float(value) != 0.0:
                ...
                usage_logged = True
        after_resolved = sum(1 for idx in chosen_indices if resolved[idx])
        _record_terrain_elev_many_metrics(demResolvedByTile=max(0, after_resolved - before_resolved))
    finally:
        _record_terrain_elev_many_metrics(
...
def warm_terrain_cache(coords: Iterable[Any] | None = None) -> Dict[str, Any]:
    """Prime DEM metadata/bands and optional mission-area pixels before hot-path planning."""
    pairs: list[Tuple[float, float]] = []
    for item in coords or []:
        pair = _coerce_lat_lon(item)
        if pair is not None:
