# modules/common/generator/message0501_generator.py
# auto-generated at 2025-08-24T14:56:18.146121+00:00
...
def _gen_CurrentIndividualMission(source: str = "DS"):
    obj = {}
    obj["individualMissionID"] = gen_value("individualMissionID", "uint32", MSG_ID, obj, 0)
    return obj

def _gen_IndividualMissionProgressStatus(source: str = "DS"):
    obj = {}
    obj["aircraftID"] = gen_value("aircraftID", "uint32", MSG_ID, obj, 0)
    obj["currentIndividualMission"] = _gen_CurrentIndividualMission(source=source)
    obj["currentIndividualMissionProgress"] = gen_value("currentIndividualMissionProgress", "uint32", MSG_ID, obj, 0)
    return obj

def _gen_MissionProgress(source: str = "DS"):
    obj = {}
    obj["timestamp"] = gen_value("timestamp", "uint64", MSG_ID, obj, 0)
    obj["source"] = gen_value("source", "string", MSG_ID, obj, 0)
    obj["currentMissionPlanID"] = gen_value("currentMissionPlanID", "uint32", MSG_ID, obj, 0)
    obj["currentInputMissionID"] = gen_value("currentInputMissionID", "uint32", MSG_ID, obj, 0)
    obj["individualMissionProgressStatusList"] = [ _gen_IndividualMissionProgressStatus(source=source) for _ in range(LIST_LEN) ]
    return obj




def make_msg0501_body(source: str = "DS"):
    """자동 생성된 메시지 바디 — MessageID: 0501, RootType: MissionProgress"""
    ex = _gen_MissionProgress(source=source)
...
def make_random_and_push(node_messenger):
    from generator.message0501_push import make_and_push
