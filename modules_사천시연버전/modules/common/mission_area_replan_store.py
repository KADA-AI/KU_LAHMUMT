...
def _align_carried_snapshot_entries_to_target_inputs(
    ...
    *,
    ...
    source_by_input: Dict[int, Dict[str, Any]] = {}
    for mission in source_entries:
        input_id = _mission_input_id(mission)
        if input_id is not None:
            source_by_input[int(input_id)] = mission
    ...
    alias_count = 0
    if missing_area_ids and len(source_area_entries) == 1:
        source_area = source_area_entries[0]
        source_input_id = _mission_input_id(source_area)
        for target_input_id in missing_area_ids:
            alias = deepcopy(source_area)
            alias["inputMissionID"] = int(target_input_id)
            alias["missionPlanID"] = int(target_plan_id)
            if source_input_id is not None:
                alias["carriedAliasFromInputMissionID"] = int(source_input_id)
                alias["sourceInputMissionID"] = int(source_input_id)
            alias["carriedAliasFromMissionPlanID"] = int(source_plan_id)
            aligned.append(alias)
            alias_count += 1

    if len(aligned) == len(source_entries) and alias_count <= 0:
        return carried
    ...
    if alias_count > 0:
        updated["carriedAliasCount"] = int(alias_count)
    _audit(
        "snapshot_carried_entries_aligned_to_target_inputs",
        {
            ...
            "droppedMissionCount": max(0, len(source_entries) - len(aligned) + alias_count),
...
def _merge_with_existing_snapshot(
    ...
    for mission in missions:
        ...
        if previous_done and not incoming_done:
            if is_area_snapshot:
                previous_ready = snapshot_entry_ready_for_replan(previous)
                incoming_ready = snapshot_entry_ready_for_replan(mission)
                keep_previous = bool(previous_ready) or not (
                    incoming_has_area_remaining and bool(incoming_ready)
                )
                reason = (
                    "previous_done_verified"
                    if previous_ready
                    else "previous_done_unverified_incoming_not_ready"
                )
            else:
                keep_previous = True
    ...
    if mission_plan_id is not None:
        ...
        if isinstance(exact_snapshot, dict) and isinstance(exact_entry, dict):
            audit_snapshot_entry_access(
                ...
                snapshot_mission_plan_id=_to_int(exact_snapshot.get("missionPlanID")) or int(mission_plan_id),
