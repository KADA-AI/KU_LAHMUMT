...
function getCoveragePayload(mode = coverageMode) {
  ...
  if (mode === "adjusted" && adjusted?.sections?.length) {
    return { coverage: adjusted, playback: currentScenario.playback0401 };
  }
  return { coverage: currentScenario.footprintCoverage, playback: currentScenario.playback0401 };
}

function getCoverageFeatures(mode = coverageMode) {
  if (!currentScenario) return null;
  if (mode === "adjusted" && currentScenario.inputAreaFeatures15Hz) {
    return currentScenario.inputAreaFeatures15Hz;
  }
  return currentScenario.inputAreaFeaturesRaw || currentScenario.inputAreaFeatures || null;
}
...
function renderCoveragePanel(coverage, playback) {
  ...
  const rawSummary = currentScenario?.footprintCoverage?.summary || {};
  const adjustedSummary = currentScenario?.footprintCoverage15Hz?.summary || {};
  const hasAdjusted = !!currentScenario?.footprintCoverage15Hz?.sections?.length;
