...
class MainWindow(QMainWindow):
    ...
    def _run_area_snapshot_worker(self) -> None:
        try:
            ...
            update_interval_sec = (
                max(200, int(self._env_float("MSM_AREA_SNAPSHOT_UPDATE_INTERVAL_MS", 1000.0)))
                / 1000.0
            )
    ...
    def _refresh_replan_queue_snapshot(self) -> None:
        ...
        if tab is None or manager is None:
            return
        try:
            tab.set_snapshot(manager.build_snapshot())
        except Exception:
            pass
    ...
    def _apply_attack_exclusion_ignore(self, mission_plan_id: int) -> bool:
        ...
        try:
            ...
            self._append_log_line(
                f"[0702] 공격 배제 선택 -> target bundle isIgnored=1 반영 (count={len(target_entries)})"
            )
        except Exception as exc:
            self._append_log_line(
    ...
    def _on_rx_0401(self, payload: object | None) -> None:
        ...
        states = []
        try:
            with self._trace_0401_phase(seq, "parse"):
                try:
                    raw_body = parse_payload(payload)
                    if not raw_body:
                        raw_body = parse_payload(self._unwrap_payload(payload))
