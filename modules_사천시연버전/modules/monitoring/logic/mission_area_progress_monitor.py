...
_AREA_ROW_PROGRESS_MIN_INTERVAL_MS = 250
...
def _top_level_area_replan_geometry(
    ...
    *,
    ...
    broad_clip: BaseGeometry | None = None
    if assignment_geometry is not None and not assignment_geometry.is_empty:
        try:
            broad_clip = assignment_geometry.convex_hull
        except Exception:
            broad_clip = None
    ...
    if gap_m > 0.0:
        try:
            ...
            if broad_clip is not None and not broad_clip.is_empty:
                closed = broad_clip.intersection(closed)
            closed = _filter_small_polygons(closed, area_threshold_m2=float(area_threshold_m2))
            if closed is not None and not closed.is_empty and len(_iter_polygons(closed)) <= 1:
                return closed
        except Exception:
            pass

    try:
        outer = merged.convex_hull
    except Exception:
        return remaining_geometry
    ...
    if assignment_geometry is not None and not assignment_geometry.is_empty:
        try:
            clipped = assignment_geometry.intersection(outer)
        except Exception:
            clipped = outer
        if clipped is not None and not clipped.is_empty and len(_iter_polygons(clipped)) <= 1:
            outer = clipped
        else:
            try:
                assignment_outer = broad_clip if broad_clip is not None else assignment_geometry.convex_hull
                hull_clipped = assignment_outer.intersection(outer)
                if hull_clipped is not None and not hull_clipped.is_empty:
                    remaining_area = max(0.0, float(remaining_geometry.area or 0.0))
                    hull_area = max(0.0, float(hull_clipped.area or 0.0))
                    max_allowed_area = max(remaining_area * 1.12, remaining_area + 250_000.0)
                    if hull_area <= max_allowed_area:
                        outer = hull_clipped
                    else:
                        outer = clipped
            except Exception:
    ...
    outer = _filter_small_polygons(outer, area_threshold_m2=float(area_threshold_m2))
    return outer if outer is not None and not outer.is_empty else remaining_geometry
...
def _restore_planned_cut_progress(state: _MissionAreaState) -> None:
    ...
    if restored_indexes:
        ...
        state.progress_origin_line_index = 0 if str(state.mission_type) == "line" else min(restored_indexes)
...
def _apply_area_row_progress_to_state(
    ...
    *,
    ...
    state.area_progress_source = "0401_area_row_pass"
    state.area_progress_sweep_points = int(frontier_index) + 1
    ...
    state.area_progress_confidence = "planned_row_chainage_pass"
    if frontier_index <= current_boundary:
        return False

    state.progress_origin_line_index = 0
    state.progress_boundary_line_index = int(frontier_index)
    state.progress_direction_sign = 1
    state.completed_cut_line_indexes = set(range(0, int(frontier_index) + 1))
    state.last_cut_line_index = max(int(state.last_cut_line_index), int(frontier_index))
    state.provisional_frontier_line_index = None
    return True
    ...
    if str(mission_type) == "area":
        if area_progress_details:
            strongest_progress = max(
                ...
                key=lambda item: int(item.get("mappedBoundaryLineIndex") or -1),
            )
...
class MissionProgressAreaSnapshotMonitor:
    ...
    def update_agent_status(self, *, timestamp_ms: int | None, agent_states: list[dict[str, Any]], fuel_state_map: dict[int, str] | None = None) -> None:
        ...
        for item in agent_states or []:
            ...
            if str(state.mission_type) == "area" and current_waypoint_id is not None:
                state.area_progress_current_waypoint_id = int(current_waypoint_id)
            if (
                str(state.mission_type) == "area"
                and state.planned_cut_lines
    ...
    def _build_group_remaining_detail(self, states: list[_MissionAreaState]) -> tuple[dict[str, Any], float]:
        ...
        replan_geometry = _top_level_area_replan_geometry(
            ...
            bridge_gap_m=max(
                5.0,
                min(
                    max(float(state.cut_half_width_m) for state in states) * 0.5,
                    25.0,
                ),
            ),
        )
