...
class WaypointPIDController:
    ...
    def _should_force_waypoint_advance(
        ...
        boundary_target = self.curr_idx in self.block_indices or self.curr_idx + 1 >= len(self.targets)
        if is_flyover and boundary_target:
            # A mission boundary/final flyover must be reachable by fixed-wing turn geometry;
            # otherwise high-speed return legs can orbit around the boundary waypoint forever.
            capture_radius = max(capture_radius, self._fixed_wing_capture_radius(gains, flyover=False))
    ...
    def update(self, dt: float, dem=None, wall_dt: float | None = None) -> bool:
        ...
        if (
            not self.blocked
            and not self.is_loitering
            and int(target.pass_type or 0) == 2
            and self.curr_idx not in self.block_indices
        ):
            loiter_prop = target.loiter if isinstance(target.loiter, dict) else None
