...
def main() -> int:
    ...
    p.add_argument("--speed-uav", type=float, default=DEFAULT_SPEED_UAV, help="UAV tuning speed target (m/s)")
    p.add_argument("--speed-lah", type=float, default=DEFAULT_SPEED_LAH, help="LAH tuning speed target (m/s)")
    ...
    def run_one(name: str, tuner_path: Path, test_db: Path, runtime_db: Path) -> None:
        ...
        speed_target = float(args.speed_uav) if name == "uav" else float(args.speed_lah)
