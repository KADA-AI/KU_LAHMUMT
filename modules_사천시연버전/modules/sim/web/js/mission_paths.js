...
const buildSweepFeatures = (payload, colors, selectedAgent, selectedWaypoint) => {
      ...
      coordinateList.forEach((coord, pointIdx) => {
        ...
        features.push({
          ...
          properties: {
            ...
            pointOpacity,
            pointStrokeOpacity,
            pointRadius: isSelectedWaypoint ? (isEndpoint ? 7.2 : 5.6) : isEndpoint ? 4.5 : 3.2,
...
export const initMissionPaths = (map) => {
  ...
  const applyWaypointLabelStyle = (el, entry, color) => {
    ...
    const primaryColor = isSelected || isCurrent ? "#0d1117" : color || "#e7eddc";
    ...
    el.style.zIndex = isSelected ? "5" : isCurrent ? "4" : isDone ? "2" : "1";
  ...
  const updateSweepSource = (payloadForAreas = pendingData) => {
    ...
    const nextSweepFeatures = buildSweepFeatures(payloadForAreas, colors, selectedAgent, selectedWaypoint);
    ...
    sweepFeatures = selectedWaypoint && !hasSelectedSweep
      ? buildSweepFeatures(payloadForAreas, colors, selectedAgent, null)
      : nextSweepFeatures;
    source.setData({
      type: "FeatureCollection",
