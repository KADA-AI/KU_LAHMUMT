...
_UAV_IDS = [4, 5, 6]
_MISSION_PLANNER_DIR = Path(__file__).resolve().parents[3] / "MissionPlanner"
...
# ---------------------------------------------------------------------------
@lru_cache(maxsize=1)
def _cached_fov_db_rows() -> Tuple[Tuple[float, float, float, float], ...]:
        ...
        with _FOV_DB_PATH.open("r", encoding="utf-8-sig", newline="") as fh:
            ...
            for row in reader:
                try:
                    ...
                    continue
                if width_m > 0.0:
                    rows.append((float(width_m), float(sep_m), float(vel), float(fov)))
    except Exception:
        rows = []
