...
def _fov_db_min_sep_for_fov(fov_deg: Any) -> float:
    ...
    adjust_scale = _runtime_camera_adjust_scale_no_log()
    db_sig = _fov_db_cache_signature()
...
def _route_offset_cap_m_for_waypoint(waypoint: Dict[str, Any]) -> float:
    ...
    if fov_deg is None or fov_deg <= 0.0:
        return 0.0
    sep_m = _fov_db_min_sep_for_fov(float(fov_deg))
    if sep_m <= 0.0:
        return 0.0
    return max(float(sep_m) * _runtime_line_route_offset_scale(), 1.0)
...
def _clamp_signed_offset_for_waypoint(
    ...
    if not math.isfinite(float(signed)):
        return 0.0
    cap_m = _route_offset_cap_m_for_waypoint(waypoint)
