...
_SEP_DB_CACHE_LOCK = threading.Lock()
_SEP_DB_CACHE_SIG: Tuple[str, int, int] | None = None
_SEP_DB_CACHE_ROWS: List[Tuple[float, float]] | None = None
...
def _load_sep_db(path: Path) -> List[Tuple[float, float]]:
    global _SEP_DB_CACHE_SIG, _SEP_DB_CACHE_ROWS
    sig = _db_sig(path)
    if sig is not None:
        with _SEP_DB_CACHE_LOCK:
            if _SEP_DB_CACHE_SIG == sig and isinstance(_SEP_DB_CACHE_ROWS, list):
                return _SEP_DB_CACHE_ROWS

    rows: List[Tuple[float, float]] = []
    with _SEP_DB_CACHE_LOCK:
        if sig is not None and _SEP_DB_CACHE_SIG == sig and isinstance(_SEP_DB_CACHE_ROWS, list):
            return _SEP_DB_CACHE_ROWS
        for r in read_fov_db_rows_from_path(path):
            w = _to_float(r.get("width"), -1.0)
            s = _to_float(r.get("sep"), -1.0)
            if w > 0 and s > 0:
                rows.append((w, s))
        if sig is not None:
            _SEP_DB_CACHE_SIG = sig
            _SEP_DB_CACHE_ROWS = rows
    return rows
