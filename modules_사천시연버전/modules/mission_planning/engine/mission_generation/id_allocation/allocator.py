...
def _sync_active_db_scope() -> None:
    global _ACTIVE_DB_ROOT, _STORE, _state, _WAYPOINT_SIGNATURE_CACHE
    try:
...
def _record_waypoint_usage(value: int, *, signature: dict | None = None) -> None:
    global _WAYPOINT_SIGNATURE_CACHE
    started = time.perf_counter()
    ...
    try:
        target.parent.mkdir(parents=True, exist_ok=True)
        previous = _read_waypoint_usage_record()
...
def mark_waypoint_files_written(max_waypoint_id: int | None = None) -> None:
    ...
    with _LOCK:
        ...
        if isinstance(signature, dict):
            _WAYPOINT_SIGNATURE_CACHE = dict(signature)
        _record_waypoint_usage(value, signature=signature)
    _emit_id_metric(
