...
def _build_input_missions(
    ...
    *,
    ...
    for seq, mission in enumerate(imp_payload.get("inputMissionList") or [], start=1):
        ...
        out: Dict[str, Any] = {
            ...
            "PolyLine": {"Width": 0, "CoordinateListN": 0, "CoordinateList": []},
            "Polygons": {"AreaListN": 0, "AreaList": []},
            "IsDisplay": bool(display_default),
        }
        ...
        if lines:
            out["ShapeType"] = 2
            line_coords = lines[0].get("coordinateList") if lines else []
            width = _int_value(lines[0].get("width", 0)) if lines else 0
            out["PolyLine"] = {
                "Width": width,
                "CoordinateListN": len(line_coords or []),
                "CoordinateList": _coord_list(line_coords or []),
            }
            out["Polygons"] = {"AreaListN": 0, "AreaList": []}
        elif areas:
            out["ShapeType"] = 3
            out["PolyLine"] = {"Width": 0, "CoordinateListN": 0, "CoordinateList": []}
            out["Polygons"] = {
                "AreaListN": len(areas),
...
def build_scenario(
    *,
    ...
    missions_out = _build_input_missions(
        imp_payload, coordinate_template=coordinate_template, display_default=display_default
    )
