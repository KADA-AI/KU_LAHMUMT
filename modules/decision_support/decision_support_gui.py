...
from modules.common.qt_safety import set_enabled_if_changed, set_text_if_changed, set_value_if_changed
...
class MainWindow(QMainWindow):
    ...
    def __init__(self, *args, **kwargs):
        ...
        if self._viewer_only:
            self._append_log_line("[VIEWER] passive decision GUI; service owns 0901/0701/0102")
        else:
    ...
    def _resolve_mode_slider_value(self, text: str) -> int:
        norm = re.sub(r"\s+", "", str(text)).lower()
        return int(_MODE_TEXT_ALIASES.get(norm, 1))

    def _should_ignore_ctrl_mode_change(self, requested_slider: int) -> bool:
        requested_code = _SLIDER_TO_SYSTEM_MODE.get(int(requested_slider))
        if self._system_mode_code == _MISSION_EXECUTE_CODE and requested_code != _MISSION_EXECUTE_CODE:
    ...
    def _on_mode_slider_changed(self, val: int):
        labels = _MODE_LABELS
        try:
            set_text_if_changed(self.mode_now, labels[int(val)])
        except Exception:
            pass
        self._update_system_mode_state_from_slider(val)
        self._power_on = True
    ...
    def _update_tx_table_enabled(self, enabled: bool):
        try:
            ...
            if tbl is None:
                return
            set_enabled_if_changed(tbl, enabled)
            for r in range(tbl.rowCount()):
                w = tbl.cellWidget(r, 3)
                if w is not None and hasattr(w, "setEnabled"):
                    set_enabled_if_changed(w, enabled)
        except Exception:
            pass
    ...
    def _is_0102_push_enabled(self) -> bool:
        raw = str(os.getenv("KU_MOB_0102_PUSH_ENABLED", "0") or "0").strip().lower()
    ...
    def _start_0102_stream(self, _retry: int = 0):
        ...
        if not self._is_0102_push_enabled():
            self._append_log_line("[0102] auto status push disabled (set KU_MOB_0102_PUSH_ENABLED=1 to enable)")
            return
    ...
    def _ensure_selfcheck_0102(self, on: bool) -> bool:
        if on and not self._is_0102_push_enabled():
            self._append_log_line("[0102] self-check stream disabled (set KU_MOB_0102_PUSH_ENABLED=1 to enable)")
            self._stop_all_periodic()
            return False
    ...
    def _handle_ctrl_payload(self, payload: dict):
        if handle_window_control(self, payload, role="decision", log=self._append_log_line):
            return
        ...
        if cmd == "mode":
            ...
            if self._should_ignore_ctrl_mode_change(self._resolve_mode_slider_value(text)):
                return
    ...
    def _send_0102_when_ready(self, _retry: int = 0):
        if not self._is_0102_push_enabled():
            self._append_log_line("[0102] startup status push skipped (set KU_MOB_0102_PUSH_ENABLED=1 to enable)")
            return
        if not getattr(self, "_bus_ready", False):
            if _retry < 20:
                if _retry == 0:
    ...
    def _send_self_check_0102(self, status: int = 1, _retry: int = 0):
        ...
        if status_int == 1 and not self._is_0102_push_enabled():
            self._append_log_line("[0102] self-check push skipped (set KU_MOB_0102_PUSH_ENABLED=1 to enable)")
            return
    ...
    def _push_0701_from_entries(self, option_entries: list[dict]) -> None:
            ...
            entries.append(
                {
                    ...
                    "optionMeta": dict(entry.get("optionMeta") or {}),
                }
            )
        if not entries:
        ...
        if ok:
            ...
            try:
                ...
                if getattr(self, "_tab", None) is not None:
                    self.tab_mark_sent_sig.emit(_z4("0701"), raw_bytes)
            except Exception:
                pass
        else:
