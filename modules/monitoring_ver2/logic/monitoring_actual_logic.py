...
def run_monitoring_procedure(
    ...
    def _progress_entries_for_agent(agent_state, agent_plan):
        ...
        if current_wp is None:
            ...
            # Avoid falsely completing missions when an unknown waypoint ID arrives.
            # Only map fallback if the waypoint lies within a known mission range.
            fallback_idx = None
            fallback_pos = None
            if current_wp is not None:
                for idx, mission in enumerate(missions):
                    ...
                    try:
                        ...
                        waypoint_ints = []
                    if not waypoint_ints:
                        continue
                    min_wp, max_wp = min(waypoint_ints), max(waypoint_ints)
                    if not (min_wp <= current_wp <= max_wp):
                        continue
