...
DEFAULT_REPLAN_DETAILS: Dict[str, Any] = {
    "status": "normal",
    "source": None,
    "lastUpdated": None,
    "roiCoordinate": None,
...
# --------------------------------------------------------------------------- #
# Replan detail helpers (legacy support)
# --------------------------------------------------------------------------- #
...
def load_replan_level_details() -> Dict[str, Any]:
    ...
    try:
        ...
        data = {}
    return _merge_replan_defaults(data)
...
# --------------------------------------------------------------------------- #
# Target info helpers (new snapshot for 0402)
# --------------------------------------------------------------------------- #
...
def load_target_info() -> Dict[str, Any]:
    ...
    try:
        ...
        data = {}
    merged = _merge_target_defaults(data)
...
def _ensure_file(filename: str, initial_data: Dict[str, Any]) -> Path:
    target = db_paths.get_db_subpath("DSS_Internal", filename)
    target.parent.mkdir(parents=True, exist_ok=True)
    if not target.exists():
        _write_json(target, dict(initial_data))
    return target


def _merge_replan_defaults(data: Dict[str, Any]) -> Dict[str, Any]:
    merged = dict(DEFAULT_REPLAN_DETAILS)
    ...
    if "targetList" not in merged:
        merged["targetList"] = {}
    return merged


def _merge_target_defaults(data: Dict[str, Any]) -> Dict[str, Any]:
...
def _extract_roi_entries(message: Any) -> list[Dict[str, Any]]:
    ...
    if roi_list is None:
        roi_single = _safe_get(message, "roiInfo", "ROIInfo")
...
def _serialize_target(
    ...
    *,
    ...
    # Calculate elapsed time since first detection for active targets.
...
def _upsert_roi_entry(target_map: Dict[str, Dict[str, Any]], roi_entry: Dict[str, Any], timestamp: Optional[int]) -> None:
