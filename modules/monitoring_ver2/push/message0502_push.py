...
try:
    from modules.common.push import message0502_push as _common_push
except ModuleNotFoundError:
    import importlib

    _common_push = importlib.import_module("modules.common.push.message0502_push")
...
def make_and_push(body, node_messenger) -> bytes:
    if isinstance(body, dict):
        body = MissionEndRequestBodyModel(**_normalize_dict(body))
    elif not isinstance(body, MissionEndRequestBodyModel):
        raise TypeError(
            f"body must be a MissionEndRequestBodyModel or dict, not {type(body).__name__}"
        )
...
def make_random_and_push(node_messenger) -> bytes:
    """Delegate random payload generation to the shared common implementation."""
