# modules/monitoring/receive/message0601_receiver.py
import traceback
...
from data.message_models import BaseBehaviorModel
...
class BasicActionReceiver_0601(IFusionReceive[BasicAction], IsLocal, IsSingletone):
    ...
    def Receive(self, data: BasicAction, src):
        try:
            ...
            python_data = BaseBehaviorModel(
                timestamp=_get(data, "timestamp", "Timestamp"),
                source=_get(data, "source", "Source"),
                aircraft=_get(data, "aircraft", "Aircraft"),
                flightMode=_get(data, "flightMode", "FlightMode"),
                filmingMode=_get(data, "filmingMode", "FilmingMode"),
            )

            # 데이터를 중앙 저장소에 저장
            ReceiveStorage().set_data("0601", python_data)

            # Manager 및 다른 모듈에 데이터 수신 알림
            notify_to_manager("0601", python_data)

        except Exception as e:
            print(f"[ERROR][Receive-0601] traceback ↓↓↓")
            traceback.print_exc()
            print(f"[ERROR][Receive-0601] Exception: {e}")