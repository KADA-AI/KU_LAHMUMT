...
class MonitoringTab(QWidget):
    ...
    def _refresh_mission_overview(self) -> None:
        ...
        mission_plan_id = _to_int(context.get("missionPlanID"))
        ...
        if self.plan_id_value:
            self.plan_id_value.setText(str(mission_plan_id) if mission_plan_id is not None else "-")
        ...
        mission_rows: List[Tuple[Any, Dict[str, Any]]] = []
        ...
        for aircraft_id, payload in aircraft_data.items():
            ...
            for mission in missions:
                mission_rows.append((aircraft_int, mission))
                input_id = _to_int(mission.get("inputMissionID"))
                if input_id is None:
                    continue
                entry = input_summary.setdefault(
                    input_id, {"aircraft": set(), "count": 0, "completed": True}
                )
                entry["aircraft"].add(aircraft_int)
                entry["count"] += 1
        ...
        if self.input_table is not None:
            ...
            for row, input_id in enumerate(sorted(input_summary.keys())):
                ...
                status = "완료" if entry["completed"] else "진행 중"
            ...
            if self.plan_completion_value:
                ...
                completed_inputs = sum(1 for info in input_summary.values() if info["completed"])
                self.plan_completion_value.setText(
                    f"{completed_inputs} / {total_inputs}" if total_inputs else "0 / 0"
                )
        else:
            if self.plan_completion_value:
                self.plan_completion_value.setText("0 / 0")
        ...
        if self.mission_table is not None:
            ...
            for row, (aircraft_id, mission) in enumerate(mission_rows):
                ...
                waypoint_text = ", ".join(
                    str(_to_int(wp) if _to_int(wp) is not None else wp) for wp in waypoint_ids
                ) or "-"
                ...
                if is_done:
                    background = QColor(255, 220, 220)
                elif active_input_id is not None and input_id == active_input_id:
                    background = QColor(220, 235, 255)
                if background:
