...
export function createAnalysisPanel(container) {
  ...
  // Search
  const searchRow = _el("div", "ap-search-row");
  const searchInput = _el("input", "ap-search-input");
  searchInput.type = "search";
  searchInput.placeholder = "Search location, field, rule, source";
  ...
  function renderSummary() {
    ...
    for (const item of topRules(allIssues, 3)) {
      const ruleBadge = _el("span", "ap-summary-rule", `${item.ruleId} ${item.count}`);
      summaryEl.append(ruleBadge);
    }
  ...
  function renderList() {
    ...
    const filtered = allIssues.filter(
      (i) => severityFilter[i.severity] && categoryFilter[i.category] && matchesSearch(i, query),
    );
    ...
    const countLabel = _el("div", "ap-list-count", `${filtered.length}\uAC74 \uD45C\uC2DC \uC911 / ${allIssues.length}\uAC74`);
  ...
  function buildIssueCard(issue) {
    ...
    if (issue.ruleId || issue.source) {
      const metaRow = _el("div", "ap-card-meta");
      if (issue.ruleId) metaRow.append(_el("span", "ap-card-rule", issue.ruleId));
      if (issue.source) metaRow.append(_el("span", "ap-card-source", issue.source));
      card.append(metaRow);
    }

    card.append(fieldRow, msgRow);

    // Value / Expected (if present)
    if ((issue.value !== null && issue.value !== undefined) || issue.expected) {
    ...
    if (issue.hint) {
      const hintRow = _el("div", "ap-card-hint", String(issue.hint));
      card.append(hintRow);
    }
...
function topRules(issues, limit) {
