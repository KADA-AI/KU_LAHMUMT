...
class LogAnalyzerHandler(BaseHTTPRequestHandler):
    ...
    def _serve_scenario_validate(self) -> None:
        ...
        try:
            ...
            issues.append({
                ...
                "location": "Validator",
                "field": "raw",
                "message": f"raw validation failed: {type(exc).__name__}: {exc}",
                "value": str(exc),
                "expected": "raw validator completes",
                "ruleId": "VALIDATOR-ENGINE",
                "source": "logAnalyzer validation runtime",
            })
        ...
        if entry is not None:
            try:
                ...
                issues.append({
                    ...
                    "location": "Validator",
                    "field": "parsed",
                    "message": f"parsed validation failed: {type(exc).__name__}: {exc}",
                    "value": str(exc),
                    "expected": "parsed validator completes",
                    "ruleId": "VALIDATOR-ENGINE",
                    "source": "logAnalyzer validation runtime",
                })
        ...
        summary = {
            ...
            "byCategory": dict(Counter(i.get("category", "") for i in issues if i.get("category"))),
            "byRule": dict(Counter(i.get("ruleId", "") for i in issues if i.get("ruleId"))),
        }
    ...
    def _serve_static(self, path: str) -> None:
        ...
        mime, _ = mimetypes.guess_type(str(file_path))
        content_type = _ensure_utf8_content_type(mime or "application/octet-stream")
        # The analyzer is iterated locally and is commonly relaunched in an
        # already-open browser tab. Never let that tab keep an older UI bundle.
        self._send_bytes(
            HTTPStatus.OK,
            file_path.read_bytes(),
            content_type,
