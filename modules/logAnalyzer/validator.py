...
def _validate_coordinate(coord: dict, location: str) -> list[dict]:
    ...
    for field, value, expected in (
        ("latitude", lat, "double -90..90"),
        ("longitude", lon, "double -180..180"),
...
def _validate_line_list(lines: Any, loc: str, issues: list[dict]) -> None:
    ...
    for i, line in enumerate(lines):
        ...
        if width is not None:
            issues.extend(_check_icd_int(width, "lineList.width", line_loc, 0, 50000))
...
def _validate_lat_lon(obj: Any, location: str, field_prefix: str = "") -> list[dict]:
    ...
    if lat is None:
        issues.append(_issue("error", "structure", location, f"{field_prefix}.latitude".strip("."),
                             "latitude is required", None, "double -90..90"))
    else:
    ...
    if lon is None:
        issues.append(_issue("error", "structure", location, f"{field_prefix}.longitude".strip("."),
                             "longitude is required", None, "double -180..180"))
    else:
...
def _validate_0701_mission_plan_option_info(
    ...
    *,
    ...
    for idx, option in enumerate(_as_list(option_list)):
        ...
        target_count = _pick(option, "targetIDListN", "TargetIDListN")
        target_list = _pick(option, "targetIDList", "TargetIDList")
        if target_count is not None:
            for issue in _check_uint(target_count, "targetIDListN", opt_loc):
                issue["ruleId"] = rule_id
                issue["source"] = source
                issues.append(issue)
        if target_list is not None:
            issues.extend(_check_list(target_list, "targetIDList", opt_loc))
            actual_count = len(_as_list(target_list))
            target_count_int = _coerce_int(target_count)
            if target_count_int is not None and target_count_int != actual_count:
                issues.append(_issue("error", "rule", opt_loc, "targetIDListN",
                                     f"targetIDListN={target_count_int} does not match targetIDList length {actual_count}",
                                     target_count_int, actual_count,
                                     rule_id=rule_id,
                                     source="ICD 0701 Option.targetIDList"))
            for target_idx, target_item in enumerate(_as_list(target_list)):
                target_loc = f"{opt_loc} > targetIDList[{target_idx}]"
                if isinstance(target_item, dict):
                    target_id = _pick(target_item, "targetID", "TargetID", "targetId")
                else:
                    target_id = target_item
                if target_id is None:
                    issues.append(_issue("error", "structure", target_loc, "targetID",
                                         "targetID is required", None, "uint32 > 0",
                                         rule_id=rule_id,
                                         source="ICD 0701 TargetID"))
                    continue
                issues.extend(_require_positive_uint(target_id, "targetID", target_loc, rule_id=rule_id, source="ICD 0701 TargetID"))
        else:
            legacy_target = _pick(option, "target", "Target")
            if legacy_target is not None:
                issues.append(_issue("warning", "structure", opt_loc, "target",
                                     "legacy target field is present; ICD 0701 now uses targetIDList",
                                     legacy_target, "targetIDListN + targetIDList",
                                     rule_id=rule_id,
                                     source="ICD 0701 Option.targetIDList"))
