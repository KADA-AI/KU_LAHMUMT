...
class _MiniMap(QWidget):
    ...
    def paintEvent(self, ev):
        ...
        lats = [x for x, _ in all_pts]
        lons = [y for _, y in all_pts]
        min_lat, max_lat = min(lats), max(lats)
        min_lon, max_lon = min(lons), max(lons)
        pad = 1e-6
        if abs(max_lat - min_lat) < pad:
            max_lat += pad
            min_lat -= pad
        if abs(max_lon - min_lon) < pad:
            max_lon += pad
            min_lon -= pad
        ...
        for aid, lst in self._hist.items():
            ...
            for lat, lon in lst[-60:]:
                ...
                if prev:
                    p.drawLine(
                        QPointF(prev[0], prev[1]), QPointF(x, y)
                    )  # ← QPointF 로 타입 고정
                prev = (x, y)
            p.setBrush(col)
...
class _Agent0401Panel(QGroupBox):
        ...
        for k, v in walk(obj):
            ...
            try:
                if aid is None and kl in ("aircraftid", "agentid", "vehicleid", "id"):
                    ...
                    lon = float(v)
                elif alt is None and kl in ("altitude", "alt"):
                    alt = float(v)
                elif spd is None and kl in ("speed", "groundspeed"):
                    spd = float(v)
            except Exception:
                continue
        if aid is not None and lat is not None and lon is not None:
    ...
    def _upsert_row(self, aid, lat, lon, alt, spd):
        ...
        if row < 0:
            ...
            self._tbl.setItem(
                row, 3, QTableWidgetItem("-" if alt is None else f"{alt:.1f}")
            )
            self._tbl.setItem(
                row, 4, QTableWidgetItem("-" if spd is None else f"{spd:.1f}")
            )
            ...
            if alt is not None:
                self._tbl.item(row, 3).setText(f"{alt:.1f}")
            if spd is not None:
                self._tbl.item(row, 4).setText(f"{spd:.1f}")
...
class _MissionSidePanel(QGroupBox):
    def __init__(self, on_log=None, parent=None):
        ...
        r1.setSpacing(8)
        self._lamp_0401 = QLabel(row1)
        self._set_lamp(self._lamp_0401, False)
        lbl1 = QLabel("0401 Log", row1)
        self._path_0401 = QLineEdit(row1)
        self._path_0401.setReadOnly(True)
        self._path_0401.setPlaceholderText("선택된 파일 없음")
        btn1 = QPushButton("찾아보기", row1)
        btn1.setMinimumWidth(84)
        btn1.clicked.connect(lambda: self._browse("0401"))
        r1.addWidget(self._lamp_0401)
        r1.addWidget(lbl1)
        r1.addWidget(self._path_0401)
        r1.addWidget(btn1)
        lay.addWidget(row1)
        ...
        r2.setSpacing(8)
        self._lamp_0402 = QLabel(row2)
        self._set_lamp(self._lamp_0402, False)
        lbl2 = QLabel("0402 Log", row2)
        self._path_0402 = QLineEdit(row2)
        self._path_0402.setReadOnly(True)
        self._path_0402.setPlaceholderText("선택된 파일 없음")
        btn2 = QPushButton("찾아보기", row2)
        btn2.setMinimumWidth(84)
        btn2.clicked.connect(lambda: self._browse("0402"))
        r2.addWidget(self._lamp_0402)
        r2.addWidget(lbl2)
        r2.addWidget(self._path_0402)
        r2.addWidget(btn2)
        lay.addWidget(row2)

        # 임무 데이터 기반 덮어쓰기 옵션
        self._chk_use_mission = QCheckBox("임무 데이터 기반 0401 덮어쓰기", self)
        self._chk_use_mission.setToolTip(
            "임무 계획에 저장된 각 무인기의 비행 경로를 따라 currentWaypointID와 연료를 자동으로 갱신합니다."
        )
    ...
    def _set_lamp(self, w: QLabel, on: bool):
        ...
        w.setStyleSheet(
            f"border-radius:6px; background:{'#16a34a' if on else '#dc2626'}; border:1px solid rgba(0,0,0,0.1);"
        )
    ...
    def _browse(self, kind: str):
        paths, _ = QFileDialog.getOpenFileNames(
            ...
            f"{kind} 파일 선택",
            "",
            "NDJSON/JSON (*.ndjson *.json);;모든 파일 (*.*)",
        )
        if not paths:
            return
        if kind == "0401":
            self._paths_0401 = list(paths)
            self._path_0401.setText(
                paths[0] if len(paths) == 1 else f"{len(paths)}개 파일 선택"
            )
            self._set_lamp(self._lamp_0401, True)
            try:
                self.tracker_0401._map.clear()  # ← 새 파일 선택 시 히스토리 초기화
            except Exception:
                pass
        else:
            self._paths_0402 = list(paths)
            self._path_0402.setText(
                paths[0] if len(paths) == 1 else f"{len(paths)}개 파일 선택"
            )
            self._set_lamp(self._lamp_0402, True)
        if callable(self._on_log):
            self._on_log(f"[FILES] {kind} → {', '.join(paths)}")
...
class _MissionPlanWaypointOverlay:
    ...
    def prepare(self, *, force: bool = False) -> bool:
        ...
        self._prepared = True
        summary = ", ".join(
            f"UAV{aid}:{len(seq)}개" for aid, seq in sorted(self._waypoints.items())
        )
        self._log_info(
            f"[REPLAY] 임무 기반 0401 덮어쓰기 준비 완료 ({summary}, step={self._advance_every})"
        )
        return True
    ...
    def _collect_waypoints(self, imp_id: int) -> list[int]:
        try:
            imp_path = db_paths.get_db_subpath(
                "IndividualMissionPlan", f"{imp_id}.json"
            )
        except Exception:
            return []
...
class _ReplayManager(QObject):
    def __init__(
        self, tab: IntegrationTab, messenger, logger, side_panel: _MissionSidePanel
    ):
        super().__init__(tab)
        self._tab = tab
    ...
    def start_from_two(self, paths_0401: list[str], paths_0402: list[str]):
        ...
        if self._side.use_mission_overlay():
            if self._overlay.prepare(force=True):
                ...
                if callable(self._log):
                    self._log(
                        "[REPLAY] 임무 데이터 덮어쓰기 준비 실패 → 기본 데이터 사용"
                    )
        ...
        def first_ts(rows):
            if not rows:
                return None
            rows_sorted = sorted(
                rows, key=lambda x: (float("inf") if x[0] is None else x[0])
            )
        ...
        for msg_id, rows in buckets.items():
            ...
            if msg_id == "0401":
                ...
                if callable(self._log):
                    self._log(
                        f"[REPLAY] 0401 {len(rows)}건 예약 완료 (5Hz, anchor={anchor})"
                    )
                ...
                if callable(self._log):
                    self._log(
                        f"[REPLAY] 0402 {cnt}건 예약 완료 (event, anchor={anchor})"
                    )
    ...
    def _schedule_send(self, msg_id: str, body: dict, delay_ms: int):
        ...
        def iter_states(obj):
            ...
            if isinstance(lst, list) and lst:
                for it in lst:
                    aid = (
                        get_ci(it, "AircraftID")
                        or get_ci(it, "agentID")
                        or get_ci(it, "vehicleID")
                        or get_ci(it, "id")
                    )
        ...
        def do_send():
            ...
            try:
                push_message(
                    ...
                    self._msgr,
                    on_done=(
                        (lambda mid, raw: self._tab._mark_single_sent(row, mid, raw))
                        if row >= 0
                        else None
                    ),
                    body_dict=body,
                ...
                if callable(self._log):
                    self._log(f"[REPLAY] {msg_id} push 실패: {e}")
...
class MainWindow(QMainWindow):
    ...
    def __init__(self, *args, **kwargs):
        ...
        h.setSpacing(10)
        h.addWidget(self._side, 0, Qt.AlignTop)
        h.addWidget(right, 1)
        self.setCentralWidget(center)

        # 리플레이 매니저
        self._replay = _ReplayManager(
            self._tab, NodeMessenger, self._append_log_line, self._side
        )
        ...
        start_ctrl_listener(
            ...
            lambda payload: (
                send_status_ok("INT")
                if (payload or {}).get("cmd") == "self_check"
                and int((payload or {}).get("status", 0)) == 1
                else None
            ),
        )
    ...
    def _handle_ctrl_payload(self, payload: dict):
        ...
        now = time.monotonic()
        last = getattr(self, "_last_ctrl_ts", {}).get(key, 0.0)
        if (now - last) < 1.0:
            return
    ...
    def _on_click_start_sim(self):
        ...
        p0402 = []
        try:
            p0401 = self._side.selected_paths_0401()
            p0402 = self._side.selected_paths_0402()
        except Exception:
            pass
