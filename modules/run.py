...
_RUN_RELEASE = load_release_info()
RUNPY_GUI_UPDATE_VERSION = _RUN_RELEASE.display_version
RUNPY_GUI_UPDATE_DATE = _RUN_RELEASE.release_date
RUNPY_GUI_UPDATE_NOTE = f"현재 돌린 코드: {_RUN_RELEASE.code_label}"
...
def _bootstrap_hidden_windows_launch() -> None:
    ...
    root_launcher = _PROJECT_ROOT / "run.py"
    subprocess.Popen(
        [str(pythonw), str(root_launcher), *sys.argv[1:]],
...
def _force_reset_id_files() -> None:
    ...
    """
    try:
        data_def_dir = _MODULE_ROOT / "mission_planning" / "MissionPlanner" / "data_def"
...
def _load_qss(app: QApplication):
    candidates = [
        _MODULE_ROOT / "app" / "resources" / "style.qss",
...
def _reset_ids_for_active_scenario() -> None:
    """
    Reset mission/IMP/path counters for the currently active scenario DB root.
    This is used on new sortie/scenario activation so IDs restart from baseline
    ...
    try:
        ...
        pass
# 메인 윈도우 구현은 modules 패키지 내부에서만 소유한다.
...
class DashboardOrchestrator(QObject):
    ...
    def __init__(self, window: MainWindow):
        ...
        self._last_launch_all_requested_at = 0.0
    ...
    def _configure_dashboard_quality_monitor(self) -> None:
        ...
        try:
            ...
            pass
        self._set_dashboard_quality_csv_logging_enabled(False)

    def _set_dashboard_quality_csv_logging_enabled(self, enabled: bool) -> None:
        monitor = getattr(self, "_dashboard_quality_monitor", None)
        if monitor is None:
            return
        try:
            setter = getattr(monitor, "set_csv_logging_enabled", None)
            if callable(setter):
                setter(bool(enabled))
        except Exception:
            pass
    ...
    def _bind_service_gui_buttons(self) -> None:
        ...
        for role, button in buttons.items():
            ...
            try:
                ...
                pass
        mission_creator_getter = getattr(self.win, "mission_creation_gui_button", None)
        if callable(mission_creator_getter):
            try:
                mission_creator_button = mission_creator_getter()
            except Exception:
                mission_creator_button = None
            if mission_creator_button is not None:
                try:
                    mission_creator_button.clicked.disconnect()
                except Exception:
                    pass
                try:
                    mission_creator_button.clicked.connect(
                        lambda _checked=False: self._launch_mission_creation_gui()
                    )
                except Exception:
                    pass

    def _launch_mission_creation_gui(self) -> None:
        import webbrowser

        if not self._ensure_launch_ready(show_message=True, context="임무 생성 GUI 실행"):
            return
        root = PROJECT_ROOT
        script = _MODULE_ROOT / "info_manage" / "mission_creation_gui.py"
        if not script.exists():
            try:
                self._safe_log(f"[RUN ERR] mission creation GUI script not found: {script}")
            except Exception:
                pass
            return

        host = str(os.environ.get("MISSION_CREATION_HOST") or "127.0.0.1").strip() or "127.0.0.1"
        try:
            port = int(os.environ.get("MISSION_CREATION_PORT") or 8211)
        except Exception:
            port = 8211
        ...
        existing = role_processes.get("mission_creation_gui") if isinstance(role_processes, dict) else None
        if existing is not None and existing.poll() is None:
            try:
                self._safe_log(f"[RUN] mission creation GUI already running ({url}, pid={existing.pid})")
            except Exception:
                pass
            webbrowser.open(url)
            return
        if isinstance(role_processes, dict):
            role_processes.pop("mission_creation_gui", None)
        ...
        killer = getattr(self.win, "_kill_port_occupant", None)
        if callable(killer):
            try:
                killer(port, "MissionCreationGUI")
            except Exception:
                pass

        env = os.environ.copy()
        env["PYTHONUNBUFFERED"] = "1"
        env["KU_MISSION_DB_ROOT"] = str(db_root)
        env["MISSION_CREATION_HOST"] = host
        env["MISSION_CREATION_PORT"] = str(port)
        env["KU_CONSOLE_TITLE"] = "KU Mission Creation GUI Server"
        ...
        try:
            ...
            if isinstance(role_processes, dict):
                role_processes["mission_creation_gui"] = proc
            self._safe_log(f"[RUN] mission creation GUI launched ({url}, pid={getattr(proc, 'pid', '?')})")
            QTimer.singleShot(800, lambda u=url: webbrowser.open(u))
        except Exception as exc:
            try:
                self._safe_log(f"[RUN ERR] mission creation GUI launch failed: {exc}")
            except Exception:
                pass
    ...
    def _maybe_activate_scenario(self, timestamp: int | None) -> None:
        ...
        version_log_path = info.get("version_log")
        version_log_error = info.get("version_log_error")
        if version_log_path:
            self._safe_log(
                f"[VERSION] {_RUN_RELEASE.code_label} / "
                f"{_RUN_RELEASE.display_version} @ {version_log_path}"
            )
        elif version_log_error:
            self._safe_log(f"[WARN] Scenario version log failed: {version_log_error}")
        ...
        try:
            ...
            pass
        if changed:
            _reset_ids_for_active_scenario()
            self._safe_log("[OPS] New sortie detected -> mission IDs reset for active scenario")
        if db_root_str:
            try:
    ...
    def _set_mode_text_all(self, text: str):
        ...
        for key in ("assignment", "monitoring", "decision"):
            ...
            try:
                ...
                pass
        self._set_dashboard_quality_csv_logging_enabled(norm == "임무 수행")
        self._refresh_service_status_panel()
    ...
    def _launch_all_guis(self):
        if not self._ensure_launch_ready(show_message=True, context="run.py 초기 실행"):
            return
        self._last_launch_all_requested_at = time.monotonic()
        script_names = (
        ...
        try:
            launch_stagger_ms = max(
                0,
                int(os.environ.get("KU_NFUSION_LAUNCH_STAGGER_MS") or 200),
            )
        except (TypeError, ValueError):
            launch_stagger_ms = 200
        QTimer.singleShot(
            int(launch_stagger_ms) * len(script_names),
            self._auto_launch_mission_status_monitor,
        )
    ...
    def _auto_launch_mission_status_monitor(self) -> None:
        launcher = getattr(self.win, "_launch_mission_status_monitor", None)
        if not callable(launcher):
            self._safe_log("[RUN WARN] Mission Status Monitoring auto-start is unavailable")
            return
        try:
            launcher(auto_start=True)
        except Exception as exc:
            # Dashboard and the other core modules must stay available even if
            # the optional browser monitor cannot be started.
            self._safe_log(f"[RUN WARN] Mission Status Monitoring auto-start failed: {exc}")
    ...
    def _launch_service(self, role: str) -> None:
        ...
        candidates = [
            root / script_name,
            _MODULE_ROOT / script_name,
            _MODULE_ROOT / "decision_support" / script_name,
            _MODULE_ROOT / "info_manage" / script_name,
        ]
        ...
        env["PYTHONUNBUFFERED"] = "1"
        # run.py로 기동하는 MMR/MSM/MOB는 0102 상태정보를 5 Hz로 송신한다.
        # INF는 주기 송신 대상이 아니므로 명시적으로 비활성화한다.
        env.update(
            {
    ...
    def _launch_gui(
        ...
        *,
        ...
        python_exec = preferred_console_python(sys.executable)
        ...
        try:
            proc = subprocess.Popen(
                [python_exec, str(script), *extra_args],
                cwd=str(root),
                env=env,
                start_new_session=True,
    ...
    def _append_log_global(self, text: str):
        try:
            emit_process_log("dashboard", str(text))
        except Exception:
            pass
