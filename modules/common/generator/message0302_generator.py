# modules/common/generator/message0302_generator.py
# auto-generated at 2025-08-24T14:56:18.134119+00:00
...
def _gen_RelatedMission(source: str = "DS"):
    obj = {}
    obj["relatedMissionType"] = gen_value("relatedMissionType", "uint32", MSG_ID, obj, 0)
    obj["inputMissionID"] = gen_value("inputMissionID", "uint32", MSG_ID, obj, 0)
    obj["priorMissionID"] = gen_value("priorMissionID", "uint32", MSG_ID, obj, 0)
    return obj
...
def _gen_Line(source: str = "DS"):
    obj = {}
    obj["width"] = gen_value("width", "float32", MSG_ID, obj, 0)
    obj["coordinateList"] = [ _gen_Coordinate(source=source) for _ in range(LIST_LEN) ]
    return obj

def _gen_Area(source: str = "DS"):
    obj = {}
    obj["isHole"] = gen_value("isHole", "bool", MSG_ID, obj, 0)
    obj["coordinateList"] = [ _gen_Coordinate(source=source) for _ in range(LIST_LEN) ]
    return obj

def _gen_IndividualMissionInfo(source: str = "DS"):
    obj = {}
    obj["individualMissionType"] = gen_value("individualMissionType", "uint32", MSG_ID, obj, 0)
    obj["patternType"] = gen_value("patternType", "uint32", MSG_ID, obj, 0)
    obj["autoZoomIn"] = gen_value("autoZoomIn", "bool", MSG_ID, obj, 0)
    obj["coordinateList"] = [ _gen_Coordinate(source=source) for _ in range(LIST_LEN) ]
    obj["lineList"] = [ _gen_Line(source=source) for _ in range(LIST_LEN) ]
    obj["areaList"] = [ _gen_Area(source=source) for _ in range(LIST_LEN) ]
    obj["targetID"] = gen_value("targetID", "uint32", MSG_ID, obj, 0)
    return obj

def _gen_IndividualMission(source: str = "DS"):
    obj = {}
    obj["individualMissionID"] = gen_value("individualMissionID", "uint32", MSG_ID, obj, 0)
    obj["isDone"] = gen_value("isDone", "bool", MSG_ID, obj, 0)
    obj["relatedMission"] = _gen_RelatedMission(source=source)
    obj["individualMissionInfo"] = _gen_IndividualMissionInfo(source=source)
    obj["pathID"] = gen_value("pathID", "uint32", MSG_ID, obj, 0)
    return obj

def _gen_IndividualMissionPlanData(source: str = "DS"):
    obj = {}
    obj["timestamp"] = gen_value("timestamp", "uint64", MSG_ID, obj, 0)
    obj["individualMissionPackageID"] = gen_value("individualMissionPackageID", "uint32", MSG_ID, obj, 0)
    obj["aircraftID"] = gen_value("aircraftID", "uint32", MSG_ID, obj, 0)
    obj["individualMissionList"] = [ _gen_IndividualMission(source=source) for _ in range(LIST_LEN) ]
    return obj




def make_msg0302_body(source: str = "DS"):
    """자동 생성된 메시지 바디 — MessageID: 0302, RootType: IndividualMissionPlanData"""
    ex = _gen_IndividualMissionPlanData(source=source)
...
def make_random_and_push(node_messenger):
    from generator.message0302_push import make_and_push
    return make_and_push(make_msg0302_body(), node_messenger)
