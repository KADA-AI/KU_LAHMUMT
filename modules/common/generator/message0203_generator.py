# modules/common/generator/message0203_generator.py
# auto-generated at 2025-08-24T14:56:18.130613+00:00
...
def gen_value(field_name: str, type_token: str, ctx_msg_id: str, obj_context: Optional[Dict[str, Any]]=None, depth: int=0):
    ...
    if base in ("uint32","int32","uint64","int64"):
        ...
        if lname in ("lowerlimit", "upperlimit"): return random.randint(0, 1000)
        return random.randint(0, 1000) if base.startswith("u") else random.randint(-1000, 1000)
    if base in ("float32","float64"):
        if lname == "fuel": return round(random.uniform(0.0, 100.0), 6)
...
def _gen_TakeOverInfo(source: str = "DS"):
    obj = {}
    obj["aircraftID"] = gen_value("aircraftID", "uint32", MSG_ID, obj, 0)
    obj["coordinate"] = _gen_Coordinate(source=source)
    return obj

def _gen_HandOverInfo(source: str = "DS"):
    obj = {}
    obj["aircraftID"] = gen_value("aircraftID", "uint32", MSG_ID, obj, 0)
    obj["coordinate"] = _gen_Coordinate(source=source)
    return obj

def _gen_RTBCoordinate(source: str = "DS"):
    obj = {}
    obj["latitude"] = gen_value("latitude", "float64", MSG_ID, obj, 0)
    obj["longitude"] = gen_value("longitude", "float64", MSG_ID, obj, 0)
    obj["altitude"] = gen_value("altitude", "int32", MSG_ID, obj, 0)
...
def _gen_AreaLatLon(source: str = "DS"):
    obj = {}
    obj["latitude"] = gen_value("latitude", "float64", MSG_ID, obj, 0)
    obj["longitude"] = gen_value("longitude", "float64", MSG_ID, obj, 0)
    return obj

def _gen_AltitudeLimits(source: str = "DS"):
    obj = {}
    obj["lowerLimit"] = gen_value("lowerLimit", "int32", MSG_ID, obj, 0)
    obj["upperLimit"] = gen_value("upperLimit", "int32", MSG_ID, obj, 0)
    return obj

def _gen_FlightArea(source: str = "DS"):
    obj = {}
    obj["flightAreaID"] = gen_value("flightAreaID", "uint32", MSG_ID, obj, 0)
    obj["areaLatLonList"] = [ _gen_AreaLatLon(source=source) for _ in range(LIST_LEN) ]
    obj["altitudeLimits"] = _gen_AltitudeLimits(source=source)
    return obj

def _gen_ProhibitedArea(source: str = "DS"):
    obj = {}
    obj["prohibitedAreaID"] = gen_value("prohibitedAreaID", "uint32", MSG_ID, obj, 0)
    obj["areaLatLonList"] = [ _gen_AreaLatLon(source=source) for _ in range(LIST_LEN) ]
    obj["altitudeLimits"] = _gen_AltitudeLimits(source=source)
    return obj

def _gen_FlightReferenceInfoData(source: str = "DS"):
    obj = {}
    obj["timestamp"] = gen_value("timestamp", "uint64", MSG_ID, obj, 0)
    obj["missionReferencePackageID"] = gen_value("missionReferencePackageID", "uint32", MSG_ID, obj, 0)
    obj["inputTimestamp"] = gen_value("inputTimestamp", "uint64", MSG_ID, obj, 0)
    obj["takeOverInfoList"] = [ _gen_TakeOverInfo(source=source) for _ in range(LIST_LEN) ]
    obj["handOverInfoList"] = [ _gen_HandOverInfo(source=source) for _ in range(LIST_LEN) ]
    obj["rtbCoordinateList"] = [ _gen_RTBCoordinate(source=source) for _ in range(LIST_LEN) ]
    obj["flightAreaList"] = [ _gen_FlightArea(source=source) for _ in range(LIST_LEN) ]
    obj["prohibitedAreaList"] = [ _gen_ProhibitedArea(source=source) for _ in range(LIST_LEN) ]
    return obj




def make_msg0203_body(source: str = "DS"):
    """자동 생성된 메시지 바디 — MessageID: 0203, RootType: FlightReferenceInfoData"""
    ex = _gen_FlightReferenceInfoData(source=source)
...
def make_random_and_push(node_messenger):
    from generator.message0203_push import make_and_push
