...
def _target_id_value(item: Any) -> Optional[int]:
...
def _target_id_list_from_option(item: dict) -> list[dict]:
    meta = _get_ci(item, "optionMeta")
    if isinstance(meta, dict):
        meta_items = _target_id_list_from_option(meta)
        if meta_items:
            return meta_items

    raw_list = (
        _get_ci(item, "targetIDList")
        or _get_ci(item, "targetIDs")
        or _get_ci(item, "targetIds")
        or _get_ci(item, "targetList")
    )
    ids: list[int] = []
    ...
    if not ids:
        for key in ("targetID", "targetId", "target"):
            value = _target_id_value(_get_ci(item, key))
            if value is not None and value not in ids:
                ids.append(value)

    if not ids:
        count = _target_id_value(_get_ci(item, "targetCount"))
...
def _build_option_list_from_cached() -> list[dict]:
    ...
    for idx, item in enumerate(src.get("pendingOptionList") or []):
        ...
        try:
            ...
            fuel_warning = 0
        target_id_list = _target_id_list_from_option(item)
        options.append({
            "optionID": option_id,
            "recommend": recommend,
            "optionName": opt_code,
            ...
            "distance": 15000,
            "targetIDListN": len(target_id_list),
...
def _gen_Option(source: str = "DS"):
    ...
    target_ids = sorted({random.randint(1, 20) for _ in range(random.randint(0, 3))})
    obj["targetIDListN"] = len(target_ids)
    obj["targetIDList"] = [{"targetID": target_id} for target_id in target_ids]
    return obj
