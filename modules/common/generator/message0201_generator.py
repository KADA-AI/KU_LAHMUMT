# modules/common/generator/message0201_generator.py
# auto-generated at 2025-08-24T14:56:18.125615+00:00
...
def _gen_AvailableAircraft(source: str = "DS"):
    obj = {}
...
def _gen_MissionDetail(source: str = "DS"):
    obj = {}
    obj["coordinateList"] = [ _gen_Coordinate(source=source) for _ in range(LIST_LEN) ]
...
def _gen_InputMission(source: str = "DS"):
    obj = {}
...
def _gen_InputMissionPlanData(source: str = "DS"):
    obj = {}
    obj["timestamp"] = gen_value("timestamp", "uint64", MSG_ID, obj, 0)
    obj["inputMissionPackageID"] = gen_value("inputMissionPackageID", "uint32", MSG_ID, obj, 0)
    obj["inputMissionPackageType"] = gen_value("inputMissionPackageType", "uint32", MSG_ID, obj, 0)
    obj["mainSensor"] = gen_value("mainSensor", "uint32", MSG_ID, obj, 0)
    obj["availableAircraftList"] = [ _gen_AvailableAircraft(source=source) for _ in range(LIST_LEN) ]
    obj["inputMissionList"] = [ _gen_InputMission(source=source) for _ in range(LIST_LEN) ]
    return obj




def make_msg0201_body(source: str = "DS"):
    """자동 생성된 메시지 바디 — MessageID: 0201, RootType: InputMissionPlanData"""
    ex = _gen_InputMissionPlanData(source=source)
...
def make_random_and_push(node_messenger):
    from generator.message0201_push import make_and_push
