...
class AssignmentPlanningTab(CSCTabBase):
    ...
    PUSH_MESSAGES = [
        ...
        ("0102", "모듈 상태 정보"),
        ("0301", "임무 계획"),
        # 0204는 MMR이 발신하지 않음(MSM 전용, type=1 리뷰 선송신) - 수신만 유지.
        ("0302", "개별 임무 계획"),
        ("0303", "무인기 비행 계획"),
        ("0304", "LAH 비행 계획"),
        ("0305", "재계획 수행 상태 정보"),
    ...
    def mark_received(self, msg_id: str, raw: Optional[bytes] = None):
        ...
        if str(msg_id).zfill(4) == "0902" and callable(self._replan_callback):
            try:
                ...
                self._log_replan_callback_failure(msg_id, exc)

    def _log_replan_callback_failure(self, msg_id: str, exc: BaseException) -> None:
        message = f"[ERR] 0902 replan callback failed (msgId={str(msg_id).zfill(4)}): {exc}"
        try:
            emit_process_log("mission_planning", message)
            trace_text = traceback.format_exc().strip()
            if trace_text:
                emit_process_log("mission_planning", "[TRACE] " + trace_text)
        except Exception:
            pass
        try:
