...
def _to_dict_Option(obj):
    ...
    _v = _get(obj, 'targetIDListN', 'TargetIDListN')
    if _v is not None: d['targetIDListN'] = int(_v)
    _coll = _get(obj, 'targetIDList', 'TargetIDList')
    if _coll is not None:
        d['targetIDList'] = [_to_dict_TargetID(it) for it in _coll]
    return d

def _to_dict_TargetID(obj):
    d = {}
