# modules/common/receive/message0503_receiver.py
# auto-generated at 2025-08-24T16:37:13.103866+00:00

from dll_files.nFusionImports import *            # IFusionReceive, IsLocal, IsSingletone
from nFusion.Model.msg_0503 import *            # C# 모델
...
def _to_dict_MissionResult(obj):
    d = {}
    ...
    _v = _get(obj, 'systemRecommend', 'SystemRecommend')
    if _v is not None: d['systemRecommend'] = int(_v)
    return d

class MissionResultReceiver_0503(IFusionReceive[MissionResult], IsLocal, IsSingletone):
    """0503 MissionResult 메시지 수신 리시버"""
    __namespace__ = "MissionResultReceiver_0503"

    def Receive(self, data: MissionResult, src):
        try:
            _try_save_received('0503', data)

            body = _try_read_db_body('0503', data)
            if body is None:
                body = _to_dict_MissionResult(data)

            notify("0503", json.dumps(body, ensure_ascii=False).encode("utf-8","ignore"))
