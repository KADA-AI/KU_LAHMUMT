# modules/common/receive/message0301_receiver.py
# auto-generated at 2025-08-24T16:37:13.087600+00:00

from dll_files.nFusionImports import *            # IFusionReceive, IsLocal, IsSingletone
from nFusion.Model.msg_0301 import *            # C# 모델
...
def _to_dict_MissionPlanData(obj):
    d = {}
    ...
    _v = _get(obj, 'missionPlanTimestamp', 'MissionPlanTimestamp')
    if _v is not None: d['missionPlanTimestamp'] = int(_v)
    _v = _get(obj, 'planningTime', 'PlanningTime')
    if _v is not None: d['planningTime'] = float(_v)
    _v = _get(obj, 'plannerID', 'PlannerID')
    if _v is not None: d['plannerID'] = int(_v)
    ...
    _coll = _get(obj, 'aircraftList', 'AircraftList') or []
    if _coll:
        d['aircraftList'] = [_to_dict_Aircraft(it) for it in _coll]
    return d

def _to_dict_MissionPlan(obj):
    d = {}
    _v = _get(obj, 'timestamp', 'Timestamp')
    if _v is not None: d['timestamp'] = int(_v)
    _v = _get(obj, 'missionPlanID', 'MissionPlanID')
...
class MissionPlanReceiver_0301(IFusionReceive[MissionPlan], IsLocal, IsSingletone):
    ...
    def Receive(self, data: MissionPlan, src):
        try:
            _try_save_received('0301', data)

            body = _try_read_db_body('0301', data)
            if body is None:
                body = _to_dict_MissionPlan(data)

            notify("0301", json.dumps(body, ensure_ascii=False).encode("utf-8","ignore"))
