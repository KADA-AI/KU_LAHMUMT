...
def _merge_transport_payload(base_payload: dict, transport_payload: dict) -> dict:
    ...
    if not isinstance(base_payload, dict):
        merged = dict(transport_payload or {})
    elif not isinstance(transport_payload, dict):
    ...
    replan_perf.add_elapsed(
        ...
        perf_start,
        base_keys=len(base_payload) if isinstance(base_payload, dict) else 0,
        transport_keys=len(transport_payload) if isinstance(transport_payload, dict) else 0,
