# modules/common/receive/message0801_receiver.py
# auto-generated at 2025-08-24T16:37:13.111869+00:00

from dll_files.nFusionImports import *            # IFusionReceive, IsLocal, IsSingletone
from nFusion.Model.msg_0801 import *            # C# 모델
...
def _to_dict_InitialPlanCommand(obj):
    d = {}
    ...
    _v = _get(obj, 'operatorReplanRequestTime', 'OperatorReplanRequestTime')
    if _v is not None: d['operatorReplanRequestTime'] = int(_v)
...
class InitialPlanCommandReceiver_0801(IFusionReceive[InitialPlanCommand], IsLocal, IsSingletone):
    ...
    def Receive(self, data: InitialPlanCommand, src):
        try:
            _try_save_received('0801', data)

            body = _try_read_db_body('0801', data)
            if body is None:
                body = _to_dict_InitialPlanCommand(data)

            notify("0801", json.dumps(body, ensure_ascii=False).encode("utf-8","ignore"))
