...
def _to_dict_SystemOperationMode(obj):
    d = {}
...
class SystemOperationModeReceiver_0101(IFusionReceive[SystemOperationMode], IsLocal, IsSingletone):
    ...
    def Receive(self, data: SystemOperationMode, src):
        try:
            _try_save_received('0101', data)

            body = _try_read_db_body('0101', data)
            if body is None:
                body = _to_dict_SystemOperationMode(data)

            notify("0101", json.dumps(body, ensure_ascii=False).encode("utf-8","ignore"))
