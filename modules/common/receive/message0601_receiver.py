# modules/common/receive/message0601_receiver.py
# auto-generated at 2025-08-24T16:37:13.104870+00:00

from dll_files.nFusionImports import *            # IFusionReceive, IsLocal, IsSingletone
from nFusion.Model.msg_0601 import *            # C# 모델
...
def _to_dict_BasicAction(obj):
    ...
    _v = _get(obj, 'aircraft', 'Aircraft')
    if _v is not None: d['aircraft'] = int(_v)
    _v = _get(obj, 'flightMode', 'FlightMode')
    if _v is not None: d['flightMode'] = int(_v)
    _v = _get(obj, 'filmingMode', 'FilmingMode')
    if _v is not None: d['filmingMode'] = int(_v)
    return d

class BasicActionReceiver_0601(IFusionReceive[BasicAction], IsLocal, IsSingletone):
    """0601 BasicAction 메시지 수신 리시버"""
    __namespace__ = "BasicActionReceiver_0601"

    def Receive(self, data: BasicAction, src):
        try:
            _try_save_received('0601', data)

            body = _try_read_db_body('0601', data)
            if body is None:
                body = _to_dict_BasicAction(data)

            notify("0601", json.dumps(body, ensure_ascii=False).encode("utf-8","ignore"))
