# modules/common/receive/message0806_receiver.py
# auto-generated at 2025-08-24T16:37:13.117870+00:00

from dll_files.nFusionImports import *            # IFusionReceive, IsLocal, IsSingletone
from nFusion.Model.msg_0806 import *            # C# 모델
...
def _to_dict_BootCommand(obj):
    ...
    _v = _get(obj, 'command', 'Command')
    if _v is not None: d['command'] = int(_v)
    return d
...
class BootCommandReceiver_0806(IFusionReceive[BootCommand], IsLocal, IsSingletone):
    ...
    def Receive(self, data: BootCommand, src):
        try:
            _try_save_received('0806', data)

            body = _try_read_db_body('0806', data)
            if body is None:
                body = _to_dict_BootCommand(data)

            notify("0806", json.dumps(body, ensure_ascii=False).encode("utf-8","ignore"))
