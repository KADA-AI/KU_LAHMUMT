# modules/common/receive/message0802_receiver.py
# auto-generated at 2025-08-24T16:37:13.112869+00:00

from dll_files.nFusionImports import *            # IFusionReceive, IsLocal, IsSingletone
from nFusion.Model.msg_0802 import *            # C# 모델
...
def _to_dict_MandatoryCommand(obj):
    d = {}
    ...
    _v = _get(obj, 'mandatoryType', 'MandatoryType')
    if _v is not None: d['mandatoryType'] = int(_v)
    return d
...
class MandatoryCommandReceiver_0802(IFusionReceive[MandatoryCommand], IsLocal, IsSingletone):
    ...
    def Receive(self, data: MandatoryCommand, src):
        try:
            _try_save_received('0802', data)

            body = _try_read_db_body('0802', data)
            if body is None:
                body = _to_dict_MandatoryCommand(data)

            notify("0802", json.dumps(body, ensure_ascii=False).encode("utf-8","ignore"))
