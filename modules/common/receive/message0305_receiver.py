# modules/common/receive/message0305_receiver.py
# auto-generated at 2025-08-24T16:37:13.094867+00:00

from dll_files.nFusionImports import *            # IFusionReceive, IsLocal, IsSingletone
from nFusion.Model.msg_0305 import *            # C# 모델
...
def _to_dict_ReplanStatus(obj):
    ...
    _v = _get(obj, 'missionPlanningStatus', 'MissionPlanningStatus')
    if _v is not None: d['missionPlanningStatus'] = int(_v)
    _v = _get(obj, 'replanReason', 'ReplanReason')
    if _v is not None and _v != '': d['replanReason'] = str(_v)
    return d

class ReplanStatusReceiver_0305(IFusionReceive[ReplanStatus], IsLocal, IsSingletone):
    """0305 ReplanStatus 메시지 수신 리시버"""
    __namespace__ = "ReplanStatusReceiver_0305"

    def Receive(self, data: ReplanStatus, src):
        try:
            _try_save_received('0305', data)

            body = _try_read_db_body('0305', data)
            if body is None:
                body = _to_dict_ReplanStatus(data)

            notify("0305", json.dumps(body, ensure_ascii=False).encode("utf-8","ignore"))
