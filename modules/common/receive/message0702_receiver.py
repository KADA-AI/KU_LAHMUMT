# modules/common/receive/message0702_receiver.py
# auto-generated at 2025-08-24T16:37:13.109869+00:00

from dll_files.nFusionImports import *            # IFusionReceive, IsLocal, IsSingletone
from nFusion.Model.msg_0702 import *            # C# 모델
...
def _to_dict_PilotDecision(obj):
    d = {}
    ...
    _v = _get(obj, 'ignore', 'Ignore')
    if _v is not None: d['ignore'] = int(_v)
...
class PilotDecisionReceiver_0702(IFusionReceive[PilotDecision], IsLocal, IsSingletone):
    ...
    def Receive(self, data: PilotDecision, src):
        try:
            _try_save_received('0702', data)

            body = _try_read_db_body('0702', data)
            if body is None:
                body = _to_dict_PilotDecision(data)

            notify("0702", json.dumps(body, ensure_ascii=False).encode("utf-8","ignore"))
