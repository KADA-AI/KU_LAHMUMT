...
def _to_dict_Attitude(obj):
    d = {}
    _v = _get(obj, 'roll', 'Roll')
    if _v is not None: d['roll'] = float(_v)
    _v = _get(obj, 'pitch', 'Pitch')
    if _v is not None: d['pitch'] = float(_v)
    _v = _get(obj, 'yaw', 'Yaw')
    if _v is not None: d['yaw'] = float(_v)
    return d
...
def _to_dict_AgentState(obj):
    ...
    _sub = _get(obj, 'attitude', 'Attitude')
    if _sub is not None: d['attitude'] = _to_dict_Attitude(_sub)
