...
from nFusion.Model.msg_0303 import *            # C# 모델
...
DB_DIR_RULES        = {'0201': 'InputMissionPlan', '0203': 'FlightReferenceInfo', '0301': 'MissionPlan', '0302': 'IndividualMissionPlan', "0303": "FlightPath", '0304': 'FlightPath'}
...
def _to_dict_FilmingProperty(obj):
    d = {}
...
def _to_dict_Waypoint(obj):
    ...
    _v = _get(obj, 'waypointPassType', 'WaypointPassType')
    if _v is not None: d['waypointPassType'] = int(_v)
    _sub = _get(obj, 'loiterProperty', 'LoiterProperty')
    if _sub is not None: d['loiterProperty'] = _to_dict_LoiterProperty(_sub)
    _sub = _get(obj, 'filmingProperty', 'FilmingProperty')
    if _sub is not None: d['filmingProperty'] = _to_dict_FilmingProperty(_sub)
    return d

def _to_dict_UAVFlightPlanData(obj):
    d = {}
    ...
    _v = _get(obj, 'isFormationFlight', 'IsFormationFlight')
    if _v is not None: d['isFormationFlight'] = bool(_v)
    _sub = _get(obj, 'formationInfo', 'FormationInfo')
    if _sub is not None: d['formationInfo'] = _to_dict_FormationInfo(_sub)
    _coll = _get(obj, 'waypointList', 'WaypointList') or []
    if _coll:
        d['waypointList'] = [_to_dict_Waypoint(it) for it in _coll]
    return d

def _to_dict_UAVFlightPlan(obj):
    d = {}
    _v = _get(obj, 'timestamp', 'Timestamp')
    if _v is not None: d['timestamp'] = int(_v)
    _v = _get(obj, 'pathID', 'PathID')
...
class UAVFlightPlanReceiver_0303(IFusionReceive[UAVFlightPlan], IsLocal, IsSingletone):
    ...
    def Receive(self, data: UAVFlightPlan, src):
        try:
            _try_save_received('0303', data)

            body = _try_read_db_body('0303', data)
            if body is None:
                body = _to_dict_UAVFlightPlan(data)

            notify("0303", json.dumps(body, ensure_ascii=False).encode("utf-8","ignore"))
