# modules/common/receive/message0805_receiver.py
# auto-generated at 2025-08-24T16:37:13.115870+00:00

from dll_files.nFusionImports import *            # IFusionReceive, IsLocal, IsSingletone
from nFusion.Model.msg_0805 import *            # C# 모델
...
def _to_dict_SystemEvent(obj):
    ...
    _v = _get(obj, 'eventType', 'EventType')
    if _v is not None: d['eventType'] = int(_v)
    return d
...
class SystemEventReceiver_0805(IFusionReceive[SystemEvent], IsLocal, IsSingletone):
    ...
    def Receive(self, data: SystemEvent, src):
        try:
            _try_save_received('0805', data)

            body = _try_read_db_body('0805', data)
            if body is None:
                body = _to_dict_SystemEvent(data)

            notify("0805", json.dumps(body, ensure_ascii=False).encode("utf-8","ignore"))
