# modules/common/receive/message0904_receiver.py
# auto-generated at 2025-08-24T16:37:13.123886+00:00

from dll_files.nFusionImports import *            # IFusionReceive, IsLocal, IsSingletone
from nFusion.Model.msg_0904 import *            # C# 모델
...
def _to_dict_RequestBehaviorTree(obj):
    d = {}
    ...
    _v = _get(obj, 'BehaviorTreeFileID', 'BehaviorTreeFileID')
    if _v is not None: d['BehaviorTreeFileID'] = int(_v)
    return d

class RequestBehaviorTreeReceiver_0904(IFusionReceive[RequestBehaviorTree], IsLocal, IsSingletone):
    """0904 RequestBehaviorTree 메시지 수신 리시버"""
    __namespace__ = "RequestBehaviorTreeReceiver_0904"

    def Receive(self, data: RequestBehaviorTree, src):
        try:
            _try_save_received('0904', data)

            body = _try_read_db_body('0904', data)
            if body is None:
                body = _to_dict_RequestBehaviorTree(data)

            notify("0904", json.dumps(body, ensure_ascii=False).encode("utf-8","ignore"))

        except Exception:
            print("[ERROR][Receive-0904] traceback ↓↓↓")
