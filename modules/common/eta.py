...
from modules.common.turn_dynamics import turn_period_s
...
def _polyline_len_3d_m(points: Iterable[Coord]) -> float:
    pts = list(points)
    if len(pts) < 2:
        return 0.0
    total = 0.0
    for idx in range(1, len(pts)):
        prev = pts[idx - 1]
        curr = pts[idx]
        horizontal_m = _dist_m(prev, curr)
        dz_m = float(curr.get("altitude", 0.0) or 0.0) - float(prev.get("altitude", 0.0) or 0.0)
        total += math.hypot(horizontal_m, dz_m)
    return total
...
def _line_search_duration_s(wp: Waypoint, *, include_altitude: bool = False) -> float:
    """Return the execution time of the waypoint's camera sweep.

    ``lineSearch.searchSpeed`` is the speed of the camera target over the
    supplied sweep polyline, not the aircraft speed. The SIM starts directly
    on that polyline, so aircraft-to-sweep connector distances are excluded.
    """
    ...
    try:
        search_speed_mps = float(line_search.get("searchSpeed", 0.0) or 0.0)
    except (TypeError, ValueError):
        return 0.0
    if search_speed_mps <= 0.0:
        return 0.0

    length_m = _polyline_len_3d_m(points) if include_altitude else _polyline_len_m(points)
    return length_m / search_speed_mps
...
def _time_from_prev_to_curr_s(prev: Waypoint,
                              ...
                              *,
                              line_search_timing: str = "previous") -> float:
    """
    prev → curr 구간 소요시간(초).
    - 기본: 수평거리 / 속도
    ...
    if isinstance(loiter, dict):
        ...
        if isinstance(t, (int, float)) and t > 0:
            ...
            if R > 0.0 and v > 0.0:
                ...
                extra_hold_s += float(turn_period_s(R, v) or 0.0)

    # 2) 이동 경로 시간
    timing_mode = str(line_search_timing or "previous").strip().lower()
    if timing_mode == "incoming":
        # 0303 attaches filming to the destination/current waypoint. The SIM
        # flies toward that waypoint while its camera traverses lineSearch, so
        # route and sweep time run concurrently. The current target also owns
        # the commanded aircraft speed in the SIM controller.
        route_speed_mps = (
            _to_mps(curr.get("speed"))
            or _to_mps(prev.get("speed"))
            or default_speed_mps
        )
        route_dist_m = _dist_m(start, goal)
        route_s = (
            route_dist_m / route_speed_mps
            if route_speed_mps and route_speed_mps > 0.0
            else 0.0
        )
        sweep_s = _line_search_duration_s(curr, include_altitude=True)
        return extra_hold_s + max(route_s, sweep_s)
...
def annotate_eta_flight_plan(fp: FlightPlan,
                             *,
                             ...
                             line_search_timing: str = "previous") -> None:
    """
    ...
    timing_mode = str(line_search_timing or "previous").strip().lower()
    initial_sweep_s = (
        _line_search_duration_s(ordered[0], include_altitude=True)
        if timing_mode == "incoming"
        else 0.0
    )

    for i in range(1, len(ordered)):
        dt = _time_from_prev_to_curr_s(
            ordered[i-1],
            ordered[i],
            default_speed_mps=default_speed_mps,
            line_search_timing=timing_mode,
        )
        # The first waypoint has no incoming leg in the payload. If it owns a
        # sweep (common for a compact replan), execute it once before leg 0->1.
        if i == 1 and initial_sweep_s > 0.0:
            dt += initial_sweep_s
        acc_s += dt
        eta_s = int(round(acc_s))
        if i == len(ordered) - 1 and acc_s > 0.0 and eta_s <= 0:
            eta_s = 1
        ordered[i]["eta"] = max(0, eta_s)

def annotate_eta_done_flight_plan(fp: FlightPlan,
                                  *,
                                  default_speed_mps: float = 40.0,
                                  waypoint_list_keys: Tuple[str, ...] = ("waypointList", "waypoints"),
                                  min_terminal_eta_s: int = 1,
                                  line_search_timing: str = "previous") -> None:
    """
    ...
    annotate_eta_flight_plan(
        ...
        line_search_timing=line_search_timing,
    )
    ...
    for wp in ordered:
        try:
            ...
            eta_val = 0.0
        wp["eta"] = int(round(min(max(eta_val, 0.0), final_eta)))
    # ICD ecf is per-leg fuel in litres, not progress along the route.
    apply_leg_fuel_inplace(ordered)

def annotate_eta_list(
    fps: Iterable[FlightPlan],
    *,
    default_speed_mps: float = 40.0,
    line_search_timing: str = "previous",
) -> None:
    for fp in fps or []:
        try:
            annotate_eta_flight_plan(
