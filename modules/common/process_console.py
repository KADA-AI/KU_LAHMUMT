...
PROCESS_LOG_WORKER_BATCH_SIZE = 512
_DIAGNOSTIC_WAKE = object()
...
def process_logging_budget() -> dict[str, Any]:
    return {
        ...
        "runtimeDiagnosticsOnAsyncWriter": True,
    }


def _create_runtime_diagnostics(module_name: str):
    """Best-effort import boundary: diagnostics must never block process logs."""
    try:
        from modules.common.runtime_diagnostics import create_runtime_diagnostics

        return create_runtime_diagnostics(module_name)
    except Exception:
        return None


def _diagnostic_activity_transition(
    lifecycle: str,
    component: str | None,
    outcome: str | None,
) -> bool | None:
    """Map planning lifecycle boundaries to the recorder's fast-sample window."""
    text = " ".join((str(lifecycle or ""), str(component or ""), str(outcome or ""))).lower()
    planning_related = any(
        token in text
        for token in ("replan", "mission_plan", "mission planning", "pipeline", "algorithm")
    )
    if not planning_related:
        return None
    if any(token in text for token in ("exception", "failure", "failed", "crash", "stop", "finish", "complete")):
        return False
    if any(token in text for token in ("start", "begin", "running")):
        return True
    return None
...
class _AsyncProcessFileSink:
    def __init__(self, module_name: str) -> None:
        ...
        self._last_batch_write_ms: float | None = None
        self._last_batch_items = 0
        self._last_batch_chars = 0
        self._last_batch_write_mono: float | None = None
        self._runtime_diagnostics = _create_runtime_diagnostics(self.module_name)
    ...
    def close(self) -> None:
        with self._stats_lock:
            if self._close_requested or self._close_in_progress:
                return
            self._close_in_progress = True
        try:
            self.request_diagnostic_snapshot(
                "process_stop",
                context={"lifecycle": "process_stop", "outcome": "graceful_shutdown"},
                active=False,
            )
            self.emit(
        ...
        if threading.current_thread() is not self._thread:
            try:
                timeout_sec = max(
                    0.0,
                    min(10.0, float(os.getenv("KU_PROCESS_LOG_CLOSE_TIMEOUT_SEC", "2.0") or "2.0")),
                )
            except Exception:
                timeout_sec = 2.0
            if timeout_sec > 0.0:
    ...
    def stats(self) -> dict[str, Any]:
        with self._stats_lock:
            result = {
                ...
                "droppedTotal": int(self._dropped_total),
                "flushFailures": int(self._flush_failures),
                "queueMaxSize": int(getattr(self, "_queue_maxsize", 0) or 0),
                "lastBatchItems": int(self._last_batch_items),
                "lastBatchChars": int(self._last_batch_chars),
                "lastBatchWriteMs": self._last_batch_write_ms,
            }
        diagnostics = getattr(self, "_runtime_diagnostics", None)
        if diagnostics is not None:
            try:
                result["runtimeDiagnostics"] = diagnostics.stats()
            except Exception:
                pass
        return result
    ...
    def _base_stats(self) -> dict[str, Any]:
        with self._stats_lock:
            ...
            last_write_mono = self._last_batch_write_mono
        result["lastBatchAgeMs"] = (
            None
            if last_write_mono is None
            else round(max(0.0, time.monotonic() - last_write_mono) * 1000.0, 3)
        )
        return result

    def request_diagnostic_snapshot(
        self,
        trigger: str,
        *,
        ...
        if diagnostics is None:
            return
        try:
            diagnostics.request_snapshot(trigger, context=context, active=active)
        except Exception:
            return
        try:
            if self._queue.empty() and not self._close_requested and not self._close_in_progress:
                self._queue.put_nowait(_DIAGNOSTIC_WAKE)
        except Exception:
            pass

    def _poll_runtime_diagnostics(self, *, force: bool = False) -> None:
        diagnostics = getattr(self, "_runtime_diagnostics", None)
        if diagnostics is None:
            return
        try:
            log_stats = self._base_stats()
            diagnostics.poll(
                process_log_stats=log_stats,
                module_log_write_ms=log_stats.get("lastBatchWriteMs"),
                force=force,
            )
        except Exception:
            pass
        ...
        while True:
            try:
                ...
                if self._close_requested:
                    self._poll_runtime_diagnostics(force=True)
                    break
                self._poll_runtime_diagnostics()
                continue
            if item is None:
                self._poll_runtime_diagnostics(force=True)
                break
            if item is _DIAGNOSTIC_WAKE:
                self._poll_runtime_diagnostics()
                continue
            batch = [item]
            while True:
                if len(batch) >= PROCESS_LOG_WORKER_BATCH_SIZE:
                    break
                try:
            ...
            try:
                ...
                if self._handle is not None:
                    ...
                    self._handle.flush()
                    with self._stats_lock:
                        self._last_batch_write_ms = round(
                            (time.perf_counter() - write_started) * 1000.0,
                            3,
                        )
                        self._last_batch_items = len(batch)
                        self._last_batch_chars = sum(len(text) for text in batch)
                        self._last_batch_write_mono = time.monotonic()
            except Exception:
                self._record_flush_failure()
            self._poll_runtime_diagnostics(force=item is None)
            if item is None:
                break
        self._close_handle()
        diagnostics = getattr(self, "_runtime_diagnostics", None)
...
def install_process_file_logging(module_name: str) -> _AsyncProcessFileSink:
    ...
    sink.request_diagnostic_snapshot(
        "process_start",
        context={"lifecycle": "process_start", "component": "process", "outcome": "ok"},
    )
    sink.emit(
        _format_process_event(
            key,
...
def emit_process_lifecycle_event(
    ...
    *,
    ...
    if extra:
        diagnostic_context["extra"] = extra
    sink.request_diagnostic_snapshot(
        "process_lifecycle",
        context=diagnostic_context,
        active=_diagnostic_activity_transition(lifecycle, component, outcome),
    )


def request_runtime_diagnostic_snapshot(
    module_name: str,
    trigger: str,
    *,
    context: dict[str, Any] | None = None,
    ...
    """Public non-blocking boundary marker for planning/logic code."""
    sink = _ACTIVE_SINKS.get(_slugify_module_name(module_name))
    if sink is None:
        sink = install_process_file_logging(module_name)
    sink.request_diagnostic_snapshot(trigger, context=context, active=active)


def close_process_file_logging(module_name: str) -> None:
    """Bounded drain for entrypoints that terminate with ``os._exit``."""
    sink = _ACTIVE_SINKS.get(_slugify_module_name(module_name))
    if sink is not None:
        sink.close()
