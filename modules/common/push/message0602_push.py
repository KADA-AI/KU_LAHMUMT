# modules/common/push/message0602_push.py
# auto-generated at 2025-08-24T20:13:14.029449+00:00


import json, importlib
from datetime import datetime, timezone
from System.Collections.Generic import List
from nFusion.Model.msg_0602 import *    # C# 모델(우선)
from nFusion.Model.CommonType import *     # 공통 타입(항상)
from System import Int32, Single, String, UInt32, UInt64
from generator.message0602_generator import make_msg0602_body
...
def _dict_to_PathFollowing(data: dict):
    obj = _new('PathFollowing')
    if "startWaypointID" in data: _try_set(obj, "startWaypointID", int(data["startWaypointID"]))
    return obj

def _dict_to_TargetTracking(data: dict):
    obj = _new('TargetTracking')
...
def _dict_to_LoiterProperty(data: dict):
    ...
    if "loiterTime" in data: _try_set(obj, "loiterTime", float(data["loiterTime"]))
    if "loiterRadius" in data: _try_set(obj, "loiterRadius", float(data["loiterRadius"]))
    if "loiterDirection" in data: _try_set(obj, "loiterDirection", int(data["loiterDirection"]))
    if "loiterSpeed" in data: _try_set(obj, "loiterSpeed", float(data["loiterSpeed"]))
    return obj
...
def _dict_to_FlightModeCommand(data: dict):
    obj = _new('FlightModeCommand')
    if "flightMode" in data: _try_set(obj, "flightMode", int(data["flightMode"]))
    if "pathFollowing" in data and isinstance(data["pathFollowing"], dict):
        _try_set(obj, "pathFollowing", _dict_to_PathFollowing(data["pathFollowing"]))
    if "targetTracking" in data and isinstance(data["targetTracking"], dict):
        _try_set(obj, "targetTracking", _dict_to_TargetTracking(data["targetTracking"]))
...
def _dict_to_FilmingModeCommand(data: dict):
    obj = _new('FilmingModeCommand')
...
def _dict_to_UAVCommand(data: dict):
    obj = _new('UAVCommand')
    ...
    if "uavCommandModeType" in data: _try_set(obj, "uavCommandModeType", int(data["uavCommandModeType"]))
    if "aircraftID" in data: _try_set(obj, "aircraftID", int(data["aircraftID"]))
    if "flightModeCommand" in data and isinstance(data["flightModeCommand"], dict):
        _try_set(obj, "flightModeCommand", _dict_to_FlightModeCommand(data["flightModeCommand"]))
    if "filmingModeCommand" in data and isinstance(data["filmingModeCommand"], dict):
        _try_set(obj, "filmingModeCommand", _dict_to_FilmingModeCommand(data["filmingModeCommand"]))
    return obj
...
def _dict_to_obj(body_dict: dict):
    return _dict_to_UAVCommand(body_dict)
