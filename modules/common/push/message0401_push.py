...
def _dict_to_Attitude(data: dict):
    obj = _new('Attitude')
    if not isinstance(data, dict):
        return obj
    roll = _first_key_ci(data, "roll")
    if roll is not None: _try_set(obj, "roll", float(roll))
    pitch = _first_key_ci(data, "pitch")
    if pitch is not None: _try_set(obj, "pitch", float(pitch))
    yaw = _first_key_ci(data, "yaw")
    if yaw is not None: _try_set(obj, "yaw", float(yaw))
    return obj
...
def _dict_to_UnmannedInfo(data: dict):
    ...
    flying = _first_key_ci(data, "flying", "Flying")
    if flying is not None:
        data["flying"] = int(flying)
        _try_set(obj, "flying", int(flying))
...
def _dict_to_AgentState(data: dict):
    ...
    attitude = _first_key_ci(data, "attitude")
    if isinstance(attitude, dict):
        _try_set(obj, "attitude", _dict_to_Attitude(attitude))
    fuel = data.get("fuel")
