...
from nFusion.Model.msg_0806 import *    # C# 모델(우선)
...
def _dict_to_BootCommand(data: dict):
    obj = _new('BootCommand')
    ...
    if "command" in data: _try_set(obj, "command", int(data["command"]))
    return obj
...
def _dict_to_obj(body_dict: dict):
    return _dict_to_BootCommand(body_dict)
