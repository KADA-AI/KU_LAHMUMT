...
from modules.common.version_info import write_scenario_version_log
...
ENV_ALLOW_SCENARIO_ROLLBACK = "KU_ALLOW_SCENARIO_ROLLBACK"
...
def _refresh_cache_unlocked() -> None:
    ...
    try:
        ...
        _cache["mtime"] = mtime
        return
    if _is_stale_scenario_rollback(data):
        # A launcher/settings writer can briefly publish an older scenario while
        # the current simulation is still running.  Accepting that transient
        # value makes monitoring/planning read old snapshots and then jump back,
        # which appears as disappearing/reappearing mission areas.  Keep the
        # active scenario; a later file update will be evaluated normally.
        _cache["mtime"] = mtime
        return
...
def _is_stale_scenario_rollback(data: Any) -> bool:
    if not isinstance(data, dict):
        return False
    allow = str(os.environ.get(ENV_ALLOW_SCENARIO_ROLLBACK, "") or "").strip().lower()
    if allow in {"1", "true", "yes", "y", "on"}:
        return False
    if str(data.get("source") or "") != "scenario":
        return False
    if str(_cache.get("source") or "") != "scenario":
        return False
    try:
        incoming_timestamp = int(data.get("timestamp_ms"))
        active_timestamp = int(_cache.get("timestamp_ms"))
    except (TypeError, ValueError):
        return False
    if incoming_timestamp >= active_timestamp:
        return False
    active_root = _cache.get("db_root")
    if active_root is None:
        return False
    try:
        return Path(active_root).exists()
    except Exception:
        return False
...
def peek_active_db_root(*, existing_only: bool = False, allow_legacy: bool = False) -> Optional[Path]:
    ...
    if info and info.get("db_root") and not _is_stale_scenario_rollback(info):
        try:
...
def _record_scenario_version_safe(
    scenario_dir: Path,
    *,
    timestamp_ms: int,
    scenario_iso: str,
    ...
    """Record run identity without allowing optional logging to block startup."""

    try:
        path = write_scenario_version_log(
            scenario_dir,
            timestamp_ms=int(timestamp_ms),
            scenario_iso=str(scenario_iso),
            agency=str(agency),
        )
...
def activate_scenario(timestamp_ms: int, agency: Optional[str] = None, *, copy_legacy: bool = True) -> Dict[str, Any]:
    ...
    with _lock:
        ...
        version_log_path, version_log_error = _record_scenario_version_safe(
            scenario_dir,
            timestamp_ms=int(timestamp_ms),
            scenario_iso=iso,
            agency=agency_code,
        ...
        if version_log_path is not None:
            info["version_log"] = str(version_log_path)
        if version_log_error:
            info["version_log_error"] = version_log_error
        _write_info_unlocked(info)
        _cache.update({
            "mtime": _info_mtime(),
            "db_root": db_dir,
