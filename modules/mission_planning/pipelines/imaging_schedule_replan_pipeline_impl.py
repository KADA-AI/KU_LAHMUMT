...
from modules.mission_planning.pipelines.mission_path_trim import relink_waypoints
...
def _build_replanned_waypoints(
    ...
    *,
    ...
    relink_waypoints(new_waypoints)
    return new_waypoints, int(current_waypoint_id), int(candidate_waypoint_id)
...
def run_imaging_schedule_replan_pipeline(
    ...
    *,
    ...
    if current_waypoint_id is None or current_waypoint_id <= 0:
        emit("[IMGSCH] currentWaypointID is 0/invalid -> not a replan target.")
        return None
    if source_plan_id is None or source_plan_id <= 0 or aircraft_id is None or aircraft_id <= 0:
        emit(
            "[IMGSCH] required source identifiers missing "
    ...
    if artifacts is None:
        emit("[IMGSCH] failed to resolve source mission artifacts.")
        return None
    ...
    try:
        ...
        emit(f"[IMGSCH] failed to load source artifacts: {exc}")
        return None
    ...
    if not plan_aircraft_updated:
        emit(f"[IMGSCH] aircraft {aircraft_id} not found in source MissionPlan {source_plan_id}.")
        return None
    ...
    if target_index is None:
        emit(
            f"[IMGSCH] individualMissionID {artifacts.individual_mission_id} "
    ...
    if not source_waypoints:
        emit(f"[IMGSCH] FlightPath {artifacts.path_id} has no waypointList.")
        return None
    ...
    emit(
        "[IMGSCH] stored imaging-schedule artifacts -> "
    ...
    log_path = log_dir / f"ImagingSchedule_{aircraft_id}_{now_ms}.json"
    log_payload = {
    ...
    emit(f"[IMGSCH] log captured -> {log_path}")
    try:
        imaging_schedule_replan_store.save_event(
    ...
    plan_meta_entry.update(
        {
            "triggerType": "imagingScheduleDeviation",
            "sourceMissionPlanID": int(source_plan_id),
            "aircraftID": int(aircraft_id),
