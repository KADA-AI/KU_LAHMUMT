...
def _intersect_distance_interval_sets(
    interval_sets: Iterable[Iterable[tuple[float, float]]],
    *,
    length_m: float,
) -> List[tuple[float, float]]:
    sets = [
        _merge_distance_intervals(intervals, length_m=float(length_m))
    ...
    for rows in sets[1:]:
        ...
        left_idx = 0
        right_idx = 0
        while left_idx < len(common) and right_idx < len(rows):
            left_start, left_end = common[left_idx]
            right_start, right_end = rows[right_idx]
            start = max(float(left_start), float(right_start))
            end = min(float(left_end), float(right_end))
            if end > start + 1e-6:
                intersection.append((start, end))
            if float(left_end) <= float(right_end):
                left_idx += 1
            else:
                right_idx += 1
        common = intersection
        if not common:
            break
    return _merge_distance_intervals(common, length_m=float(length_m))
...
def _collaborative_line_fragment_floor_m(
    *,
    source_length_m: float,
    source_width_m: float,
    configured_min_fragment_m: float,
) -> float:
    """Ignore tiny LINE scraps that cannot be assigned as useful work."""
    # A few metres of positional/footprint disagreement between parallel UAV
    # lanes must not become a new collaborative area.  Scale the tolerance to
    # the original LINE while keeping it small enough to preserve real work.
    relative_floor_m = max(
        max(0.0, float(source_length_m)) * 0.02,
        max(0.0, float(source_width_m)) * 0.10,
    )
    return max(
        max(0.0, float(configured_min_fragment_m)),
        min(150.0, float(relative_floor_m)),
    )
...
def build_line_scan_remaining_detail(
    ...
    *,
    ...
    common_aircraft_coverage: bool = False,
) -> Dict[str, Any]:
    ...
    contributing_timestamps_ms: set[int] = set()
    ...
    for _path_id, progress_entry in sorted(progress_map.items(), key=lambda item: int(item[0])):
        ...
        timestamp_ms = _to_int(
            line_entry.get("timestampMs")
            or line_entry.get("timestamp_ms")
            or line_entry.get("timestamp")
        )
        ...
        if timestamp_ms is not None and timestamp_ms > 0:
            contributing_timestamps_ms.add(int(timestamp_ms))
    ...
    row_width_candidates = [
        float(width)
        for entry in matching_entries
        for row in (entry.get("lineList") or [])
        if isinstance(row, dict)
        and (width := _to_float(row.get("width"))) is not None
        and float(width) > 0.0
    ]
    active_row_width_m = max(row_width_candidates) if row_width_candidates else float(source_width_m)
    # Reassigned UAV lanes are intentionally offset from the input centerline.
    # The old 80 m ceiling rejected those valid lanes and silently expanded the
    # domain back to the original full line during post-attack rejoin.
    max_projection_distance_m = max(
        80.0,
        min(
            (float(source_width_m) * 0.5) + (float(active_row_width_m) * 0.5) + 50.0,
            5000.0,
        ),
    )
    ...
    domains_by_aircraft: Dict[int, List[tuple[float, float]]] = {}
    covered_by_aircraft: Dict[int, List[tuple[float, float]]] = {}
    fallback_ratios_by_aircraft: Dict[int, List[float]] = {}

    for line_entry in matching_entries:
        entry_aircraft_id = _to_int(
            line_entry.get("aircraftID") or line_entry.get("aircraft_id")
        )
        ...
        for line_row in line_rows:
            ...
            if row_span is not None and float(row_span[2]) <= float(max_projection_distance_m):
                ...
                domain_intervals.append(domain_interval)
                if entry_aircraft_id is not None:
                    domains_by_aircraft.setdefault(int(entry_aircraft_id), []).append(domain_interval)
            planned_m = _to_float(line_row.get("plannedLengthM"))
            if planned_m is None or planned_m <= 1e-6:
            ...
            if precise_rows:
                for interval in precise_rows:
                    ...
                    if mapped is None:
                        continue
                    covered_interval = (float(mapped[0]), float(mapped[1]))
                    covered_intervals.append(covered_interval)
                    if entry_aircraft_id is not None:
                        covered_by_aircraft.setdefault(int(entry_aircraft_id), []).append(
                            covered_interval
                        )
                    precise_added = True
        if precise_added:
        ...
        if ratio is not None:
            ...
            if entry_aircraft_id is not None:
                fallback_ratios_by_aircraft.setdefault(int(entry_aircraft_id), []).append(
                    float(ratio)
                )

    # Every matching row describes the exact along-track domain assigned by
    # the source plan.  Successive replans keep the original centerline in
    # ``sourceCoordinateList`` for lineage, while their executable rows cover
    # only the still-active suffix/fragments.  Expanding an unfiltered call
    # back to the full source line resurrects the already completed prefix.
    active_domain_intervals = (
        domain_intervals
        if domain_intervals
        else [(0.0, float(source_len_m))]
    )
    if common_aircraft_coverage:
        for aircraft_id, ratios in fallback_ratios_by_aircraft.items():
            aircraft_domain = domains_by_aircraft.get(int(aircraft_id)) or active_domain_intervals
            covered_by_aircraft.setdefault(int(aircraft_id), []).extend(
        ...
        # When the caller names the active aircraft set, every member must
        # participate in the common-coverage decision.  A temporarily missing
        # progress row is not proof that the aircraft completed its share; if
        # it were ignored here, the LINE could be marked complete and then
        # appear to come back when that row arrives on the next sample.
        common_aircraft_ids = sorted(
            int(aid)
            for aid in (
                wanted_aircraft
                if wanted_aircraft
                else contributing_aircraft
            )
        )
        if common_aircraft_ids:
            # A projected lane may start/end slightly inside another lane's
            # domain (especially on curved or laterally offset rows).  An
            # aircraft must veto only uncovered work inside its own domain;
            # treating out-of-domain chainage as uncovered creates false
            # fragments at either end even when every lane is complete.
            missing_by_any_aircraft: List[tuple[float, float]] = []
            for aircraft_id in common_aircraft_ids:
                aircraft_domain = (
                    domains_by_aircraft.get(int(aircraft_id))
                    or active_domain_intervals
                )
                missing_by_any_aircraft.extend(
                    _remaining_distance_intervals_in_domains(
                        length_m=float(source_len_m),
                        domain_intervals=aircraft_domain,
                        covered_intervals=covered_by_aircraft.get(int(aircraft_id), []),
                    )
                )
            missing_by_any_aircraft = _merge_distance_intervals(
                missing_by_any_aircraft,
                length_m=float(source_len_m),
            )
            covered_intervals = _remaining_distance_intervals_in_domains(
                length_m=float(source_len_m),
                domain_intervals=active_domain_intervals,
                covered_intervals=missing_by_any_aircraft,
            )
    ...
    effective_min_fragment_m = max(0.0, float(min_fragment_m))
    if common_aircraft_coverage:
        effective_min_fragment_m = _collaborative_line_fragment_floor_m(
            source_length_m=float(source_len_m),
            source_width_m=float(source_width_m),
            configured_min_fragment_m=float(min_fragment_m),
        )
    line_rows: List[Dict[str, Any]] = []
    total_remaining_m = 0.0
    discarded_fragment_count = 0
    ...
    for interval_idx, (start_m, end_m) in enumerate(remaining_intervals):
        ...
        if interval_len < float(effective_min_fragment_m):
            discarded_fragment_count += 1
            discarded_fragment_length_m += float(interval_len)
            continue
        ...
        row = {
            ...
            "width": max(0, min(50000, int(round(float(source_width_m))))),
            "lineIndex": 0,
    ...
    remaining_policy = (
        "centerline_interval_union_short_fragment_suppressed"
        if discarded_fragment_count
        else "centerline_interval_union"
    )
    if not line_rows:
        # Empty geometry with matching progress rows means the LINE is
        # complete.  Keep this distinct from "no matching progress" so a
        # caller cannot fall back to an older remaining-area snapshot and
        # resurrect already completed work.
        return {
            "coordinateList": [],
            "lineList": [],
            "areaList": [],
            ...
            "lineRemainingPolicy": remaining_policy,
            "lineRemainingFragmentCount": 0,
            ...
            "lineRemainingLengthM": 0.0,
            "lineRemainingMinimumFragmentM": round(float(effective_min_fragment_m), 3),
            "lineRemainingDiscardedFragmentCount": int(discarded_fragment_count),
            "lineRemainingDiscardedFragmentLengthM": round(
                float(discarded_fragment_length_m), 3
            ),
            ...
            "lineScanLatestTimestampMs": (
                max(contributing_timestamps_ms) if contributing_timestamps_ms else None
            ),
        }
    ...
    return {
        ...
        "lineRemainingSource": "line_scan_progress_centerline",
        "lineRemainingPolicy": remaining_policy,
        "lineCoverageAggregationPolicy": (
            "active_aircraft_intersection"
            if common_aircraft_coverage
            else "aircraft_union"
        ),
...
def load_line_scan_remaining_detail(
    *,
    ...
    return build_line_scan_remaining_detail(
        ...
        common_aircraft_coverage=common_aircraft_coverage,
    )
...
def apply_line_scan_remaining_to_input_mission(
    ...
    *,
    ...
    if bool(line_detail.get("lineRemainingCompleted")):
        mission["isDone"] = True
        mission["lineTakeoverSource"] = "line_scan_progress_completed"
