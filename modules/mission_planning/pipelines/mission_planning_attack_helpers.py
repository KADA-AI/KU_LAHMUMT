...
MIN_ATTACK_STANDOFF_M = 3000.0
PREFERRED_ATTACK_STANDOFF_M = 9000.0
...
_ATTACK_LOS_MAX_RADIUS_M = PREFERRED_ATTACK_STANDOFF_M
...
def _batch_dem_los_attack_coordinate(
    attack_coord: Dict[str, Any],
    friendly_coord: Dict[str, Any],
    target_coord: Dict[str, Any],
) -> Dict[str, Any]:
    """Apply LOS altitude using the shared DEM cache when GDAL LOS is unavailable."""
    configured_step_m = max(0.0, get_runtime_attack_float("attack_los_profile_step_m", 0.0))
    try:
        reset_terrain_elev_many_metrics()
        profile = profile_with_batch_dem(
            ...
            target_height_m=max(
                0.0,
                get_runtime_attack_float(
                    "attack_los_target_height_m", ENEMY_OBSERVER_HEIGHT_M
                ),
            ),
            clearance_m=max(
                0.0,
                get_runtime_attack_float("attack_los_clearance_m", LOS_CLEARANCE_M),
            ),
            sample_step_m=float(configured_step_m or 10.0),
            max_samples=1024,
        )
        metrics = get_terrain_elev_many_metrics(reset=True)
    except Exception:
        return dict(attack_coord)
    sample_count = _to_int(profile.get("sample_count")) or 0
    resolved_count = _to_int(metrics.get("demResolvedByTile")) or 0
    required_altitude_m = _to_float(profile.get("required_altitude_m"))
    attack_ground_m = _to_float(profile.get("attack_ground_m"))
    if (
        not bool(profile.get("verified"))
        or sample_count <= 0
        or resolved_count < sample_count
        or required_altitude_m is None
        or attack_ground_m is None
    ):
        return dict(attack_coord)
    altitude_offset_m = get_runtime_attack_float("attack_point_altitude_offset_m", 300.0)
    altitude_candidates = [
        float(attack_ground_m) + float(altitude_offset_m),
        float(required_altitude_m),
    ]
    friendly_altitude_m = _to_float(friendly_coord.get("altitude"))
    if friendly_altitude_m is not None and math.isfinite(friendly_altitude_m):
        altitude_candidates.append(float(friendly_altitude_m))
    result = dict(attack_coord)
    result.update(
        {
            "altitude": int(math.ceil(max(altitude_candidates))),
            "selection_mode": "los_area_direct_profile",
            "los_area": True,
            "terrain_altitude_m": float(attack_ground_m),
            "altitude_offset_m": float(altitude_offset_m),
            "los_verified": True,
            "los_required_altitude_m": float(required_altitude_m),
            "los_selected_altitude_m": int(math.ceil(max(altitude_candidates))),
            "los_altitude_adjusted": bool(
                float(required_altitude_m)
                > float(attack_ground_m) + float(altitude_offset_m) + 1e-6
            ),
            "los_distance_m": _to_float(profile.get("distance_m")),
            "los_profile_sample_count": int(sample_count),
            "los_dem_resolved_sample_count": int(resolved_count),
            "los_profile_step_m": _to_float(profile.get("sample_step_m")),
            "los_clearance_m": _to_float(profile.get("clearance_m")),
            "los_target_height_m": _to_float(profile.get("target_height_m")),
        }
    )
    return result
...
def _lah_leg_time_seconds(
    ...
    *,
    ...
    horizontal_s = max(0.0, float(horizontal_m)) / max(1e-6, float(speed_mps))
    altitude_delta_m = float(right_coord.get("altitude", 0.0) or 0.0) - float(
        left_coord.get("altitude", 0.0) or 0.0
    )
    vertical_s = abs(altitude_delta_m) / _lah_planned_vertical_rate_mps(altitude_delta_m)
    return max(0, int(math.ceil(max(horizontal_s, vertical_s) - 1e-9)))
...
def get_attack_standoff_distances() -> Tuple[float, float]:
    ...
    preferred_distance = get_runtime_attack_float(
        "attack_preferred_standoff_m",
        PREFERRED_ATTACK_STANDOFF_M,
    )
    try:
    ...
    try:
        ...
        preferred_distance = PREFERRED_ATTACK_STANDOFF_M
    ...
    if friendly is not None and current_distance is not None:
        if float(current_distance) < min_distance:
            return {
                ...
                "mode": "current_position_inside_min_standoff_no_retreat",
...
def compute_attack_waypoint(
    ...
    if (
        ...
        los_target_height_m = max(
            0.0,
            get_runtime_attack_float(
                "attack_los_target_height_m", ENEMY_OBSERVER_HEIGHT_M
            ),
        )
        los_profile_step_m = max(0.0, get_runtime_attack_float("attack_los_profile_step_m", 0.0))
        cmd = [
        ...
        try:
            ...
            if result.returncode == 0:
                ...
                altitude_offset_m = get_runtime_attack_float("attack_point_altitude_offset_m", 300.0)
                baseline_altitude_m = float(alt_val) + float(altitude_offset_m)
                los_profile = data.get("los_profile") or {}
                los_required_altitude_m = None
                if bool(los_profile.get("verified")):
                    try:
                        parsed_required = float(los_profile.get("required_altitude_m"))
                        if math.isfinite(parsed_required):
                            los_required_altitude_m = parsed_required
                    except (TypeError, ValueError):
                        los_required_altitude_m = None
                friendly_altitude_m = _to_float(friendly.get("altitude"))
                altitude_candidates = [float(baseline_altitude_m)]
                if los_required_altitude_m is not None:
                    altitude_candidates.append(float(los_required_altitude_m))
                if friendly_altitude_m is not None and math.isfinite(friendly_altitude_m):
                    altitude_candidates.append(float(friendly_altitude_m))
                selected_altitude_m = int(math.ceil(max(altitude_candidates)))
                selected = {
                    "latitude": float(lat_val),
                    "longitude": float(lon_val),
                    "altitude": int(selected_altitude_m),
                    "selection_mode": "los_area",
                    "los_area": True,
                    "terrain_altitude_m": float(alt_val),
                    "altitude_offset_m": float(altitude_offset_m),
                    "los_verified": bool(los_required_altitude_m is not None),
                    "los_required_altitude_m": los_required_altitude_m,
                    "los_selected_altitude_m": int(selected_altitude_m),
                    "los_altitude_adjusted": bool(
                        los_required_altitude_m is not None
                        and float(los_required_altitude_m) > float(baseline_altitude_m) + 1e-6
                    ),
                    "los_distance_m": _to_float(los_profile.get("distance_m")),
                    "los_profile_sample_count": _to_int(los_profile.get("sample_count")),
                    "los_profile_step_m": _to_float(los_profile.get("sample_step_m")),
                    "los_clearance_m": _to_float(los_profile.get("clearance_m")),
                    "los_target_height_m": _to_float(los_profile.get("target_height_m")),
                ...
                if candidate_distance_m is None or float(candidate_distance_m) < float(min_standoff_m):
                    ...
                    return _batch_dem_los_attack_coordinate(fallback, friendly, target)
                _safe_emit(
                    log_cb,
                ...
                if not bool(selected.get("los_verified")):
                    selected = _batch_dem_los_attack_coordinate(selected, friendly, target)
                return selected
            else:
...
def apply_attack_customizations(
    ...
    *,
    ...
    existing_flight_plan = None
    for fp in flight_plans_0304 or []:
        try:
            fp_path_id = int(fp.get("pathID"))
            fp_aircraft_id = int(fp.get("aircraftID", 0))
        except Exception:
            continue
        if fp_path_id == attack_path_id and fp_aircraft_id == attack_aircraft_id:
            existing_flight_plan = fp
            break

    mission_info = primary_mission.get("individualMissionInfo") or {}
    coord_list = mission_info.get("coordinateList") or []
    friendly_coord = None
    # d0304 has already inserted the reported/live LAH position at the head of
    # the generated flight plan.  This legacy attack customization replaces the
    # whole waypoint list, so retain that position instead of falling back to
    # the (potentially stale) mission geometry anchor.
    if isinstance(existing_flight_plan, dict):
        existing_waypoints = existing_flight_plan.get("lahWaypointList") or []
        if existing_waypoints and isinstance(existing_waypoints[0], dict):
            friendly_coord = _normalize_coord(existing_waypoints[0].get("coordinate"))
    if coord_list and isinstance(coord_list[0], dict):
        friendly_coord = friendly_coord or _normalize_coord(coord_list[0])
    ...
    base_wp_id = 10_000 + variant_no * 1_000
    ...
    if low_profile:
        try:
            ...
            reported_start_altitude_m = float("nan")
        terrain_start_altitude_m = float(low_profile[0].get("altitude", 0.0) or 0.0)
        if (
            math.isfinite(reported_start_altitude_m)
            and reported_start_altitude_m > 0.0
            and abs(reported_start_altitude_m - terrain_start_altitude_m) >= 1.0
        ):
            low_profile = [
                {
                    **dict(low_profile[0]),
                    "latitude": float(approach_coord["latitude"]),
                    "longitude": float(approach_coord["longitude"]),
                    "altitude": int(round(reported_start_altitude_m)),
                    "cum_m": 0.0,
                }
            ] + low_profile
    low_waypoints: List[Dict[str, Any]] = []
    total_horizontal_m = max(
        float((low_profile[-1] if low_profile else {}).get("cum_m", 0.0) or 0.0),
        1.0,
    )
    attack_wp_id = base_wp_id + len(low_profile)
    cumulative_eta_s = 0
    for index, sample in enumerate(low_profile):
        waypoint_id = base_wp_id + index
        next_waypoint_id = (
            base_wp_id + index + 1
            if index + 1 < len(low_profile)
            else attack_wp_id
        )
        cumulative_m = float(sample.get("cum_m", 0.0) or 0.0)
        if index > 0:
            previous_sample = low_profile[index - 1]
            previous_cumulative_m = float(previous_sample.get("cum_m", 0.0) or 0.0)
            cumulative_eta_s = min(
                0xFFFFFFFF,
                cumulative_eta_s
                + _lah_leg_time_seconds(
                    previous_sample,
                    sample,
        ...
        low_waypoints.append(
            _build_wp_entry(
                sample,
                waypoint_id=waypoint_id,
                next_waypoint_id=next_waypoint_id,
                eta_s=int(cumulative_eta_s),
                target_id_value=0,
                weapon_type_value=0,
                ecf_value=min(0.99, round(cumulative_m / total_horizontal_m, 2)),
                speed_value=attack_speed_mps,
            )
        )
    attack_horizontal_m = (
        _haversine_distance_m(low_profile[-1], attack_waypoint)
        if low_profile
        else 0.0
    )
    attack_eta_s = int(cumulative_eta_s)
    if low_profile:
        attack_eta_s = min(
            0xFFFFFFFF,
            attack_eta_s
            + _lah_leg_time_seconds(
                low_profile[-1],
                attack_waypoint,
                horizontal_m=float(attack_horizontal_m or 0.0),
                speed_mps=attack_speed_mps,
            ),
        )
    attack_wp = _build_wp_entry(
        attack_waypoint,
        waypoint_id=attack_wp_id,
        next_waypoint_id=0,
        eta_s=int(attack_eta_s),
    ...
    replaced_fp = existing_flight_plan is not None
    if existing_flight_plan is not None:
        existing_flight_plan["lahWaypointList"] = low_waypoints + [attack_wp]
    if not replaced_fp:
