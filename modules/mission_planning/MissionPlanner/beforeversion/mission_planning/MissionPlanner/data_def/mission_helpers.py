...
def _scan_dem_tiles() -> Tuple[Tuple[Path, Tuple[float, float, float, float]], ...]:
    ...
    if not _DEM_DIR.exists():
        raise FileNotFoundError(f"DEM ���͸��� ã�� �� �����ϴ�: {_DEM_DIR}")
    ...
    if not tiles:
        raise FileNotFoundError(f"resource/ �Ʒ����� ����� GeoTIFF (*.tif)�� ã�� ���߽��ϴ�: {_DEM_DIR}")
    return tuple(tiles)
...
def _load_dem_data(path: Path):
    ...
    if not path.exists():
        raise FileNotFoundError(f"DEM ������ ã�� �� �����ϴ�: {path}")
