...
def _solve_with_milp_legacy(
    ...
    for g, slot in enumerate(slots):
        for i, _ in enumerate(slot.candidates):
            vels = vel_table[(g, i)]
            for u in uav_ids:
                x[(g, i, u)] = pulp.LpVariable(f"x_{g}_{i}_{u}", lowBound=0, upBound=1, cat="Binary")
                ws = []
                for k, _ in enumerate(vels):
                    key = (g, i, u, k)
                    w[key] = pulp.LpVariable(f"w_{g}_{i}_{u}_{k}", lowBound=0, upBound=1, cat="Binary")
                    ws.append(w[key])
...
def _solve_greedy(
    ...
    for g, slot in enumerate(slots):
        ...
        for u in sorted(uav_ids, key=lambda x: cur_time[x]):
            best_idx = None
            best_cost = float("inf")
            best_vel = 100.0
            for i in remaining:
