...
def _slot_candidate_count(slots: List[ScheduleSlot]) -> int:
    return sum(len(slot.candidates) for slot in slots)


def _max_slot_candidate_count(slots: List[ScheduleSlot]) -> int:
    return max((len(slot.candidates) for slot in slots), default=0)


def _dense_transition_count(slots: List[ScheduleSlot], uav_ids: List[int]) -> int:
    return sum(
        len(slots[g].candidates) * len(slots[g + 1].candidates) * len(uav_ids)
        for g in range(max(0, len(slots) - 1))
    )


def _problem_variable_count(prob: Any) -> int:
    try:
        return len(prob.variables())
    except Exception:
        return 0


def _problem_constraint_count(prob: Any) -> int:
    try:
...
def _solve_with_milp_legacy(
    ...
    model_start = replan_perf.start_timer()
    ...
    replan_perf.add_elapsed(
        ...
        slots=n_slots,
        uavs=len(uav_ids),
        candidates=_slot_candidate_count(slots),
        max_candidates_per_slot=_max_slot_candidate_count(slots),
        dense_transitions=_dense_transition_count(slots, uav_ids),
        x_variables=len(x),
        w_variables=len(w),
        q_variables=len(q),
        variables=_problem_variable_count(prob),
        constraints=_problem_constraint_count(prob),
    )
    ...
    is_optimal = int(status_code) == int(getattr(pulp, "LpStatusOptimal", 1))
    replan_perf.add(
...
def _solve_with_milp_barrier(
    ...
    replan_perf.add_elapsed(
        ...
        max_candidates_per_slot=_max_slot_candidate_count(slots),
        dense_transitions=_dense_transition_count(slots, uav_ids),
        move_speed_candidates=len(move_speeds),
        x_variables=len(x),
        w_variables=len(w),
        q_variables=len(q),
        y0_variables=len(y0),
...
def run_milp_scheduling(
    ...
    replan_perf.add(
        ...
        elapsed_ms=float(solved.solve_ms),
        slots=len(slots),
        uavs=len(uav_ids),
        candidates=_slot_candidate_count(slots),
        dense_transitions=_dense_transition_count(slots, uav_ids),
        objective_value=float(solved.objective) if solved.objective is not None else 0.0,
        used_greedy=str(solved.solver).lower() == "greedy",
        optimal=str(solved.status).lower() == "optimal",
    )
    setattr(split_result, "schedule_result", out)
    return out
