...
from ..algo.area_review import review_assigned_areas_local
...
class MainWindow(QMainWindow):
    ...
    def _run_assignment_pipeline(self) -> None:
        ...
        try:
            ...
            if use_replan_flow:
                ...
                self._log("[PIPE][4] area-review skipped: disabled for base mission")
            QApplication.processEvents()
    ...
    def _run_build_0303_0304(self) -> None:
        ...
        try:
            ...
            fp_0303, fp_0304 = build_0303_0304_from_0302_packages(
                packages,
                mrpk=self.mrpk_data,
                planning_mode=getattr(self.split_result, "planning_mode", None),
            )
    ...
    def _open_algo_config_dialog(self) -> None:
        ...
        def _on_apply(payload):
            ...
            self._log(
                ...
                f"fov={values.get('line_custom_fov_deg')}, "
    ...
    def _run_review_area(self) -> None:
        ...
        if self.mrpk_data is None:
            QMessageBox.warning(self, "Missing 0203", "Load 0203 first.")
            return
        area_pieces = [p for p in self.split_result.pieces if int(p.mission_type) in (2, 3, 4, 5, 6)]
        ...
        try:
            ...
            if use_replan_flow:
                ...
                self._log("[REVIEW] area-review skipped: disabled for base mission")
                report = {"details": []}
                for d in report.get("details", []):
