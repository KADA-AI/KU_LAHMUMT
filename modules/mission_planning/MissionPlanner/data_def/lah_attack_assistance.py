...
if __package__ in (None, ""):  # launched as a script by the attack pipeline
    import sys
    from pathlib import Path

    # Running a file by path puts the script's own directory on sys.path, not
    # the repository root, so the absolute ``modules.*`` imports below fail.
...
ENEMY_HEIGHT_OFFSET = float(ENEMY_OBSERVER_HEIGHT_M)
ATTACK_LOS_CLEARANCE_M = float(LOS_CLEARANCE_M)
ATTACK_LOS_PROFILE_MAX_SAMPLES = 4096
...
def _list_tif_files(directory: str) -> List[str]:
    if not os.path.isdir(directory):
        return []
    return [str(path) for path in regional_dem_paths(directory)]
...
def _detect_raster_paths_cached(preferred_path: Optional[str] = None) -> Tuple[str, ...]:
    """
    Returns operational regional GeoTIFF paths. If preferred_path points to a
    file it is used directly; directories are restricted to the canonical
    Hongik/Inje/Jipo rasters.
    """
    candidates: List[str] = []
    if preferred_path:
...
def minimum_attack_altitude_for_los(
    elevation: np.ndarray,
    geotransform: Sequence[float],
    attack_world: Tuple[float, float],
    enemy_world: Tuple[float, float],
    *,
    target_height_m: float = ENEMY_HEIGHT_OFFSET,
    clearance_m: float = ATTACK_LOS_CLEARANCE_M,
    sample_step_m: Optional[float] = None,
    max_samples: int = ATTACK_LOS_PROFILE_MAX_SAMPLES,
) -> dict:
    """Return the minimum attack-point ASL altitude that clears the terrain profile.

    The DEM is already loaded by the attack-point search, so this adds only one
    vectorized sample along the target-to-attack segment.  For each terrain
    sample the required attack altitude is solved analytically from the LOS
    line equation; no iterative altitude search or additional raster I/O is
    performed.
    """
    try:
        attack_px, attack_py = world_to_pixel(
            float(attack_world[0]),
            float(attack_world[1]),
            geotransform,
        )
        enemy_px, enemy_py = world_to_pixel(
            float(enemy_world[0]),
            float(enemy_world[1]),
            geotransform,
        )
    except (TypeError, ValueError) as exc:
        return {"verified": False, "reason": f"coordinate_transform_failed:{exc}"}

    distance_m = world_distance_m(attack_world, enemy_world, geotransform)
    if not math.isfinite(distance_m) or distance_m <= 0.0:
        return {"verified": False, "reason": "invalid_attack_target_distance"}

    midpoint_px = (float(attack_px) + float(enemy_px)) * 0.5
    midpoint_py = (float(attack_py) + float(enemy_py)) * 0.5
    pixel_size_m = estimate_pixel_size_meters(geotransform, midpoint_px, midpoint_py)
    requested_step_m = float(sample_step_m) if sample_step_m is not None else 0.0
    if not math.isfinite(requested_step_m) or requested_step_m <= 0.0:
        # Half-cell sampling prevents a diagonal segment from skipping a DEM
        # cell while remaining tiny compared with the radial LOS calculation.
        requested_step_m = max(1.0, float(pixel_size_m) * 0.5)
    sample_cap = max(3, min(65536, int(max_samples)))
    sample_count = max(3, int(math.ceil(float(distance_m) / requested_step_m)) + 1)
    sample_count = min(sample_cap, sample_count)

    fractions = np.linspace(0.0, 1.0, int(sample_count), dtype=float)
    xs = float(enemy_px) + (float(attack_px) - float(enemy_px)) * fractions
    ys = float(enemy_py) + (float(attack_py) - float(enemy_py)) * fractions
    terrain = _nearest_sample(elevation, xs, ys)
    finite = np.isfinite(terrain)
    if not bool(np.all(finite)):
        return {
            "verified": False,
            "reason": "terrain_profile_contains_nodata",
            "distance_m": float(distance_m),
            "sample_count": int(sample_count),
            "valid_sample_count": int(np.count_nonzero(finite)),
        }

    # One shared analytical kernel keeps the optional GDAL/in-process path and
    # the always-available batched DEM path on exactly the same LOS policy.
...
def _candidate_between_friendly_and_enemy(
    candidate_world: Tuple[float, float],
    friendly_world: Tuple[float, float],
    enemy_world: Tuple[float, float],
) -> bool:
    """Reject terrain candidates beyond either the target or the LAH."""
    lat_ref = (float(friendly_world[1]) + float(enemy_world[1])) * 0.5
    lon_scale = meters_per_degree_lon(lat_ref)
    lat_scale = meters_per_degree_lat(lat_ref)
    friendly_x = (float(friendly_world[0]) - float(enemy_world[0])) * float(lon_scale)
    friendly_y = (float(friendly_world[1]) - float(enemy_world[1])) * float(lat_scale)
    candidate_x = (float(candidate_world[0]) - float(enemy_world[0])) * float(lon_scale)
    candidate_y = (float(candidate_world[1]) - float(enemy_world[1])) * float(lat_scale)
    length_sq = (friendly_x * friendly_x) + (friendly_y * friendly_y)
    if length_sq <= 1.0:
        return False
    projection = ((candidate_x * friendly_x) + (candidate_y * friendly_y)) / length_sq
    return -0.01 <= float(projection) <= 1.01
...
def choose_attack_point(
    ...
    centroid_infos = [
        info
        for info in centroid_infos
        if _candidate_between_friendly_and_enemy(
            info["centroid"],
            friendly_world,
            enemy_world,
        )
...
def parse_args():
    ...
    parser.add_argument(
        ...
        type=str,
        default=None,
        help="GeoTIFF file or directory. Directory/default discovery uses only Hongik_48km, Inje_10m, and Jipo_48km.",
    )
    ...
    parser.add_argument(
        "--los-clearance-m",
        type=float,
        default=ATTACK_LOS_CLEARANCE_M,
        help="Minimum vertical clearance above terrain along the target-to-attack LOS.",
    )
    parser.add_argument(
        "--los-target-height-m",
        type=float,
        default=ENEMY_HEIGHT_OFFSET,
        help="Target height above local terrain used as the LOS origin.",
    )
    parser.add_argument(
        "--los-profile-step-m",
        type=float,
        default=0.0,
        help="Terrain-profile sample spacing; <=0 selects half a DEM cell.",
    )
...
def main():
    ...
    los_profile = minimum_attack_altitude_for_los(
        elevation,
        geotransform,
        (float(best_point[0]), float(best_point[1])),
        enemy_world,
        target_height_m=max(0.0, float(args.los_target_height_m)),
        clearance_m=max(0.0, float(args.los_clearance_m)),
        sample_step_m=(
            float(args.los_profile_step_m)
            if float(args.los_profile_step_m) > 0.0
            else None
        ),
    )
    altitude_int = None
