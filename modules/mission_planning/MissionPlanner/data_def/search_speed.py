...
# ICD lineSearch.searchSpeed is validated to the 0..1000 m/s range.
    ...
    *,
    ...
    try:
        ...
        if _SearchSpeedCalculator is not None:
            ...
            return round(min(base_speed * float(effective_weight), ICD_SEARCH_SPEED_MAX_MPS), 2)
        ratio = cruise_speed_mps / spacing_m
        search_speed = sweep_len_m * ratio
        return round(min(max(0.0, search_speed * effective_weight), ICD_SEARCH_SPEED_MAX_MPS), 2)
    except Exception:
        return None
