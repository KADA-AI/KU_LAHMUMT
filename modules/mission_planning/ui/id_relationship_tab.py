...
class RelationshipCache:
    ...
    def _load_packages(self) -> None:
        ...
        for path in sorted(input_dir.glob("*.json")):
            payload = self._safe_load(path, "[InputMissionPlan]")
            if payload is None:
                continue
    ...
    def _load_mission_plans(self) -> None:
        ...
        for path in sorted(plan_dir.glob("*.json")):
            payload = self._safe_load(path, "[MissionPlan]")
            if payload is None:
                continue
    ...
    def _load_individual_packages(self) -> None:
        ...
        for path in sorted(imp_dir.glob("*.json")):
            payload = self._safe_load(path, "[IndividualMissionPlan]")
            if payload is None:
                continue
    ...
    def _load_flight_paths(self) -> None:
        ...
        for path in sorted(path_dir.glob("*.json")):
            payload = self._safe_load(path, "[FlightPath]")
            if payload is None:
                continue
    ...
    def _load_replan_logs(self) -> None:
        ...
        for path in sorted(log_dir.glob("*.json")):
            payload = self._safe_load(path, "[DSS_Internal]")
