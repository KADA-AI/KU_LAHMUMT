...
class MissionAlgoConfigTab(QWidget):
    ...
    def _build_flyover_group(self) -> QGroupBox:
        ...
        form.addRow(self._label("flyover_dubins_prefix", "Dubins prefix 사용"), self._check("flyover_dubins_prefix"))
    ...
    def _apply_payload(self, payload: Dict[str, Any]) -> None:
        ...
        try:
            ...
            self._widgets["search_speed_weight"].setValue(float(values.get("search_speed_weight", 3.0)))
    ...
    def _collect_payload(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "flyover": {
                ...
                "dubins_prefix": bool(self._widgets["flyover_dubins_prefix"].isChecked()),
    ...
    def _schedule_save(self, *args) -> None:
        if self._building:
            return
        preset_combo: QComboBox | None = self._widgets.get("preset_key")
        if (
            not self._applying_preset
            and isinstance(preset_combo, QComboBox)
            and preset_combo.currentData() != "custom"
        ):
            self._building = True
            try:
                idx = preset_combo.findData("custom")
                if idx >= 0:
    ...
    def _refresh_mode_hints(self) -> None:
        ...
        if sep_widget is not None:
            sep_widget.setEnabled(is_custom)
        if fov_widget is not None:
            fov_widget.setEnabled(is_custom)
        ...
        if use_db:
            ...
            self._db_usage_hint.setText(
                ...
                "위 두 값은 DB를 끄거나 fallback이 필요할 때만 사용된다."
            )
        else:
            if sep_label is not None:
            ...
            if is_custom:
                self._db_usage_hint.setText(
                    "Custom 모드는 DB를 사용하지 않는다. 현재 일반 Line/Area는 아래 기본 이격거리/FOV 하드코딩 값을 직접 사용한다."
                )
            else:
                self._db_usage_hint.setText(
                    "현재 일반 Line/Area는 아래 기본 이격거리/FOV 하드코딩 값을 직접 사용한다."
                )
        ...
        if not is_custom:
            self._status.setText(f"프리셋 고정값 사용 중: {preset_combo.currentText() if isinstance(preset_combo, QComboBox) else preset_key}")
