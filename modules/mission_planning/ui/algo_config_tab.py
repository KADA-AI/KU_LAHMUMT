...
GENERAL_GROUPS: list[tuple[str, str, list[FieldSpec]]] = [
    (
        ...
        [
            ...
            FieldSpec("values", "area_wp_interval_m", "Area WP 간격(m)", "float", 100.0, 10000.0, 100.0, 1),
    ...
    (
        ...
        [
            FieldSpec("values", "global_manual_fov_deg", "기본 FOV (수동)", "float", MIN_LINE_FOV_DEG, 120.0, 0.1, 2),
            ...
            FieldSpec("values", "fov_db_sep_safety_factor", "FOV DB SEP 보수 배수", "float", 1.0, 5.0, 0.05, 2),
    ...
    (
        ...
        [
            ...
            FieldSpec("values", "line_sweep_interpolation_enabled", "LINE/AREA 중간점 보간 사용", "bool"),
            FieldSpec("values", "sweep_line_interp_points", "Sweep 점 개수", "int", 2, 15, 1),
            FieldSpec("values", "sweep_interp_max_gap_m", "Sweep 점 최대 간격(m, 0=끔)", "float", 0.0, 20000.0, 50.0, 1),
    ...
    (
        ...
        [
            ...
            FieldSpec("values", "next_collab_area_density_scale", "Next Area 탐색 밀도 배수", "float", 0.2, 10.0, 0.05, 2),
            FieldSpec("values", "next_collab_area_gsd_margin_ratio", "Next Area GSD 여유비율", "float", 0.10, 1.00, 0.05, 2),
            FieldSpec("values", "area_fov_db_smaller_fov_steps", "Area DB FOV lower steps", "int", 3, 10, 1),
            FieldSpec("values", "area_search_speed_max_mps", "Area 촬영속도 상한(m/s, 0=끔)", "float", 0.0, 5000.0, 50.0, 0),
        ],
    ),
    (
    ...
    (
        ...
        [
            ...
            FieldSpec("values", "fov_db_smaller_fov_steps", "DB FOV 낮춤 단계", "int", 3, 10, 1),
...
_GENERAL_FRONT_KEYS = {"uav_wp_interval_m", "area_wp_interval_m", "lah_wp_interval_m"}
GENERAL_GROUPS = [
    (
        title,
        note,
...
NEXT_COLLAB_GROUPS: list[tuple[str, str, list[FieldSpec]]] = [
    ...
    (
        ...
        [
            ...
            FieldSpec("values", "next_collab_area_density_scale", "Area 탐색 밀도 배수", "float", 0.20, 10.00, 0.05, 2),
            FieldSpec("values", "next_collab_area_search_speed_scale", "Area sweep speed scale", "float", 0.10, 5.00, 0.05, 2),
            FieldSpec("values", "next_collab_area_gsd_margin_ratio", "GSD 여유비율", "float", 0.10, 1.00, 0.05, 2),
        ],
    ),
    (
        "전문가 / 첫 LINE",
...
OPERATION_GROUPS: list[tuple[str, str, list[FieldSpec]]] = [
    (
        ...
        [
            ...
            FieldSpec("values", "camera_adjust_percent", "확대/축소 비율(%)", "float", -90.0, 90.0, 1.0, 1),
            FieldSpec("values", "global_manual_fov_deg", "수동 FOV(deg)", "float", MIN_LINE_FOV_DEG, 120.0, 0.1, 2),
...
class MissionAlgoConfigTab(QWidget):
    ...
    def _normalize_payload(self, payload: Dict[str, Any] | None) -> Dict[str, Any]:
        ...
        values["fov_db_sep_safety_factor"] = max(
            1.0,
            min(5.0, self._as_float(values.get("fov_db_sep_safety_factor"), 1.7)),
        )
        ...
        values["fov_db_smaller_fov_steps"] = max(
            3,
            min(10, int(self._as_float(values.get("fov_db_smaller_fov_steps"), 4))),
        )
        values["area_fov_db_smaller_fov_steps"] = max(
            3,
            min(10, int(self._as_float(values.get("area_fov_db_smaller_fov_steps"), 3))),
        )
        ...
        values["next_collab_area_gsd_margin_ratio"] = max(
            0.10,
            min(1.0, self._as_float(values.get("next_collab_area_gsd_margin_ratio"), 0.90)),
        )
        values["uav_wp_interval_m"] = self._as_float(values.get("uav_wp_interval_m"), 2000.0)
        values["area_wp_interval_m"] = self._as_float(values.get("area_wp_interval_m"), 1000.0)
        ...
        values["line_sweep_interpolation_enabled"] = bool(
            values.get("line_sweep_interpolation_enabled", False)
        )
        values["enhanced_area_review_max_segment_m"] = self._as_float(values.get("enhanced_area_review_max_segment_m"), 1500.0)
        ...
        values["next_collab_area_path0_trigger_sep_m"] = max(
            0.0,
            min(1500.0, self._as_float(values.get("next_collab_area_path0_trigger_sep_m"), 1500.0)),
        )
