...
class MainGUI(QWidget):
    ...
    def __init__(self) -> None:
        ...
        self._btn_0302: QPushButton | None = None
        self._map_center = [38.128774, 127.318005]
        self._map_zoom = 14
        self._map_auto_center_done = False
    ...
    def _rebuild_map(self):
        ...
        script = """
            (function(){
                let mp = null;
                for (let k in window) {
                    if (window[k] instanceof L.Map) { mp = window[k]; break; }
                }
                if (!mp) { return null; }
                const center = mp.getCenter();
                return { lat: center.lat, lon: center.lng, zoom: mp.getZoom() };
            })();
        """
        self.map_view.page().runJavaScript(script, self._apply_map_snapshot)

    def _apply_map_snapshot(self, state: dict | None):
        if isinstance(state, dict):
            ...
            try:
                if lat is not None and lon is not None:
                    self._map_center = [float(lat), float(lon)]
                if zoom is not None:
                    self._map_zoom = float(zoom)
    ...
    def _write_map_html(self):
        ...
        center = list(self._map_center)
        zoom_start = self._map_zoom

        if bounds_coords and not self._map_auto_center_done:
            ...
            self._map_zoom = zoom_start
            self._map_auto_center_done = True
        ...
        if bounds_coords and not self._map_auto_center_done:
            min_lat = min(lat for lat, _ in bounds_coords)
            max_lat = max(lat for lat, _ in bounds_coords)
            min_lon = min(lon for _, lon in bounds_coords)
            max_lon = max(lon for _, lon in bounds_coords)
...
# ─────────────────────────── 초기임무계획 탭 ───────────────────────────
    def _build_tab_initial(self):
        ...
        for tag, label in (("0201", "0201"), ("0203", "0203"), ("0302", "0302")):
    ...
    def _build_tab_0304(self):
        ...
        btn_check = QPushButton("Check Missions")  # reuse existing button
