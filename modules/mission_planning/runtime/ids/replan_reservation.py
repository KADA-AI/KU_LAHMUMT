...
# How many IDs to pull at once when a block runs dry.  Small enough not to waste
# the namespace, large enough that a builder overshooting by a few does not make
# a round trip per ID.
_REFILL_BATCH = 8
...
class ReservedIdBlock:
    ...
    ids: list[int]
    _index: int = 0
    # Pulls more IDs from the global allocator when the reservation runs out.
    # The block sizes are estimates made before the builders run; when one
    # overshoots, extending is correct and crashing the whole replan is not.
    # The IDs come from the same allocator the reservation itself drew from, so
    # they stay unique and monotonic.
    refill: Optional[Callable[[int], list[int]]] = None
    ...
    def next(self) -> int:
        if self._index >= len(self.ids):
            ...
            extra = [int(value) for value in (self.refill(_REFILL_BATCH) or [])]
            if not extra:
                raise RuntimeError(f"reserved ID block exhausted: {self.name}")
            self.ids.extend(extra)
            self.overflow_count += len(extra)
...
def _refill_paths(aircraft_id: int) -> Callable[[int], list[int]]:
    def _pull(count: int) -> list[int]:
        return [int(value) for value in id_allocator.reserve_path_ids(int(aircraft_id), int(count))]

    return _pull


def _refill_imps(count: int) -> list[int]:
    return [int(value) for value in id_allocator.reserve_imp_ids(int(count))]


def _refill_individuals(count: int) -> list[int]:
    return [int(value) for value in id_allocator.reserve_individual_mission_ids(int(count))]


def _refill_waypoints(count: int) -> list[int]:
    start = int(id_allocator.reserve_waypoint_block(int(count)))
    return list(range(start, start + int(count)))


def _refill_plans(count: int) -> list[int]:
    return [int(value) for value in id_allocator.reserve_mission_plan_ids(int(count))]
        ...
        *,
        ...
        if can_bundle:
            ...
            for raw_aircraft_id, values in bundled_paths.items():
                ...
                path_blocks[aircraft_id] = ReservedIdBlock(
                    ...
                    refill=_refill_paths(aircraft_id),
                )
            imp_ids = [
                int(value)
        ...
        return cls(
            ...
            individual_ids=ReservedIdBlock(
                ...
                refill=_refill_individuals,
            ),
            waypoint_ids=ReservedIdBlock("waypoint", waypoint_ids, refill=_refill_waypoints),
