...
from modules.mission_planning.planning_modes import MissionPlanningMode, mission_mode_context
...
def _normalize_aircraft_rows(
    ...
    *,
    ...
    prediction_enabled = bool(
        get_runtime_bool("next_collab_area_activation_prediction_enabled", True)
    )
    prediction_confidence_min = max(
        0.0,
        min(
            1.0,
            float(
                get_runtime_float(
                    "next_collab_area_activation_prediction_confidence_min",
                    0.70,
                )
            ),
        ),
    )
    prediction_max_age_s = max(
        0.0,
        float(
            get_runtime_float(
                "next_collab_area_activation_prediction_max_age_s",
                2.0,
            )
        ),
    )
    straight_turn_rate_max_dps = max(
        0.1,
        float(
            get_runtime_float(
                "next_collab_area_activation_straight_turn_rate_max_dps",
                1.0,
            )
        ),
    )
    straight_min_samples = max(
        2,
        int(
            get_runtime_float(
                "next_collab_area_activation_straight_min_samples",
                4.0,
            )
        ),
    )
    ...
    for entry in aircraft_entries:
        ...
        current_position_xy = coord_to_xy(coord)
        if aircraft_id <= 0 or current_position_xy is None:
            continue
        speed_mps = None
        for candidate in (entry.get("speedMps"), entry.get("speed_mps")):
            try:
                parsed = float(candidate)
            except Exception:
                continue
            if math.isfinite(parsed) and parsed > 1.0:
                speed_mps = parsed
                break

        confidence = None
        try:
            parsed_confidence = float(entry.get("lineTurnDirectionConfidence"))
            if math.isfinite(parsed_confidence):
                confidence = max(0.0, min(1.0, parsed_confidence))
        except Exception:
            confidence = None
        prediction_lead_s = None
        try:
            parsed_lead = float(entry.get("linePredictionLeadS"))
            if math.isfinite(parsed_lead) and parsed_lead > 0.0:
                prediction_lead_s = parsed_lead
        except Exception:
            prediction_lead_s = None
        turn_data_age_s = None
        try:
            parsed_age = float(entry.get("lineTurnDataAgeS"))
            if math.isfinite(parsed_age) and parsed_age >= 0.0:
                turn_data_age_s = parsed_age
        except Exception:
            turn_data_age_s = None
        try:
        ...
        try:
            ...
            trend_turn_sign = 0
        try:
            current_turn_rate_dps = abs(float(entry.get("turnRateDps") or 0.0))
        except Exception:
            current_turn_rate_dps = float("inf")
        try:
            trend_turn_rate_dps = abs(float(entry.get("lineTrendTurnRateDps") or 0.0))
        except Exception:
            trend_turn_rate_dps = float("inf")
        try:
            trend_sample_count = int(entry.get("lineTrendSampleCount") or 0)
        except Exception:
            trend_sample_count = 0

        predicted_coord = entry.get("linePredictedEntryCoordinate")
        predicted_position_xy = coord_to_xy(predicted_coord)
        prediction_fresh = (
            turn_data_age_s is None
            or float(turn_data_age_s) <= float(prediction_max_age_s)
        )
        prediction_distance_ok = False
        if predicted_position_xy is not None and prediction_lead_s is not None:
            displacement_m = math.hypot(
                float(predicted_position_xy[0]) - float(current_position_xy[0]),
                float(predicted_position_xy[1]) - float(current_position_xy[1]),
            )
            expected_displacement_m = (
                float(speed_mps) * float(prediction_lead_s)
                if speed_mps is not None
                else 0.0
            )
            # Reject a corrupted/out-of-frame prediction, but allow enough
            # slack for curved motion and coordinate quantisation.
            prediction_distance_ok = displacement_m <= max(
                100.0,
                expected_displacement_m * 1.75 + 50.0,
            )
        straight_prediction = bool(
            raw_turn_sign == 0
            and trend_turn_sign == 0
            and current_turn_rate_dps <= float(straight_turn_rate_max_dps)
            and trend_turn_rate_dps <= float(straight_turn_rate_max_dps)
            and trend_sample_count >= int(straight_min_samples)
        )
        confident_turn_prediction = bool(
            confidence is not None
            and float(confidence) >= float(prediction_confidence_min)
        )
        use_prediction = bool(
            prediction_enabled
            and (confident_turn_prediction or straight_prediction)
            and prediction_lead_s is not None
            and prediction_fresh
            and prediction_distance_ok
            and predicted_position_xy is not None
        )
        position_xy = predicted_position_xy if use_prediction else current_position_xy

        heading_deg = (
            entry.get("linePredictedHeadingDeg")
            if use_prediction and entry.get("linePredictedHeadingDeg") is not None
            else entry.get("headingDeg")
        )
        try:
            heading_val = (
        ...
        rows.append(
            {
                ...
                "currentPositionXY": (
                    float(current_position_xy[0]),
                    float(current_position_xy[1]),
                ),
                "headingDeg": float(heading_val),
                "speedMps": speed_mps,
                "turnRadiusM": turn_radius_m,
                "activationPredictionUsed": bool(use_prediction),
                "activationPredictionLeadS": prediction_lead_s,
                "activationPredictionConfidence": confidence,
                "activationPredictionMode": (
                    "straight"
                    if use_prediction and straight_prediction
                    else "turn"
                    if use_prediction
                    else "current"
                ),
...
def run_next_collab_division_plan(
    *,
    ...
    mode_context = mission_mode_context(mode=planning_mode)
    ...
    if log is not None:
        forecast_rows = [
            row for row in aircraft_rows if bool(row.get("activationPredictionUsed"))
        ]
        if forecast_rows:
            log(
                "[NEXTCOLLAB][AREA] activation forecast applied "
                f"aircraft={[int(row['aircraftID']) for row in forecast_rows]} "
                f"leadS={[round(float(row.get('activationPredictionLeadS') or 0.0), 1) for row in forecast_rows]}"
            )
    ...
    try:
        try:
            planner._turn_radius_scale = max(0.1, float(turn_radius_scale or 1.2))
        except Exception:
