...
def _should_snapshot_pipeline_event(
    *,
    event: str,
    phase: str | None,
    ...
    if any(token in normalized_outcome for token in ("fail", "error", "exception", "crash")):
        return True
    if str(event or "") == "pipeline_total":
        return True
    if str(event or "") == "replan_checkpoint" and str(phase or "") in _DIAGNOSTIC_BOUNDARY_PHASES:
        return True
    try:
        return str(event or "") == "pipeline_phase" and float(elapsed_ms or 0.0) >= 250.0
    except Exception:
        return False
...
def emit_pipeline_event(
    *,
    ...
    try:
        ...
        if _should_snapshot_pipeline_event(
            event=event,
            phase=phase,
            ...
            diagnostic_extra = {
                ...
                if str(key) in _DIAGNOSTIC_EXTRA_KEYS
            }
            diagnostic_context = {
                "event": event,
                "replanTransactionId": replan_transaction_id,
            ...
            if diagnostic_extra:
                diagnostic_context["extra"] = diagnostic_extra
            request_runtime_diagnostic_snapshot(
                module,
                "replan_pipeline_boundary",
