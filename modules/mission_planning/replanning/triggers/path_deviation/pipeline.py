...
try:
    from modules.common.turn_dynamics import (
        TurnPerformance as _SharedTurnPerformance,
        normalize_attitude as _shared_normalize_attitude,
        predict_turn_motion as _shared_predict_turn_motion,
        project_attitude_motion as _shared_project_attitude_motion,
    )
except Exception:
    _SharedTurnPerformance = None
    _shared_normalize_attitude = None
    _shared_predict_turn_motion = None
    _shared_project_attitude_motion = None
...
_PATHDEV_ATTITUDE_DEFAULT_LEAD_S = 3.0
_PATHDEV_ATTITUDE_MIN_LEAD_S = 0.5
_PATHDEV_ATTITUDE_MAX_LEAD_S = 12.0
_PATHDEV_ATTITUDE_MAX_SNAPSHOT_AGE_S = 5.0
_PATHDEV_ATTITUDE_MAX_TRIGGER_AGE_S = 30.0
_PATHDEV_ATTITUDE_MIN_SPEED_MPS = 0.5
_PATHDEV_ATTITUDE_MAX_SPEED_MPS = 150.0
_PATHDEV_ATTITUDE_MAX_COURSE_YAW_ERROR_DEG = 35.0
...
def _ci_get(payload: Any, *keys: str) -> Any:
    if not isinstance(payload, dict):
        return None
...
def _normalize_attitude_payload(payload: Any) -> Optional[Dict[str, float]]:
    if not isinstance(payload, dict):
        return None

    normalized = None
    if callable(_shared_normalize_attitude):
        try:
            normalized = _shared_normalize_attitude(payload)
        except Exception:
            normalized = None
    source = normalized if isinstance(normalized, dict) else payload
    roll = _to_float(_ci_get(source, "roll", "rollDeg"))
    pitch = _to_float(_ci_get(source, "pitch", "pitchDeg"))
    yaw = _to_float(_ci_get(source, "yaw", "yawDeg"))
    if roll is None or pitch is None or yaw is None:
        return None
    if not all(math.isfinite(value) for value in (roll, pitch, yaw)):
        return None
    if not -180.0 <= float(roll) <= 180.0 or not -90.0 <= float(pitch) <= 90.0:
        return None
    return {
        "roll": float(roll),
        "pitch": float(pitch),
...
def _normalize_pathdev_velocity(payload: Any) -> Dict[str, float]:
    if not isinstance(payload, dict):
        return {}
    speed = _to_float(_ci_get(payload, "speed", "speedMps", "groundSpeed", "groundSpeedMps"))
    heading = _to_float(_ci_get(payload, "heading", "headingDeg", "course", "courseDeg"))
    result: Dict[str, float] = {}
    if (
        speed is not None
        and math.isfinite(speed)
        and _PATHDEV_ATTITUDE_MIN_SPEED_MPS <= float(speed) <= _PATHDEV_ATTITUDE_MAX_SPEED_MPS
    ):
...
def _normalize_turn_dynamics(payload: Any) -> Dict[str, Any]:
    if not isinstance(payload, dict):
        return {}
    result: Dict[str, Any] = {}
    scalar_fields = {
        "rollRateDegS": ("rollRateDegS", "roll_rate_dps"),
        "targetRollDeg": ("targetRollDeg", "target_roll_deg"),
        "yawTurnRateNavDegS": ("yawTurnRateNavDegS", "yaw_rate_nav_dps"),
        "attitudeTurnRateNavDegS": (
            "attitudeTurnRateNavDegS",
            "attitude_rate_nav_dps",
        ),
        "attitudeRateScale": ("attitudeRateScale", "turnRateScale"),
    }
    for output_key, aliases in scalar_fields.items():
        value = _to_float(_ci_get(payload, *aliases))
        if value is not None and math.isfinite(value):
            result[output_key] = float(value)
    sample_count = _to_int(
        _ci_get(payload, "attitudeRateSampleCount", "attitude_rate_sample_count")
    )
    if sample_count is not None and sample_count >= 0:
        result["attitudeRateSampleCount"] = int(sample_count)

    performance_raw = _ci_get(payload, "performance", "turnPerformance")
    if not isinstance(performance_raw, dict):
        performance_raw = {}
    performance: Dict[str, Any] = {}
    performance_fields = {
        "referenceRadiusScale": ("referenceRadiusScale", "reference_radius_scale"),
        "minTurnRadiusM": ("minTurnRadiusM", "min_turn_radius_m"),
        "maxTurnRateDegS": ("maxTurnRateDegS", "max_turn_rate_dps"),
        "bankLimitDeg": ("bankLimitDeg", "bank_limit_deg"),
        "rollRateLimitDegS": ("rollRateLimitDegS", "roll_rate_limit_dps"),
        "rollTimeConstantS": ("rollTimeConstantS", "roll_time_constant_s"),
        "turnRateScale": ("turnRateScale", "turn_rate_scale"),
    }
    for output_key, aliases in performance_fields.items():
        value = _to_float(_ci_get(performance_raw, *aliases))
        if value is not None and math.isfinite(value):
            performance[output_key] = float(value)
    use_reference = _ci_get(performance_raw, "useReferenceRadius", "use_reference_radius")
    if use_reference is not None:
        if isinstance(use_reference, str):
            performance["useReferenceRadius"] = use_reference.strip().casefold() not in {
                "0",
                "false",
                "no",
                "off",
            ...
            performance["useReferenceRadius"] = bool(use_reference)
    table_raw = _ci_get(
        performance_raw,
        "referenceRadiusTable",
        "reference_radius_table_mps",
    )
    ...
    if isinstance(table_raw, list):
        for row in table_raw:
            if not isinstance(row, (list, tuple)) or len(row) < 2:
                continue
            speed = _to_float(row[0])
            radius = _to_float(row[1])
            if speed is not None and radius is not None and speed >= 0.0 and radius > 0.0:
                table.append([float(speed), float(radius)])
    if table:
        performance["referenceRadiusTable"] = table
    if performance:
        result["performance"] = performance
    return result


def _turn_performance_from_payload(payload: Any) -> Any:
    if _SharedTurnPerformance is None:
        return None
    dynamics = _normalize_turn_dynamics(payload)
    performance = dynamics.get("performance") if isinstance(dynamics.get("performance"), dict) else {}
    table_raw = performance.get("referenceRadiusTable")
    table = None
    if isinstance(table_raw, list):
        table = tuple((float(row[0]), float(row[1])) for row in table_raw if len(row) >= 2)
    kwargs: Dict[str, Any] = {}
    if table:
        kwargs["reference_radius_table_mps"] = table
    mappings = {
        "referenceRadiusScale": "reference_radius_scale",
        "minTurnRadiusM": "min_turn_radius_m",
        "maxTurnRateDegS": "max_turn_rate_dps",
        "bankLimitDeg": "bank_limit_deg",
        "rollRateLimitDegS": "roll_rate_limit_dps",
        "rollTimeConstantS": "roll_time_constant_s",
        "turnRateScale": "turn_rate_scale",
    }
    for source_key, target_key in mappings.items():
        if source_key in performance:
            kwargs[target_key] = float(performance[source_key])
    if "turn_rate_scale" not in kwargs and "attitudeRateScale" in dynamics:
        kwargs["turn_rate_scale"] = float(dynamics["attitudeRateScale"])
    if "useReferenceRadius" in performance:
        kwargs["use_reference_radius"] = bool(performance["useReferenceRadius"])
    try:
        return _SharedTurnPerformance(**kwargs)
    except Exception:
        return _SharedTurnPerformance()


def _normalize_trigger_coordinate(payload: Any) -> Optional[Dict[str, float]]:
    if not isinstance(payload, dict):
        return None
    latitude = _to_float(_ci_get(payload, "latitude"))
    longitude = _to_float(_ci_get(payload, "longitude"))
    altitude = _to_float(_ci_get(payload, "altitude"))
...
def _snapshot_age_seconds(
    ...
    *,
    ...
    if not isinstance(snapshot_payload, dict):
        return None
    saved_at = _ci_get(snapshot_payload, "saved_at", "savedAt")
    if not isinstance(saved_at, str) or not saved_at.strip():
        return None
    try:
        parsed = datetime.fromisoformat(saved_at.strip().replace("Z", "+00:00"))
        if parsed.tzinfo is None:
            parsed = parsed.replace(tzinfo=timezone.utc)
        now_value = float(time.time() if now_epoch_s is None else now_epoch_s)
        return max(0.0, now_value - float(parsed.timestamp()))
    except Exception:
        return None


def _detail_age_seconds(
    detail: Dict[str, Any],
    *,
    now_epoch_s: Optional[float] = None,
    now_since_2000_ms: Optional[int] = None,
) -> Optional[float]:
    timestamp = _to_float(_ci_get(detail, "timestamp", "triggerTimestamp"))
    if timestamp is None or not math.isfinite(timestamp) or timestamp <= 0.0:
        return None
    now_epoch = float(time.time() if now_epoch_s is None else now_epoch_s)
    if timestamp >= 1_200_000_000_000.0:
        age_s = (now_epoch * 1000.0 - float(timestamp)) / 1000.0
    elif timestamp >= 100_000_000_000.0:
        current_ms = int(_now_ms_since_2000() if now_since_2000_ms is None else now_since_2000_ms)
        age_s = (float(current_ms) - float(timestamp)) / 1000.0
    else:
        return None
    return max(0.0, float(age_s))


def _motion_state_from_trigger_detail(detail: Dict[str, Any]) -> Optional[Dict[str, Any]]:
    coordinate = _normalize_trigger_coordinate(_ci_get(detail, "triggerCoordinate"))
    velocity = _normalize_pathdev_velocity(_ci_get(detail, "triggerVelocity"))
    attitude = _normalize_attitude_payload(_ci_get(detail, "triggerAttitude"))
    turn_dynamics = _normalize_turn_dynamics(_ci_get(detail, "triggerTurnDynamics"))
    if coordinate is None or attitude is None or "speed" not in velocity:
        return None
    if "heading" not in velocity:
        velocity["heading"] = float(attitude["yaw"]) % 360.0
    return {
        "coordinate": coordinate,
        "velocity": velocity,
        "attitude": attitude,
        "turnDynamics": turn_dynamics,
        "source": "monitoring0902Trigger",
    }


def _motion_state_from_snapshot(state: Dict[str, Any] | None) -> Optional[Dict[str, Any]]:
    if not isinstance(state, dict):
        return None
    coordinate = _normalize_trigger_coordinate(
        _ci_get(state, "coordinate", "currentCoordinate")
    )
    attitude = _normalize_attitude_payload(_ci_get(state, "attitude"))
    turn_dynamics = _normalize_turn_dynamics(
        _ci_get(state, "turnDynamics", "triggerTurnDynamics")
    )
    velocity = _normalize_pathdev_velocity(_ci_get(state, "velocity"))
    if "speed" not in velocity:
        speed = _to_float(_ci_get(state, "speedMps", "speed"))
        if (
            speed is not None
            and math.isfinite(speed)
            and _PATHDEV_ATTITUDE_MIN_SPEED_MPS <= float(speed) <= _PATHDEV_ATTITUDE_MAX_SPEED_MPS
        ):
            velocity["speed"] = float(speed)
    if "heading" not in velocity:
        heading = _to_float(_ci_get(state, "headingDeg", "heading"))
        if heading is not None and math.isfinite(heading):
            velocity["heading"] = float(heading) % 360.0
    if "heading" not in velocity and attitude is not None:
        velocity["heading"] = float(attitude["yaw"]) % 360.0
    if coordinate is None or attitude is None or "speed" not in velocity or "heading" not in velocity:
        return None
    return {
        "coordinate": coordinate,
        "velocity": velocity,
        "attitude": attitude,
        "turnDynamics": turn_dynamics,
        "source": "latest0401Snapshot",
    }


def _bounded_attitude_prediction_lead(
    detail: Dict[str, Any],
    *,
    source_coordinate: Dict[str, Any],
    fallback_coordinate: Dict[str, Any],
    speed_mps: float,
) -> tuple[float, str, Optional[float]]:
    requested = _to_float(_ci_get(detail, "alternateWaypointEtaS"))
    source = "alternateWaypointEtaS"
    if requested is None or not math.isfinite(requested) or requested <= 0.0:
        distance_m = _coord_distance_m(source_coordinate, fallback_coordinate)
        if distance_m is not None and distance_m > 1.0 and speed_mps > 0.0:
            requested = float(distance_m) / float(speed_mps)
            source = "fallbackCoordinateDistance"
        else:
            requested = float(_PATHDEV_ATTITUDE_DEFAULT_LEAD_S)
            source = "boundedDefault"
    lead_s = max(
        float(_PATHDEV_ATTITUDE_MIN_LEAD_S),
        min(float(_PATHDEV_ATTITUDE_MAX_LEAD_S), float(requested)),
    )
    return float(lead_s), source, float(requested)


def _refine_alternate_coordinate_with_attitude(
    detail: Dict[str, Any],
    *,
    fallback_coordinate: Dict[str, Any],
    snapshot_payload: Dict[str, Any] | None,
    snapshot_state: Dict[str, Any] | None,
    projector: Optional[Callable[..., Any]] = None,
    now_epoch_s: Optional[float] = None,
    now_since_2000_ms: Optional[int] = None,
) -> tuple[Dict[str, Any], Dict[str, Any]]:
    original_fallback = dict(fallback_coordinate)
    fallback = _normalize_trigger_coordinate(fallback_coordinate)
    if fallback is None:
        return original_fallback, {
            "applied": False,
            "predictionModel": "monitorAlternateCoordinate",
            "fallbackReason": "invalidMonitorAlternateCoordinate",
        }

    trace: Dict[str, Any] = {
        "applied": False,
        "predictionModel": "monitorAlternateCoordinate",
        "fallbackCoordinate": dict(fallback),
    }
    monitor_prediction_model = str(
        _ci_get(detail, "alternateWaypointPredictionModel") or ""
    ).strip()
    if monitor_prediction_model and not monitor_prediction_model.casefold().startswith("0401-"):
        trace["fallbackReason"] = f"monitorSelected:{monitor_prediction_model}"
        return original_fallback, trace
    turn_rate_source = str(_ci_get(detail, "turnRateSource") or "").strip()
    if "conflict" in turn_rate_source.casefold():
        trace["fallbackReason"] = "turnDirectionConflict"
        return original_fallback, trace
    course_yaw_error = _to_float(_ci_get(detail, "courseYawErrorDeg"))
    if (
        course_yaw_error is not None
        and math.isfinite(course_yaw_error)
        and abs(float(course_yaw_error)) > _PATHDEV_ATTITUDE_MAX_COURSE_YAW_ERROR_DEG
    ):
        trace["fallbackReason"] = "courseYawMismatch"
        trace["courseYawErrorDeg"] = float(course_yaw_error)
        return original_fallback, trace
    legacy_project_fn = projector if callable(projector) else _shared_project_attitude_motion
    dynamic_project_fn = _shared_predict_turn_motion if not callable(projector) else None
    if not callable(dynamic_project_fn) and not callable(legacy_project_fn):
        trace["fallbackReason"] = "attitudeKinematicsUnavailable"
        return original_fallback, trace

    trigger_age_s = _detail_age_seconds(
        detail,
        now_epoch_s=now_epoch_s,
        now_since_2000_ms=now_since_2000_ms,
    )
    trigger_state = _motion_state_from_trigger_detail(detail)
    snapshot_age_s = _snapshot_age_seconds(snapshot_payload, now_epoch_s=now_epoch_s)
    snapshot_motion_state = None
    if snapshot_age_s is not None and snapshot_age_s <= _PATHDEV_ATTITUDE_MAX_SNAPSHOT_AGE_S:
        snapshot_motion_state = _motion_state_from_snapshot(snapshot_state)
    if trigger_age_s is not None:
        trace["triggerAgeS"] = float(trigger_age_s)
    if snapshot_age_s is not None:
        trace["snapshotAgeS"] = float(snapshot_age_s)

    trigger_is_fresh = trigger_state is not None and (
        trigger_age_s is None or trigger_age_s <= _PATHDEV_ATTITUDE_MAX_TRIGGER_AGE_S
    )
    motion_state: Optional[Dict[str, Any]] = None
    if trigger_is_fresh:
        motion_state = trigger_state
        if (
            snapshot_motion_state is not None
            and trigger_age_s is not None
            and snapshot_age_s is not None
            and float(snapshot_age_s) + 0.05 < float(trigger_age_s)
        ):
            motion_state = snapshot_motion_state
    elif snapshot_motion_state is not None:
        motion_state = snapshot_motion_state

    if motion_state is None:
        if trigger_state is not None and trigger_age_s is not None:
            trace["fallbackReason"] = "staleTriggerAndNoFreshSnapshot"
        elif snapshot_payload is not None and snapshot_age_s is None:
            trace["fallbackReason"] = "snapshotFreshnessUnknown"
        else:
            trace["fallbackReason"] = "missingOrStaleAttitudeMotionState"
        return original_fallback, trace

    if (
        motion_state is snapshot_motion_state
        and trigger_state is not None
        and not motion_state.get("turnDynamics")
        and trigger_state.get("turnDynamics")
    ):
        motion_state = dict(motion_state)
        motion_state["turnDynamics"] = dict(trigger_state["turnDynamics"])
        motion_state["turnDynamicsSource"] = "monitoring0902Trigger"

    coordinate = dict(motion_state["coordinate"])
    velocity = dict(motion_state["velocity"])
    attitude = dict(motion_state["attitude"])
    turn_dynamics = dict(motion_state.get("turnDynamics") or {})
    roll_rate_dps = _to_float(turn_dynamics.get("rollRateDegS"))
    target_roll_deg = _to_float(turn_dynamics.get("targetRollDeg"))
    if (
        abs(float(attitude["roll"])) < 1.5
        and (target_roll_deg is None or abs(float(target_roll_deg)) < 1.5)
        and (roll_rate_dps is None or abs(float(roll_rate_dps)) < 0.2)
    ):
        trace["fallbackReason"] = "insufficientBankForTurnPrediction"
        return original_fallback, trace
    lead_s, lead_source, requested_lead_s = _bounded_attitude_prediction_lead(
        detail,
        source_coordinate=coordinate,
        fallback_coordinate=fallback,
        speed_mps=float(velocity["speed"]),
    )
    prediction_meta: Dict[str, Any] = {}
    try:
        if callable(dynamic_project_fn):
            prediction = dynamic_project_fn(
                coordinate,
                ...
                performance=_turn_performance_from_payload(turn_dynamics),
                integration_step_s=0.1,
            )
            projected_raw = getattr(prediction, "coordinate", None)
            if prediction is not None:
                prediction_meta = {
                    "predictionModel": str(
                        getattr(prediction, "model", "coordinated-turn")
                    ),
                    "predictedHeadingDeg": _to_float(
                        getattr(prediction, "heading_deg", None)
                    ),
                    "predictedRollDeg": _to_float(
                        getattr(prediction, "roll_deg", None)
                    ),
                    "averageRollDeg": _to_float(
                        getattr(prediction, "average_roll_deg", None)
                    ),
                    "predictedTurnRateNavDegS": _to_float(
                        getattr(prediction, "turn_rate_nav_dps", None)
                    ),
                    "predictedEffectiveRadiusM": _to_float(
                        getattr(prediction, "effective_radius_m", None)
                    ),
                }
        else:
            projected_raw = legacy_project_fn(
                coordinate,
                float(velocity["speed"]),
                float(velocity["heading"]),
                float(attitude["roll"]),
            ...
            prediction_meta = {"predictionModel": "constant-bank-constant-pitch"}
    except Exception as exc:
        trace["fallbackReason"] = f"projectionFailed:{type(exc).__name__}"
        return original_fallback, trace

    projected = _normalize_trigger_coordinate(projected_raw)
    if projected is None:
        trace["fallbackReason"] = "projectionReturnedInvalidCoordinate"
        return original_fallback, trace
    if "altitude" not in projected:
        fallback_altitude = _to_float(fallback.get("altitude"))
        if fallback_altitude is not None:
            projected["altitude"] = float(fallback_altitude)

    projected_distance_m = _coord_distance_m(coordinate, projected)
    max_distance_m = max(100.0, float(velocity["speed"]) * float(lead_s) * 1.5 + 50.0)
    if projected_distance_m is None or projected_distance_m > max_distance_m:
        trace.update(
            {
                "fallbackReason": "projectionOutsideMotionEnvelope",
                "projectedDistanceM": projected_distance_m,
                "maximumProjectionDistanceM": float(max_distance_m),
            }
        )
        return original_fallback, trace
    ...
    trace.update(
        {
            "applied": True,
            "predictionModel": prediction_meta.get(
                "predictionModel",
                "constant-bank-coordinated-turn",
            ),
            "motionStateSource": str(motion_state["source"]),
            "leadTimeS": float(lead_s),
            "leadTimeSource": str(lead_source),
            "requestedLeadTimeS": float(requested_lead_s) if requested_lead_s is not None else None,
            "sourceCoordinate": coordinate,
            "projectedCoordinate": dict(projected),
            "projectedDistanceM": float(projected_distance_m),
            "velocity": {
                "speed": float(velocity["speed"]),
                "heading": float(velocity["heading"]),
            },
            "attitude": attitude,
            "turnDynamics": turn_dynamics,
            "turnDynamicsSource": motion_state.get("turnDynamicsSource")
            or motion_state.get("source"),
        }
    )
    for key, value in prediction_meta.items():
        if key != "predictionModel" and value is not None:
            trace[key] = value
    trace.pop("fallbackReason", None)
    return dict(projected), trace
...
def _extract_snapshot_aircraft_states(snapshot_payload: Dict[str, Any] | None) -> Dict[int, Dict[str, Any]]:
    ...
    agent_states = _ci_get(snapshot_payload, "agent_states", "agentStateList") or []
    for entry in agent_states:
        if not isinstance(entry, dict):
            continue
        aircraft_id = _to_int(_ci_get(entry, "aircraftID", "aircraftId"))
        ...
        if waypoint_id is not None and waypoint_id > 0:
            state["currentWaypointID"] = int(waypoint_id)
        unmanned_info = _ci_get(entry, "unmannedInfo")
        if not isinstance(unmanned_info, dict):
            unmanned_info = {}
        flight_mode = _to_int(
            _ci_get(unmanned_info, "flightMode")
            if _ci_get(unmanned_info, "flightMode") is not None
            else _ci_get(entry, "flightMode")
        )
        if flight_mode is not None:
            state["flightMode"] = int(flight_mode)
        on_mission = _to_int(
            _ci_get(unmanned_info, "onMission")
            if _ci_get(unmanned_info, "onMission") is not None
            else _ci_get(entry, "onMission")
        )
        if on_mission is not None:
            state["onMission"] = int(on_mission)
        coord = _normalize_trigger_coordinate(_ci_get(entry, "coordinate"))
        if coord is None:
            coord = _normalize_trigger_coordinate(_ci_get(unmanned_info, "coordinate"))
        if coord is not None:
            state["coordinate"] = coord
        loiter_coord = _normalize_trigger_coordinate(_ci_get(unmanned_info, "loiterCoordinate"))
        if loiter_coord is not None:
            state["loiterCoordinate"] = loiter_coord
        velocity_payload = _ci_get(entry, "velocity")
        velocity = velocity_payload if isinstance(velocity_payload, dict) else {}
        unmanned_velocity = (
            _ci_get(unmanned_info, "velocity")
            if isinstance(_ci_get(unmanned_info, "velocity"), dict)
            else {}
        )
        if velocity:
        ...
        attitude_payload = _ci_get(entry, "attitude")
        if not isinstance(attitude_payload, dict):
            attitude_payload = _ci_get(unmanned_info, "attitude")
        attitude = _normalize_attitude_payload(attitude_payload)
        if attitude is not None:
            state["attitude"] = attitude
            state["rollDeg"] = float(attitude["roll"])
            state["pitchDeg"] = float(attitude["pitch"])
            state["yawDeg"] = float(attitude["yaw"])

        heading = _to_float(_ci_get(velocity, "heading", "headingDeg")) if velocity else None
        if heading is None:
            heading = _to_float(_ci_get(unmanned_velocity, "heading", "headingDeg")) if unmanned_velocity else None
        if heading is None:
            heading = _to_float(_ci_get(entry, "headingDeg", "heading"))
        if heading is None:
            heading = _to_float(_ci_get(entry, "Heading"))
        if heading is None and attitude is not None:
            heading = float(attitude["yaw"])
        if heading is not None:
            state["heading"] = float(heading) % 360.0
            state["headingDeg"] = float(heading) % 360.0

        speed = _to_float(_ci_get(entry, "speedMps", "speed"))
        if speed is None:
            speed = _to_float(_ci_get(velocity, "speed", "speedMps")) if velocity else None
        if speed is None:
            speed = _to_float(_ci_get(unmanned_velocity, "speed", "speedMps")) if unmanned_velocity else None
        ...
        for key in (
            ...
            if value is None and velocity:
                value = _ci_get(velocity, key)
            if value is None and unmanned_velocity:
                value = _ci_get(unmanned_velocity, key)
...
def _anchor_first_waypoint_to_alternate_coordinate(
    ...
    *,
    ...
    output_coord = _pathdev_coordinate_for_output(entry_coord)
    waypoint_id: Optional[int] = None
    distance_before_m: Optional[float] = None
    updated_objects: Set[int] = set()
    did_update = False
    for key in ("waypointList", "lahWaypointList", "uavWaypointList"):
        waypoints = path_payload.get(key)
        if not isinstance(waypoints, list) or not waypoints or not isinstance(waypoints[0], dict):
            continue
        first_waypoint = waypoints[0]
        object_id = id(first_waypoint)
        if object_id in updated_objects:
            continue
        updated_objects.add(object_id)

        original_coord = _normalize_coordinate(first_waypoint.get("coordinate"))
        if original_coord is not None and distance_before_m is None:
            distance_before_m = _coordinate_distance_m(original_coord, entry_coord)

        coordinate = dict(first_waypoint.get("coordinate") or {})
        coordinate["latitude"] = float(output_coord["latitude"])
        coordinate["longitude"] = float(output_coord["longitude"])
        altitude = _normalize_altitude_value(output_coord.get("altitude"))
        if altitude is not None:
            coordinate["altitude"] = int(altitude)
        first_waypoint["coordinate"] = coordinate
        first_waypoint["isDone"] = False

        if waypoint_id is None:
            waypoint_id = _to_int(first_waypoint.get("waypointID"))
        did_update = True

    if did_update:
        distance_text = (
            f", distanceBefore={float(distance_before_m):.1f}m"
            if distance_before_m is not None
            else ""
        )
        emit(
            f"{log_prefix} anchored generated entry waypoint to alternate coordinate "
            f"(aircraft={int(aircraft_id)}, path={int(path_id)}, "
            f"waypoint={int(waypoint_id) if waypoint_id is not None else '-'}{distance_text})."
        )
    return path_payload
...
def _resolve_plan_artifacts_from_explicit_detail(
    *,
    ...
    for idx, waypoint_id in enumerate(waypoint_ids):
...
def _build_other_uav_current_package(
    *,
    ...
    if _to_int(on_mission) == 2 and _mission_has_imaging_geometry(target_mission, source_waypoints):
        ...
        write_json_batch(
            [(imp_dest, updated_imp), (fp_dest, hold_fp)],
            pretty=True,
            ensure_ascii=False,
...
def _build_replanned_waypoints(
    ...
    *,
    ...
    current_waypoint = waypoint_list[current_index]
...
def run_path_deviation_replan_pipeline(
    ...
    *,
    ...
    source_has_imaging_geometry = _mission_has_imaging_geometry(source_mission, source_waypoints)
    current_waypoint_has_filming = _current_waypoint_has_filming_property(
        source_waypoints,
        current_waypoint_id,
    )
    ...
    monitor_alternate_coordinate = dict(alternate_coordinate)
    alternate_coordinate, pathdev_prediction = _refine_alternate_coordinate_with_attitude(
        detail,
        fallback_coordinate=monitor_alternate_coordinate,
        snapshot_payload=snapshot_payload,
        snapshot_state=trigger_aircraft_state,
        now_since_2000_ms=int(now_ms),
    )
    detail = dict(detail)
    detail["missionPlanningPrediction"] = deepcopy(pathdev_prediction)
    detail["monitorAlternateWaypointCoordinate"] = dict(monitor_alternate_coordinate)
    detail["plannerAdjustedAlternateWaypointCoordinate"] = dict(alternate_coordinate)
    if bool(pathdev_prediction.get("applied")):
        prediction_coordinate = _normalize_trigger_coordinate(
            pathdev_prediction.get("sourceCoordinate")
        )
        prediction_velocity = _normalize_pathdev_velocity(
            pathdev_prediction.get("velocity")
        )
        prediction_attitude = _normalize_attitude_payload(
            pathdev_prediction.get("attitude")
        )
        enriched_aircraft_state = dict(trigger_aircraft_state)
        if prediction_coordinate is not None:
            enriched_aircraft_state["coordinate"] = dict(prediction_coordinate)
            enriched_aircraft_state["currentCoordinate"] = dict(prediction_coordinate)
        if prediction_velocity:
            enriched_aircraft_state["velocity"] = dict(prediction_velocity)
            if "speed" in prediction_velocity:
                enriched_aircraft_state["speedMps"] = float(prediction_velocity["speed"])
            if "heading" in prediction_velocity:
                enriched_aircraft_state["heading"] = float(prediction_velocity["heading"])
                enriched_aircraft_state["headingDeg"] = float(prediction_velocity["heading"])
        if prediction_attitude is not None:
            enriched_aircraft_state["attitude"] = dict(prediction_attitude)
        trigger_aircraft_state = enriched_aircraft_state
        snapshot_aircraft_states[int(aircraft_id)] = dict(enriched_aircraft_state)
        if fallback_aircraft_altitude is None:
            fallback_aircraft_altitude = _normalize_altitude_value(
                (prediction_coordinate or {}).get("altitude")
            )
        attitude_used = pathdev_prediction.get("attitude") or {}
        emit(
            "[PATHDEV][ATTITUDE] alternate waypoint refined with shared turn-dynamics prediction "
            f"(source={pathdev_prediction.get('motionStateSource')}, "
            f"model={pathdev_prediction.get('predictionModel')}, "
            f"lead={float(pathdev_prediction.get('leadTimeS') or 0.0):.2f}s, "
            f"roll={float(attitude_used.get('roll') or 0.0):.2f}deg, "
            f"pitch={float(attitude_used.get('pitch') or 0.0):.2f}deg, "
            f"yaw={float(attitude_used.get('yaw') or 0.0):.2f}deg)."
        )
    else:
        emit(
            "[PATHDEV][ATTITUDE] monitor alternate waypoint retained "
            f"(reason={pathdev_prediction.get('fallbackReason') or 'predictionNotApplied'})."
        )
    ...
    if (
        ...
        and current_input_id > 0
        and (source_has_imaging_geometry or current_waypoint_has_filming)
    ):
        emit(
            "[PATHDEV][CURRENT] filming/sweep current mission accepted for path-deviation replan "
            f"(aircraft={int(aircraft_id)}, inputMissionID={int(current_input_id)}, "
            f"mission={int(artifacts.individual_mission_id)}, path={int(artifacts.path_id)}, "
            f"currentWP={int(current_waypoint_id)}, "
            f"missionImaging={bool(source_has_imaging_geometry)}, "
            f"currentWpFilming={bool(current_waypoint_has_filming)})."
        )
        entry_coord = _normalize_coordinate(alternate_coordinate)
        if entry_coord is None:
            ...
            def _pathdev_current_flight_path_transform(
                owner_aircraft_id: int,
                path_id: int,
                path_payload: Dict[str, Any],
            ) -> Dict[str, Any]:
                if int(owner_aircraft_id) != int(aircraft_id):
                    return path_payload
                return _anchor_first_waypoint_to_alternate_coordinate(
                    path_payload,
            ...
            try:
                collaborative_resume = _prepare_uav_collaborative_resume_replan(
                    ...
                    flight_path_transform=_pathdev_current_flight_path_transform,
        ...
        if int(aircraft_id) not in updated_aircraft_ids:
            ...
            if new_individual_id <= 0 or new_path_id <= 0 or inserted_waypoint_id is None:
                ...
                log_payload = {
                    ...
                    "pathDeviationPrediction": deepcopy(pathdev_prediction),
