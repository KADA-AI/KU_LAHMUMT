...
_DEFAULT_MIN_REMAINING_ETA_S = 30
_DEFAULT_REJOIN_MARGIN_S = 15
...
def _active_tracking_remains_for_plan_lineage(current_plan_id: Optional[int]) -> bool:
    ...
    for assignment in list_active_tracking_assignments():
        if not isinstance(assignment, dict) or not bool(assignment.get("active")):
            continue
        if _tracking_assignment_matches_plan_lineage(assignment, plan_lineage):
            return True
    return False
...
def _tracking_assignment_matches_plan_lineage(
    ...
    for key in ("attack_plan_id", "source_plan_id"):
        plan_id = _to_int(assignment.get(key))
        if plan_id is not None and int(plan_id) in plan_lineage:
            return True
    return False
...
def run_post_attack_rejoin_pipeline(
    ...
    *,
    ...
    run_cache = _PostAttackRunCache(
        mission_plans={int(current_plan_id): deepcopy(plan_data)},
        imp_by_aircraft={},
        flight_paths={},
        ...
        if not bool(evaluation.get("replan_needed")):
            ...
            if active_suffix_candidate_ids:
                ...
                for aircraft_id in sorted(active_suffix_candidate_ids):
                    ...
                    line_remaining_detail = load_line_scan_aircraft_remaining_detail(
                        source_plan_id=int(current_plan_id),
...
def _match_tracking_assignments(
    *,
    ...
    for assignment in list_active_tracking_assignments():
        if not isinstance(assignment, dict) or not bool(assignment.get("active")):
            continue
        if not _tracking_assignment_matches_plan_lineage(assignment, current_plan_lineage):
            continue
...
def _predict_post_attack_position_along_current_path(
    *,
    ...
    if artifacts is None:
        return None
    fp_data = _load_path_payload(getattr(artifacts, "path_id", None), run_cache=run_cache)
    ...
    *,
    ...
    def _post_attack_flight_path_transform(
        ...
        transformed = _drop_post_attack_collab_leading_prefix_waypoints(
            ...
            preserve_short_single_sweep_prefix_if_far=int(aircraft_id) in returning_aircraft_ids,
        )
...
def _drop_post_attack_collab_leading_prefix_waypoints(
    ...
    *,
    ...
    preserve_short_single_sweep_prefix_if_far: bool = False,
) -> Dict[str, Any]:
    ...
    if (
        preserve_short_single_sweep_prefix_if_far
        and _should_preserve_short_single_sweep_prefix(
            removed=removed,
            remaining=trimmed,
            reference_coord=reference_coord,
        )
    ):
        ref_coord = _normalize_coordinate(reference_coord)
        first_sweep_coord = _first_post_attack_sweep_coordinate(trimmed[0] if trimmed else None)
        distance_m = _coord_distance_m(ref_coord, first_sweep_coord)
        sep_m = _post_attack_single_sweep_sep_m(trimmed[0] if trimmed else None)
        removed_ids = [
            int(wp_id)
        ...
        emit(
            "[POSTATTACK][COLLAB] Leading entry waypoint prefix preserved for short single-sweep return "
            f"(aircraft={int(aircraft_id)}, pathID={int(path_id)}, "
            f"removedWaypointIDs={removed_ids}, distance={float(distance_m or 0.0):.1f}m, "
            f"sep={float(sep_m or 0.0):.1f}m)."
        )
        return payload
...
def _should_preserve_short_single_sweep_prefix(
    *,
    removed: List[Dict[str, Any]],
    remaining: List[Dict[str, Any]],
    reference_coord: Dict[str, Any] | None,
) -> bool:
    if not removed or not isinstance(remaining, list) or len(remaining) != 1:
        return False
    first_remaining = remaining[0] if isinstance(remaining[0], dict) else None
    if not _is_post_attack_collab_sweep_waypoint(first_remaining or {}):
        return False
    ref_coord = _normalize_coordinate(reference_coord)
    first_sweep_coord = _first_post_attack_sweep_coordinate(first_remaining)
    if ref_coord is None or first_sweep_coord is None:
        return False
    distance_m = _coord_distance_m(ref_coord, first_sweep_coord)
    sep_m = _post_attack_single_sweep_sep_m(first_remaining)
    return (
        distance_m is not None
        and sep_m is not None
        and float(distance_m) > float(sep_m)
    )


def _first_post_attack_sweep_coordinate(waypoint: Dict[str, Any] | None) -> Optional[Dict[str, Any]]:
    if not isinstance(waypoint, dict):
        return None
    filming = waypoint.get("filmingProperty")
    filming = filming if isinstance(filming, dict) else {}
...
def _post_attack_single_sweep_sep_m(waypoint: Dict[str, Any] | None) -> Optional[float]:
    candidates: List[Any] = []
    if isinstance(waypoint, dict):
        ...
        if isinstance(line_search, dict):
            candidates.extend(
                line_search.get(key)
                for key in ("SEP", "sep", "separation", "separation_m", "separationM")
            )
        candidates.extend(
            filming.get(key)
            for key in ("SEP", "sep", "separation", "separation_m", "separationM")
    ...
    for value in candidates:
        ...
        if parsed is not None and parsed > 0.0:
            return float(parsed)
    try:
        fallback = get_runtime_float("default_sweep_separation_m", 1000.0)
    except Exception:
        fallback = 1000.0
    parsed_fallback = _to_float(fallback)
    if parsed_fallback is None or parsed_fallback <= 0.0:
        return None
    return float(parsed_fallback)
    ...
    for path_id, payload in (prepared.generated_fp_by_path or {}).items():
        ...
        if owner_aircraft_id is not None:
            path_payload = _drop_post_attack_collab_leading_prefix_waypoints(
                ...
                preserve_short_single_sweep_prefix_if_far=int(owner_aircraft_id) in returning_aircraft_ids,
            )
...
def _estimate_aircraft_remaining_eta_s(
    *,
    ...
    current_path_eta_s = 0
    fp_data = _load_path_payload(int(artifacts.path_id), run_cache=run_cache)
    if not isinstance(fp_data, dict):
        current_path_eta_s = 0
    else:
...
def _estimate_path_total_eta_s(
    ...
    *,
    ...
    fp_data = _load_path_payload(int(path_id), run_cache=run_cache)
    if not isinstance(fp_data, dict):
        return 0
...
def _active_current_input_path_all_done(
    *,
    ...
    for mission in imp_data.get("individualMissionList") or []:
        ...
        if _extract_related_input_mission_id(mission) != int(current_input_id):
            continue
        path_payload = _load_path_payload(_to_int(mission.get("pathID")), run_cache=run_cache)
...
def _area_rows_union_area_degrees(area_rows: List[Dict[str, Any]]) -> float:
    try:
        ...
        return max(0.0, float(getattr(geometry, "area", 0.0) or 0.0))
    except Exception:
        return 0.0


def _ownership_projected_remaining_detail(
    entry: Dict[str, Any],
    mission_detail: Dict[str, Any],
) -> Dict[str, Any]:
    ownership_details = entry.get("areaOwnershipDetails")
    if not isinstance(ownership_details, list):
        return {}
    ...
    for owner in ownership_details:
        if not isinstance(owner, dict):
            continue
        owner_detail = owner.get("remainingDetail")
        if not isinstance(owner_detail, dict):
            continue
        for row in _area_rows_from_detail(owner_detail):
            coords = row.get("coordinateList") if isinstance(row, dict) else None
            if not isinstance(coords, list) or len(coords) < 3:
                continue
    ...
    mission_rows = _area_rows_from_detail(mission_detail)
    mission_area = _area_rows_union_area_degrees(mission_rows)
    projected_area = _area_rows_union_area_degrees(projected_rows)
    if _remaining_detail_has_geometry(mission_detail) and (
        mission_area <= 0.0
        or projected_area <= 0.0
        or mission_area <= projected_area * 1.18
    ):
        return {}
    ...
    coordinate_list = deepcopy(
        next(
            (
                ...
                if not bool(row.get("isHole")) and isinstance(row.get("coordinateList"), list)
            ),
            [],
        )
    )
...
def _remaining_snapshot_area_detail(
    *,
    ...
    entry_ready = mission_area_replan_store.snapshot_entry_ready_for_replan(entry)
    detail = entry.get("remainingDetail")
    projected_detail = _ownership_projected_remaining_detail(
        entry,
        detail if isinstance(detail, dict) else {},
    )
    if projected_detail:
        return projected_detail
    ...
    if reject_reason and not entry_ready:
        mission_area_replan_store.audit_snapshot_entry_rejected(
            entry,
            requested_mission_plan_id=int(source_plan_id),
            snapshot_mission_plan_id=_to_int(snapshot_info.get("snapshotMissionPlanID")),
