...
_ATTACK_POINT_META_KEYS = (
    ...
    "battle_area_constrained",
    "battle_area_adjustment_m",
    "battle_area_source_coordinate",
    "battle_area_coordinate_count",
    "battle_area_target_distance_m",
)
...
def _lah_special_battle_area_for_input(
    ctx: Dict[str, Any],
    *,
    aircraft_id: int,
    input_mission_id: object | None,
    ...
    if isinstance(profile, dict):
        raw_coords = profile.get("battlePositionAreaCoordinateList") or []
...
def _coord_in_polygon_lonlat(coord: Dict[str, Any], polygon: List[Dict[str, Any]]) -> bool:
    ...
    for current in polygon:
        ...
        if _coord_on_segment_lonlat(x, y, xi, yi, xj, yj):
            return True
        if (yi > y) != (yj > y):
...
def _battle_area_reference(polygon: List[Dict[str, Any]], coord: Dict[str, Any]) -> Tuple[float, float]:
    lat_values = [float(row["latitude"]) for row in polygon]
    lon_values = [float(row["longitude"]) for row in polygon]
    lat_values.append(float(coord["latitude"]))
    lon_values.append(float(coord["longitude"]))
    return (sum(lat_values) / len(lat_values), sum(lon_values) / len(lon_values))


def _coord_to_local_xy(coord: Dict[str, Any], lat0: float, lon0: float) -> Tuple[float, float]:
    radius_m = 6_371_000.0
    x = math.radians(float(coord["longitude"]) - float(lon0)) * radius_m * math.cos(math.radians(lat0))
    y = math.radians(float(coord["latitude"]) - float(lat0)) * radius_m
    return (float(x), float(y))


def _local_xy_to_coord(x: float, y: float, lat0: float, lon0: float, altitude: Any) -> Dict[str, Any]:
    radius_m = 6_371_000.0
    lat = float(lat0) + math.degrees(float(y) / radius_m)
    lon = float(lon0) + math.degrees(float(x) / (radius_m * math.cos(math.radians(lat0))))
    return {
        "latitude": float(lat),
        "longitude": float(lon),
        "altitude": _normalize_altitude_value(altitude),
    }


def _closest_local_point_on_segment(
    px: float,
    py: float,
    ax: float,
    ay: float,
    ...
    if length_sq <= 1e-9:
        dist_sq = (px - ax) * (px - ax) + (py - ay) * (py - ay)
        return (float(ax), float(ay), float(dist_sq))
    t = ((px - ax) * dx + (py - ay) * dy) / length_sq
    t = max(0.0, min(1.0, float(t)))
    cx = float(ax) + t * dx
    cy = float(ay) + t * dy
    dist_sq = (px - cx) * (px - cx) + (py - cy) * (py - cy)
    return (float(cx), float(cy), float(dist_sq))


def _centroid_coordinate(coords: List[Dict[str, Any]], altitude: Any = None) -> Optional[Dict[str, Any]]:
    if not coords:
        return None
    lat_values = [float(row["latitude"]) for row in coords]
    lon_values = [float(row["longitude"]) for row in coords]
    return {
        "latitude": sum(lat_values) / len(lat_values),
        "longitude": sum(lon_values) / len(lon_values),
        "altitude": _normalize_altitude_value(altitude),
    }


def _battle_area_internal_fallback_coordinate(
    polygon: List[Dict[str, Any]],
    *,
    altitude: Any = None,
    preferred_coord: Optional[Dict[str, Any]] = None,
) -> Optional[Dict[str, Any]]:
    if len(polygon) < 3:
        return None
    reference = preferred_coord or polygon[0]
    lat0, lon0 = _battle_area_reference(polygon, reference)
    local_polygon = [_coord_to_local_xy(row, lat0, lon0) for row in polygon]
    min_x = min(x for x, _ in local_polygon)
    max_x = max(x for x, _ in local_polygon)
    min_y = min(y for _, y in local_polygon)
    max_y = max(y for _, y in local_polygon)
    if max_x <= min_x or max_y <= min_y:
        return None

    preferred_xy: Optional[Tuple[float, float]] = None
    if preferred_coord is not None:
        preferred_xy = _coord_to_local_xy(preferred_coord, lat0, lon0)
    else:
        centroid = _centroid_coordinate(polygon, altitude)
        if centroid is not None:
            preferred_xy = _coord_to_local_xy(centroid, lat0, lon0)

    best_coord: Optional[Dict[str, Any]] = None
    best_score = float("inf")
    for steps in (5, 9, 17):
        step_x = (max_x - min_x) / float(steps)
        step_y = (max_y - min_y) / float(steps)
        for ix in range(steps):
            for iy in range(steps):
                x = min_x + (float(ix) + 0.5) * step_x
                y = min_y + (float(iy) + 0.5) * step_y
                coord = _local_xy_to_coord(x, y, lat0, lon0, altitude)
                if not _coord_in_polygon_lonlat(coord, polygon):
                    continue
                if preferred_xy is not None:
                    score = (x - preferred_xy[0]) * (x - preferred_xy[0]) + (y - preferred_xy[1]) * (y - preferred_xy[1])
                else:
                    score = 0.0
                if score < best_score:
                    best_score = score
    ...
    centroid = _centroid_coordinate(polygon, altitude)
    if centroid is not None and _coord_in_polygon_lonlat(centroid, polygon):
        return centroid
    return None


def _nearest_battle_area_coordinate(
    desired_coord: Dict[str, Any],
    polygon: List[Dict[str, Any]],
) -> Optional[Dict[str, Any]]:
    desired = _normalize_coordinate(desired_coord)
    if desired is None or len(polygon) < 3:
        return None
    if _coord_in_polygon_lonlat(desired, polygon):
        return dict(desired)

    lat0, lon0 = _battle_area_reference(polygon, desired)
    px, py = _coord_to_local_xy(desired, lat0, lon0)
    local_polygon = [_coord_to_local_xy(row, lat0, lon0) for row in polygon]
    best: Optional[Tuple[float, float, float]] = None
    previous = local_polygon[-1]
    for current in local_polygon:
        candidate = _closest_local_point_on_segment(px, py, previous[0], previous[1], current[0], current[1])
    ...
    edge_x, edge_y, _ = best
    centroid_coord = _centroid_coordinate(polygon, desired.get("altitude"))
    if centroid_coord is None:
        return _local_xy_to_coord(edge_x, edge_y, lat0, lon0, desired.get("altitude"))
    centroid_x, centroid_y = _coord_to_local_xy(centroid_coord, lat0, lon0)
    fallback_coord = _local_xy_to_coord(edge_x, edge_y, lat0, lon0, desired.get("altitude"))

    for fraction in (0.01, 0.03, 0.06, 0.1, 0.2, 0.4, 0.6, 1.0):
        cx = edge_x + (centroid_x - edge_x) * float(fraction)
        cy = edge_y + (centroid_y - edge_y) * float(fraction)
        candidate_coord = _local_xy_to_coord(cx, cy, lat0, lon0, desired.get("altitude"))
        if _coord_in_polygon_lonlat(candidate_coord, polygon):
            return candidate_coord
    if _coord_in_polygon_lonlat(centroid_coord, polygon):
        return centroid_coord
    fallback_inside = _battle_area_internal_fallback_coordinate(
        polygon,
        altitude=desired.get("altitude"),
        preferred_coord=desired,
    )
    return fallback_inside or fallback_coord


def _select_lah_battle_area_attack_coordinate(
    ctx: Dict[str, Any],
    target_coord: Dict[str, Any],
    *,
    aircraft_id: int,
    input_mission_id: object | None,
    altitude_source: Optional[Dict[str, Any]] = None,
) -> Optional[Dict[str, Any]]:
    polygon = _lah_special_battle_area_for_input(
        ctx,
    ...
    altitude = (
        _normalize_altitude_value((altitude_source or {}).get("altitude"))
        or _normalize_altitude_value((target or {}).get("altitude"))
        or _normalize_altitude_value((_lah_special_battle_anchor_for_input(
            ctx,
            aircraft_id=int(aircraft_id),
            input_mission_id=input_mission_id,
        ) or {}).get("altitude"))
    ...
    if polygon:
        desired = dict(target) if target is not None else _centroid_coordinate(polygon, altitude)
        if desired is None:
            desired = _battle_area_internal_fallback_coordinate(polygon, altitude=altitude)
        if desired is not None:
            desired["altitude"] = altitude
            selected = _nearest_battle_area_coordinate(desired, polygon)
            if selected is None:
                selected = _battle_area_internal_fallback_coordinate(
                    polygon,
                    altitude=altitude,
                    preferred_coord=desired,
                )
            if selected is not None:
                ...
                selected["selection_mode"] = "battle_position_area"
                selected["battle_area_constrained"] = True
                selected["battle_area_coordinate_count"] = len(polygon)
                if target is not None:
                    distance_m = _haversine_distance_m(target, selected)
                    if distance_m is not None:
                        selected["battle_area_target_distance_m"] = float(distance_m)
                return selected

    anchor = _lah_special_battle_anchor_for_input(
        ctx,
        aircraft_id=int(aircraft_id),
        input_mission_id=input_mission_id,
    )
    if anchor is not None:
        ...
        selected["selection_mode"] = "battle_position_anchor"
        selected["battle_area_constrained"] = True
        return selected
    return None


def _context_battle_attack_input_id(ctx: Dict[str, Any], detail: Dict[str, Any]) -> Optional[int]:
    for value in (
        detail.get("currentInputMissionID"),
        detail.get("targetInputMissionID"),
        (ctx.get("mission_ids") or [None])[0] if isinstance(ctx.get("mission_ids"), list) else None,
    ):
...
def _constrain_lah_attack_coord_to_battle_area(
    ctx: Dict[str, Any],
    ...
    *,
    ...
    normalized = _normalize_coordinate(attack_coord)
    if normalized is None:
        return attack_coord, False
    polygon = _lah_special_battle_area_for_input(
        ctx,
        aircraft_id=int(aircraft_id),
        input_mission_id=input_mission_id,
    ...
    if not polygon:
        return dict(attack_coord), False
    if _coord_in_polygon_lonlat(normalized, polygon):
        return dict(attack_coord), False
    constrained = _nearest_battle_area_coordinate(normalized, polygon)
    if constrained is None:
        return dict(attack_coord), False

    result = dict(attack_coord)
    result.update(constrained)
    result["battle_area_constrained"] = True
    result["battle_area_coordinate_count"] = len(polygon)
    result["battle_area_source_coordinate"] = dict(normalized)
    adjustment_m = _haversine_distance_m(normalized, constrained)
    if adjustment_m is not None:
        result["battle_area_adjustment_m"] = float(adjustment_m)
    source_mode = _attack_point_selection_mode(attack_coord)
    if source_mode:
        result["selection_mode"] = f"{source_mode}_battle_position_area"
    else:
        result["selection_mode"] = "battle_position_area"
    return result, True
...
def _compute_attack_point_inprocess(
    ...
    *,
    ...
    try:
        ...
        raster_paths = assist.detect_raster_paths(str(raster_root) if raster_root.exists() else None)
    ...
    *,
    ...
    for aircraft_id, sequence in manned_sequences.items():
        ...
        for seq_index, sequence_target in enumerate(sequence):
            ...
            battle_input_id = _context_battle_attack_input_id(ctx, detail)
            battle_area_attack_coord = _select_lah_battle_area_attack_coordinate(
                ctx,
                target_coord,
                aircraft_id=int(aircraft_id),
                input_mission_id=battle_input_id,
                altitude_source=previous_attack_coord or aircraft_coord,
            )
            ...
            if special_lah_attack_coord is not None and int(aircraft_id) in (1, 2, 3):
                emit(
                    "[ATTACK][LAH] special battle-position area will be phase-checked "
                    f"(aircraft={int(aircraft_id)}, target={sequence_target.get('target_id')})"
                )
            if battle_area_attack_coord is not None:
                resolved_attack_coord = dict(battle_area_attack_coord)
                resolved_attack_source = battle_area_attack_coord
                use_special_target_region_attack = True
                emit(
                    "[ATTACK][POINT] Battle-position area point selected "
                    f"(aircraft={int(aircraft_id)}, target={sequence_target.get('target_id')}, "
                    f"targetDist={battle_area_attack_coord.get('battle_area_target_distance_m')}m)."
                )
            if (
...
def _build_lah_attack_sequence_package(
    *,
    ...
    if special_lah_attack_coord is not None:
        emit(
            "[ATTACK][LAH] special operation resume uses battle-position anchor; "
            "attack point preserved "
    ...
    for idx, assigned in enumerate(valid_targets):
        raw_attack_coord = assigned.get("attack_coord") or assigned.get("coordinate")
        attack_coord = _normalize_coordinate(raw_attack_coord)
        if not attack_coord:
            continue
        attack_coord, battle_area_adjusted = _constrain_lah_attack_coord_to_battle_area(
            ctx,
            raw_attack_coord if isinstance(raw_attack_coord, dict) else attack_coord,
            aircraft_id=int(aircraft_id),
            input_mission_id=input_mission_id,
        )
        if battle_area_adjusted:
            emit(
                "[ATTACK][LAH] Attack waypoint constrained inside battle-position area "
                f"(aircraft={int(aircraft_id)}, target={assigned.get('target_id') or assigned.get('targetID')}, "
                f"adjustment={float(attack_coord.get('battle_area_adjustment_m') or 0.0):.1f}m)."
            )
        _apply_lah_altitude_floor(attack_coord, current_coord)
        preserve_attack_altitude = _preserve_attack_point_altitude(assigned.get("attack_coord"))
        attack_alt = (
    ...
    *,
    ...
    if special_lah_attack_coord is not None:
        ...
        entry_coord["altitude"] = (
            _normalize_altitude_value(special_lah_attack_coord.get("altitude"))
            or _normalize_altitude_value(current_coord.get("altitude"))
            or entry_alt
        )
    ...
    attack_coord_norm, battle_area_adjusted = _constrain_lah_attack_coord_to_battle_area(
        ctx,
        attack_coord if isinstance(attack_coord, dict) else attack_coord_norm,
        aircraft_id=int(aircraft_id),
        input_mission_id=input_mission_id,
    )
    if battle_area_adjusted:
        preserve_attack_altitude = _preserve_attack_point_altitude(attack_coord_norm)
        log_target_id = _to_int(descriptor.get("target_id")) or 0
        emit(
            "[ATTACK][LAH] Attack waypoint constrained inside battle-position area "
            f"(aircraft={int(aircraft_id)}, target={log_target_id}, "
            f"adjustment={float(attack_coord_norm.get('battle_area_adjustment_m') or 0.0):.1f}m)."
        )
