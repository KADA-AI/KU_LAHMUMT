...
def _build_quality_replanned_waypoints(
    ...
    *,
    ...
    phase_mark: Callable[[str], None] | None = None,
    ...
    mark("quality_search_speed_scale")
    ...
    mark("quality_waypoint_reassign")
...
def _build_replanned_waypoints(
    ...
    *,
    ...
    if (
        ...
        if candidate_waypoint_id is None or candidate_waypoint_id <= 0:
            replacement_fallback_reason = "missing_or_invalid"
        elif candidate_waypoint_id == int(current_waypoint_id):
            replacement_fallback_reason = "same_as_current"
        else:
            replacement_fallback_reason = "conflicts_with_source_path"
        candidate_waypoint_id = None if waypoint_allocator is not None else _reserve_waypoint_block(1)
    if candidate_waypoint_id is not None:
    ...
    if replacement_fallback_reason:
        emit(
            "[IMGSCH] replacementWaypointID fallback -> local waypoint allocator "
            f"(reason={replacement_fallback_reason}, newWaypointID={new_waypoint_id})."
        )
...
def run_imaging_schedule_replan_pipeline(
    ...
    *,
    ...
    transaction_id = new_replan_transaction_id("imaging-schedule")
    initial_trigger_type = str((detail or {}).get("triggerType") or IMAGING_TRIGGER_TYPE).strip()
    phase_timer = PipelinePhaseTimer(
        pipeline="quality_speed" if initial_trigger_type == QUALITY_TRIGGER_TYPE else "imaging_schedule",
    ...
    try:
        ...
        return None
    phase_timer.mark("source_artifact_resolve_load")
    ...
    option_names = [] if is_quality_trigger else _ensure_option_names([new_plan_id], ctx.get("option_names"))
    delivery_policy = {
        "forceDirectUpdate": True,
        "suppress0702Fallback": bool(is_quality_trigger),
        "suppress0901Options": bool(is_quality_trigger),
        "optionCount": len(option_names),
    }
    if is_quality_trigger:
        emit(
            f"{log_prefix} delivery policy: forceDirectUpdate=True, "
            "suppress0702Fallback=True, 0901 options suppressed."
        )
    now_ms = _now_ms_since_2000()

    # plan_data는 read_json_cached가 반환한 호출자 소유 사본이고 이후 재사용 없음
    ...
    # imp_data 역시 호출자 소유 사본이고 이후 재사용 없음
    ...
    anchor_waypoint_id: Optional[int] = None
    trimmed_sweep_points = 0
    raw_search_speed_scale = detail.get("searchSpeedScale")
    search_speed_scale = _coerce_float(raw_search_speed_scale)
    ...
    try:
        if is_quality_trigger:
            ...
            phase_timer.mark("quality_current_coordinate_resolve")
            if raw_search_speed_scale is None or raw_search_speed_scale == "":
                search_speed_scale = 1.0
            elif search_speed_scale is None or search_speed_scale <= 0.0:
                emit(f"{log_prefix} invalid searchSpeedScale={raw_search_speed_scale!r}; value must be > 0.")
                return None
            phase_timer.mark("quality_search_speed_scale_validate")
        ...
        return None
    phase_timer.mark("waypoint_trim_replacement")
    ...
    phase_timer.mark("plan_imp_fp_build")
    ...
    validation_summary = validate_replan_payloads(
        ...
        scope=f"{trigger_type}:{new_plan_id}",
    ...
    if log_written:
        ...
        emit(f"{log_prefix} log artifact skipped by runtime artifact mode.")
    try:
        imaging_schedule_replan_store.save_event(
            "mission_pipeline_complete",
            {
                ...
                "deliveryPolicy": dict(delivery_policy),
