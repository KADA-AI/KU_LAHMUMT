...
_CANONICAL_NAME = "modules.mission_planning.engine.mission_generation.artifacts_0301_0302_0303_0304.d0302"
_ALIASES = (
...
from modules.mission_planning.MissionPlanner.data_def.mission_helpers import now_ms_since_2000
...
def build_mission_packages(
    ...
    *,
    ...
    reserved_individual_mission_ids: list[int] | tuple[int, ...] | None = None,
) -> list[dict]:
    """
    ...
    • isDone → bool 캐스팅
    • altitude(int), lat/lon(float), width(uint), isHole(bool) 강제 보정
    • patternType 기본값 보정
    """
    ...
    total_individual_mission_count = sum(len(items) for items in grouped.values())
    if total_individual_mission_count > 0:
        reserved_individual_values = [
            int(value)
            for value in (reserved_individual_mission_ids or [])
            if value is not None
        ]
        if len(reserved_individual_values) < total_individual_mission_count:
            reserved_individual_values = reserve_individual_mission_ids(total_individual_mission_count)
    else:
        reserved_individual_values = []
    reserved_individual_mission_iter = iter(reserved_individual_values)
    for aid, im_raw in grouped.items():
        ...
        if not pkg_id:
            pkg_id = int(reserve_imp_ids(1)[0])
        ...
        if invalid_path_rows:
            try:
                reserved_path_ids_for_aid = list(
                    reserve_path_id_blocks({int(aid): len(invalid_path_rows)}).get(int(aid), [])
                )
            except Exception:
                reserved_path_ids_for_aid = reserve_path_ids(aid, len(invalid_path_rows))
        ...
        for im in im_raw:
            ...
            for blk in info.get("lineList", []) or []:
                if "width" in blk:
                    try: blk["width"] = max(0, min(50000, int(round(float(blk["width"])))))
                    except Exception: blk["width"] = 0
        ...
        for im in fixed:
            im["individualMissionID"] = int(next(reserved_individual_mission_iter))
