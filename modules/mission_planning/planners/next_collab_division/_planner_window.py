...
from modules.common.turn_dynamics import turn_arc_time_s, turn_period_s, turn_rate_for_radius_rad_s
...
from ._constants import (
    ...
    NEXT_COLLAB_AREA_PATH0_MIN_TARGET_SEP_M,
...
try:
    from ...runtime.assignment_search import iter_valid_target_permutations_in_order
except Exception:
    from modules.mission_planning.runtime.assignment_search import iter_valid_target_permutations_in_order  # type: ignore
...
def _safe_comb_perm_count(comb_n: int, comb_r: int, perm_n: int, perm_r: int) -> int:
    try:
        return int(math.comb(int(comb_n), int(comb_r))) * int(math.perm(int(perm_n), int(perm_r)))
    except Exception:
        return 0
...
class DivisionPlannerWindow(QMainWindow):
    ...
    def _aircraft_turn_radius_override_m(self, aircraft_id: int | None) -> float | None:
        ...
        if not isinstance(overrides, dict):
            return None
        try:
            radius_m = float(overrides.get(int(aircraft_id), 0.0) or 0.0)
        except Exception:
            return None
    ...
    def _turn_radius_for_speed_m(
        ...
        *,
        ...
        aircraft_scale = reference_turn_radius_scale_for_aircraft(aircraft_id)
        return (
            float(_turn_radius_for_speed(speed_mps))
            * float(self._turn_radius_scale_value())
            * float(aircraft_scale)
        )
    ...
    def _wp_only_display_row(self, row: Dict[str, Any]) -> Dict[str, Any]:
        cleaned = dict(row)
        if cleaned.get("entryTPrimeXY") is not None:
            cleaned["hasEntryTPrime"] = True
        for key in (
    ...
    def _next_collab_area_path0_target_sep_m(self, trigger_sep_m: float) -> float:
        return max(
            float(NEXT_COLLAB_AREA_PATH0_MIN_TARGET_SEP_M),
            max(0.0, float(trigger_sep_m))
            * float(self._next_collab_area_path0_target_sep_ratio()),
        )
    ...
    def _next_collab_area_path0_trigger_sep_m(self, db_max_sep_m: float) -> float:
        ...
        # This is the maximum distance at which a direct Area capture leg may
        # start.  Values above 1.5 km made almost-identical aircraft take
        # opposite branches at the old 3 km boundary: one got a near-area T0'
        # while another started filming from its current position.  Keep the
        # runtime value configurable downward, but never permit that distant
        # direct-capture regression again.
        if value <= 0.0:
            value = float(db_max_sep_m)
        return max(1.0, min(float(NEXT_COLLAB_AREA_PATH0_TRIGGER_SEP_M), value))
    ...
    def _fov_db_smaller_fov_steps(self) -> int:
        try:
            steps = int(get_runtime_int("fov_db_smaller_fov_steps", 4))
        except Exception:
            steps = 4
    ...
    def _area_fov_db_smaller_fov_steps(self) -> int:
        try:
            steps = int(get_runtime_int("area_fov_db_smaller_fov_steps", 3))
        except Exception:
            steps = 3
        return max(3, min(10, int(steps)))
    ...
    def _next_collab_area_estimated_footprint_area_m2(
        ...
        *,
        ...
        footprint = nadir_footprint_m(fov_deg)
        if footprint is not None:
            return float(footprint["areaM2"])
    ...
    def _next_collab_area_spacing_footprint_m(
        ...
        *,
        ...
        # Unified capture law: the footprint that drives spacing is the nadir
        # 세로 footprint of the selected FOV. The DB foot column and the
        # sep-based geometric proxy are no longer consulted.
        footprint = nadir_footprint_m(fov_deg)
        if footprint is not None:
            return float(footprint["verticalM"])
        return self._next_collab_area_footprint_m(sep_m, fov_deg)
    ...
    def _next_collab_area_sweep_spacing_m(
        ...
        *,
        ...
        spacing = area_vertical_sweep_spacing_m(fov_deg)
    ...
    def _resolve_next_collab_area_runtime_fov_foot(
        ...
        *,
        ...
        if manual_fov_deg is not None:
            return float(manual_fov_deg), self._next_collab_area_spacing_footprint_m(sep_m, manual_fov_deg)
        scaled_fov = self._scale_next_collab_area_fov(resolved_fov)
        return (
            scaled_fov,
            self._next_collab_area_spacing_footprint_m(sep_m, scaled_fov),
        )
    ...
    def _next_collab_area_resolved_db_values(
        ...
        # Keep next-collab area FOV monotonic: lower-step may reduce FOV, but
        # GSD margin must not promote it back to a wider camera angle.
        ...
        # Unified capture law: the aircraft scan speed comes from the
        # overlap-driven timing plan of the selected FOV, not the DB vel
        # column. resolvedVelMps stays km/h (consumers divide by 3.6).
        capture_plan = capture_speed_plan(
            resolved_fov,
            line_length_m=float(width_m or resolved_db_width or 0.0) or None,
            extra_spacing_scale=capture_area_spacing_scale(),
        )
        if capture_plan is not None:
            resolved_vel = float(capture_plan["aircraftSpeedKmh"])
        else:
    ...
    def _mid_line_overlay_bundle(
        ...
        for piece in pieces:
            ...
            if uav_state is not None:
                ...
                t0_preview = self._mid_line_t0_preview(
                    overlay_row,
                    origin_xy,
                    heading_deg,
                    turn_radius_m=self._default_turn_radius_m(aid),
    ...
    def _assignment_t0_preview_for_piece(
        ...
        origin_xy, heading_deg = uav_state
        return self._mid_line_t0_preview(
            overlay,
            origin_xy,
            float(heading_deg),
            turn_radius_m=self._default_turn_radius_m(int(aircraft_id)),
    ...
    def _mid_line_t0_preview(
        ...
        *,
        ...
        radius_m = (
            float(turn_radius_m)
            if turn_radius_m is not None and float(turn_radius_m) > 1.0
            else self._default_turn_radius_m()
        )
        effective_speed_mps = max(1.0, float(speed_mps))
        ...
        for branch, circle_center_xy in (("L", left_center_xy), ("R", right_center_xy)):
            ...
            for tangent_xy in tangent_points:
                ...
                eta_sec = float(horizon_s) + (float(ingress_dist_m) / effective_speed_mps)
                candidate = {
                    ...
                    "t0TurnRadiusM": float(radius_m),
                    "t0SpeedMps": float(effective_speed_mps),
                }
                candidate_key = (
    ...
    def _build_turn_prefix_rows(
        ...
        *,
        ...
        effective_radius_m = (
            float(radius_m)
            if radius_m is not None and float(radius_m) > 1.0
            else self._default_turn_radius_m()
        )
        effective_speed_mps = max(1.0, float(speed_mps))
    ...
    def _build_assignment_path_1_row(
        ...
        *,
        ...
        turn_radius_m = float(
            candidate.get("turnRadiusM", 0.0)
            or self._default_turn_radius_m(aid)
        )
        turn_speed_mps = float(
            candidate.get("turnSpeedMps", 0.0)
            or self._aircraft_turn_speed_mps(aid)
        )
        ...
        return {
            ...
            "horizonSec": float(horizon_s),
            "branch": branch,
            "turnRadiusM": float(turn_radius_m),
            "turnSpeedMps": float(turn_speed_mps),
    ...
    def _build_make_waypoint_row(
        ...
        *,
        ...
        turn_radius_m = float(
            base_row.get("turnRadiusM", 0.0)
            or self._default_turn_radius_m(aid)
        )
        turn_speed_mps = float(
            base_row.get("turnSpeedMps", 0.0)
            or self._aircraft_turn_speed_mps(aid)
        )
        route_xy, marker_rows, timeline_rows = self._build_turn_prefix_rows(
            origin_xy,
        ...
        return {
            ...
            "hasEntryTPrime": bool(base_row.get("entryTPrimeXY") is not None),
    ...
    def _assign_split_result_by_branch_ownership(
        ...
        own line/area to a different UAV.
        """
        by_branch: Dict[Optional[int], List[SplitPiece]] = {}
        for piece in split_result.pieces:
            data = piece.data if isinstance(piece.data, dict) else {}
            try:
                bi: Optional[int] = int(data.get("branchIndex"))
            except Exception:
                bi = None
        ...
        for bi, pieces in by_branch.items():
            owners = list(branch_ownership.get(int(bi))) if bi is not None else []
            # Fail closed: an unknown/missing branch owner is intentionally left
            # unassigned. Prediction-distance fallback would let a UAV cross
            # into another Type-2 branch while its owner is on a temporary task.
            for j, piece in enumerate(pieces):
                aid = int(owners[j % len(owners)]) if owners else 0
                piece.assigned_uav = aid if aid > 0 else None
                if piece.assigned_uav:
                    assigned_pieces += 1
                    uav_summary[aid] = int(uav_summary.get(aid, 0)) + 1
                    piece_lines.append(f"  P{piece.piece_index}: UAV{aid} (branch {bi}, sticky)")
        return {
    ...
    def _assign_split_result_by_prediction_distance(self, split_result: SplitRunResult) -> Dict[str, Any]:
        ...
        if branch_ownership:
            return self._assign_split_result_by_branch_ownership(split_result, branch_ownership)
        ...
        turn_radius_by_aircraft = {
            int(aid): max(0.0, float(self._default_turn_radius_m(int(aid))))
        ...
        def _best_candidate_for_target(
            ...
            penalty_m = 0.0
            aircraft_turn_radius_m = float(
                turn_radius_by_aircraft.get(int(aid), self._default_turn_radius_m(int(aid)))
            )
            near_turn_threshold_m = max(0.0, aircraft_turn_radius_m * 1.2)
            turn_penalty_m = max(3000.0, aircraft_turn_radius_m * 8.0)
            ...
            try:
                ...
                if perf_enabled:
                    replan_perf.add_elapsed(
                        ...
                        turn_guard_enabled=turn_guard_enabled,
                    )
    ...
    def _find_visibility_segment(
        ...
        max_steps = max(
            1,
            int(math.ceil(float(turn_period_s(radius_m, speed_mps) or 0.0) / TURN_PREVIEW_HORIZON_S)),
        )
                ...
                candidates.append(
                    {
                        ...
                        "turnPointHorizons": turn_point_horizons,
                        "turnRadiusM": float(radius_m),
                        "turnSpeedMps": float(speed_mps),
                    }
                )
                ...
                for target_point_xy, target_label in target_candidates:
                    ...
                    for aid in self.state.uav_ids:
                        if uav_overrides and int(aid) in uav_overrides:
                            ...
                            turn_speed_mps = self._aircraft_turn_speed_mps(int(aid))
                        else:
                            uav_state = self._uav_state_for_aircraft(int(aid))
                            if uav_state is None:
                                ...
                                continue
                            origin_xy, heading_deg = uav_state
                            override_radius = self._default_turn_radius_m(int(aid))
                            turn_speed_mps = self._aircraft_turn_speed_mps(int(aid))
                        left_center_xy, right_center_xy = self._turn_circle_centers_xy(
                                ...
                                tangent_rows.append(
                                    {
                                        ...
                                        "horizonSec": float(horizon_s),
                                        "turnRadiusM": float(override_radius),
                                        "turnSpeedMps": float(turn_speed_mps),
    ...
    def _assignment_path_1(self) -> None:
        ...
        try:
            ...
            perf_enabled = replan_perf.is_enabled()
            perf_start = replan_perf.start_timer() if perf_enabled else None
            dense_candidate_count = _safe_comb_perm_count(
                len(active_uavs),
                assign_count,
                len(target_keys),
                assign_count,
            ...
            try:
                for uav_subset in itertools.combinations(active_uavs, assign_count):
                    for target_perm in iter_valid_target_permutations_in_order(
                        uav_subset,
                        target_keys,
                        candidate_by_uav_target,
                    ):
                        if perf_enabled:
                            checked_candidates += 1
                ...
                if perf_enabled:
                    replan_perf.add_elapsed(
                        ...
                        candidate_pairs=len(candidate_by_uav_target),
                        estimated_candidates=dense_candidate_count,
                        checked_candidates=checked_candidates,
                        valid_candidates=valid_candidates,
                        pruned_candidates=max(0, dense_candidate_count - checked_candidates),
                    )
    ...
    def _assignment_next_mission(self) -> None:
        ...
        try:
            ...
            for row in prev_paths:
                ...
                speed_overrides = getattr(self, "_aircraft_speed_overrides", None)
                if not isinstance(speed_overrides, dict):
                    speed_overrides = {}
                    self._aircraft_speed_overrides = speed_overrides
                speed_overrides[int(aid)] = float(vel_mps)
                turn_radius_m = self._turn_radius_for_speed_m(
                    vel_mps,
                    aircraft_id=aid,
                )
        ...
        try:
            ...
            for overlay in self.state.mid_line_segments:
                ...
                if sep_cand > area_path0_trigger_sep and area_path0_trigger_sep > 0.0:
                    ...
                    target_sep = self._next_collab_area_path0_target_sep_m(
                        area_path0_trigger_sep
                    )
                    lo, hi = 0.0, ingress_len
                    if ep0 is not None and ep1 is not None:
                        for _ in range(40):
                ...
                route_xy, marker_rows, timeline_rows = self._build_turn_prefix_rows(
                    ...
                    suppress_last_turn_label=True,
                    radius_m=float(
                        overlay.get("t0TurnRadiusM", 0.0)
                        or self._default_turn_radius_m(aid)
                    ),
                    speed_mps=float(
                        overlay.get("t0SpeedMps", 0.0)
                        or self._aircraft_turn_speed_mps(aid)
                    ),
                )
    ...
    def _make_sweep(self) -> None:
        ...
        try:
            ...
            for path_row in path_0_rows:
                ...
                is_takeover = bool(
                    path_row.get("entryTPrimeXY") is not None
                    or path_row.get("hasEntryTPrime")
                    or str(path_row.get("startLabel", "") or "").endswith("'")
                )
                dense_first_step = (
