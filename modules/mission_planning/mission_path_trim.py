...
def trim_waypoints_by_sweep_points(
    ...
    *,
    ...
    for wp in waypoints:
        ...
        if remaining >= len(coords):
            removed_points += len(coords)
            remaining -= len(coords)
            if preserve_waypoints:
                # Keep waypoint identity/order; only clear sweep internals.
                line_search["coordinateList"] = coords[-1:] if coords else []
