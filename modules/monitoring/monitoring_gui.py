...
class MonitoringTab(CSCTabBase):
    ...
    def __init__(self, *, messenger, parent=None):
        ...
        self.manager = MonitoringManager(
            node_messenger=self.messenger, log_callback=self.log_from_manager
        )
...
class MainWindow(QMainWindow):
    ...
    def _sw_code(self) -> str:
        ...
        return {"mission": "MMR", "monitoring": "MSM", "decision": "MOB"}.get(
            role, "MMR"
        )
    ...
    def _on_mode_slider_changed(self, val: int):
        ...
        self._append_log_line(
            f"[MODE] 슬라이더 변경 → {labels[int(val)] if 0 <= val < len(labels) else val}"
        )
    ...
    def _send_self_check_0102(self, status: int = 1, _retry: int = 0):
        ...
        if not body:
            ...
            role = (os.environ.get("KU_ROLE") or "").lower()
            src = {"mission": "MMR", "monitoring": "MSM", "decision": "MOB"}.get(
                role, "MMR"
            )
            try:
        ...
        try:
            ...
            if _retry < 5:
                QTimer.singleShot(
                    500,
                    lambda: self._send_self_check_0102(
                        status=status, _retry=_retry + 1
                    ),
                )
            else:
            ...
            if (on and not running) or ((not on) and running):
                try:
                    ...
                    if btn is not None and hasattr(btn, "click"):
                        btn.click()
                        self._append_log_line(
                            f"[CTRL] 0102 버튼 click() → {'ON' if on else 'OFF'} 요청"
                        )
                        return True
                except Exception:
                    pass
                try:
                    if hasattr(tab, "_on_tx_button_clicked"):
                        ...
                        self._append_log_line(
                            f"[CTRL] 0102 토글 메서드 호출 → {'ON' if on else 'OFF'} 요청"
                        )
                        return True
                except Exception:
                    pass
                ...
                return True
            self._append_log_line(
                f"[CTRL] 0102 상태 유지: {'ON' if running else 'OFF'}"
            )
            return True
        except Exception as e:
    ...
    def _install_test_shortcuts(self):
        QShortcut(
            QKeySequence("1"), self, activated=lambda: self._ensure_selfcheck_0102(True)
        )
        QShortcut(
            QKeySequence("0"),
            self,
            activated=lambda: self._ensure_selfcheck_0102(False),
        )
