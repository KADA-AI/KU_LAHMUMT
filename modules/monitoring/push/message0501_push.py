...
def _dict_to_individual_mission_progress_status(
    data: dict,
    ...
    if "currentIndividualMission" in data and isinstance(
        data["currentIndividualMission"], dict
    ):
        try_set(
            obj,
            "currentIndividualMission",
            _dict_to_current_individual_mission(data["currentIndividualMission"]),
        )
    if "currentIndividualMissionProgress" in data:
        try_set(
            obj,
            "currentIndividualMissionProgress",
            int(data["currentIndividualMissionProgress"]),
        )
    return obj
...
def _dict_to_mission_progress(data: dict) -> MissionProgress:
    ...
    if "individualMissionProgressStatusList" in data and isinstance(
        data["individualMissionProgressStatusList"], list
    ):
        T = cs_new("IndividualMissionProgressStatus", MSG_ID)
        lst = List[T]()
        for item in data["individualMissionProgressStatusList"]:
            lst.Add(
                _dict_to_individual_mission_progress_status(
