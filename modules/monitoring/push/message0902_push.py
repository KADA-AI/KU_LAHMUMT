...
def _dict_to_prior_mission(data: dict) -> PriorMission:
    ...
    if "coordinateOrientation" in data and isinstance(data["coordinateOrientation"], dict):
        try_set(obj, "coordinateOrientation", _dict_to_coordinate_orientation(data["coordinateOrientation"]))
    if "targetOrientation" in data and isinstance(data["targetOrientation"], dict):
        try_set(obj, "targetOrientation", _dict_to_target_orientation(data["targetOrientation"]))
    return obj
...
def _dict_to_replan_request(data: dict) -> ReplanRequest:
    ...
    if "replanRequestTime" in data and isinstance(data["replanRequestTime"], dict):
        try_set(obj, "replanRequestTime", _dict_to_replan_request_time(data["replanRequestTime"]))
    ...
    if "individualMissionIDList" in data and isinstance(data["individualMissionIDList"], list):
        ...
        for item in data["individualMissionIDList"]:
            lst.Add(_dict_to_individual_mission_id(item if isinstance(item, dict) else {}))
