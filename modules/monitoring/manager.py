...
class MonitoringManager:
    ...
    def __init__(self, node_messenger: Any, receive_messages_config: list):
        ...
        # 2. 시스템 모드 기본값 설정
        self.logic_store.set_data("system_mode", "monitoring")
        ...
        self._log(
            "MON_MGR", "INFO", "Monitoring Manager initialized."
        )
    ...
    def handle_message_reception(self, msg_id: str, data_object: object):
        ...
        self._log(
            "MON_MGR", "RECV", f"Parsed object received: {msg_id}"
        )
        self.receive_store.set_data(msg_id, data_object)

        if self.gui_update_callback:
            self.gui_update_callback("receive", msg_id)
    ...
    def set_system_mode(self, mode: str):
        """시스템 실행 모드를 변경합니다. (예: 'monitoring' 또는 'replan')"""
        self._log("MON_MGR", "MODE_CHANGE", f"System mode set to '{mode}'.")
        self.logic_store.set_data("system_mode", mode)
