...
_SR_LOG_LOCK = threading.Lock()
_SR_LOG_MIN_INTERVAL_MS = 1000
_SR_LOG_COLUMNS = (
    "timestamp_ms",
    "timestamp_text",
    "mission_plan_id",
...
def _sr_log_path() -> Path | None:
    try:
        return db_paths.get_db_subpath("DSS_Internal", "module_logs", "spatial_resolution_monitor.csv")
    except Exception:
        return None


def _sr_csv_value(value: object | None) -> object:
    if value is None:
        return ""
...
def _append_sr_csv_row(row: dict[str, Any]) -> None:
    path = _sr_log_path()
    if path is None:
        return
    try:
    ...
    with _SR_LOG_LOCK:
        try:
            ...
            with path.open("a", encoding="utf-8", newline="") as fh:
                writer = csv.DictWriter(fh, fieldnames=list(_SR_LOG_COLUMNS), extrasaction="ignore")
                if write_header:
                    writer.writeheader()
                writer.writerow({key: _sr_csv_value(row.get(key)) for key in _SR_LOG_COLUMNS})
        except Exception:
            return


class SpatialResolutionCsvLogger:
    def __init__(self, source: str) -> None:
        self._source = str(source or "quality_monitor")
        self._enabled = False
        self._last_write_wall_ms: dict[tuple[int, int, int], int] = {}

    def set_enabled(self, enabled: bool) -> None:
        self._enabled = bool(enabled)
        if not self._enabled:
            self._last_write_wall_ms.clear()

    def write_snapshot(self, snapshot: dict[str, Any]) -> None:
        if not self._enabled:
            return
        if not snapshot or not bool(snapshot.get("measurement_enabled")):
            return
        if not bool(snapshot.get("have_data")):
            return
        if _coerce_float(snapshot.get("eq_gsd_m_per_px")) is None:
            return
        aircraft_id = _coerce_int(snapshot.get("aircraft_id"))
        if aircraft_id is None:
            return
        flying = _coerce_int(snapshot.get("flying"))
        if flying != 1:
            return
        filming = _coerce_int(snapshot.get("filming"))
        if filming != 1:
            return
        mission_plan_id = _coerce_int(snapshot.get("mission_plan_id")) or 0
        current_waypoint_id = _coerce_int(snapshot.get("current_waypoint_id")) or 0
        key = (int(mission_plan_id), int(aircraft_id), int(current_waypoint_id))
        wall_ms = int(time.monotonic() * 1000)
        last_wall_ms = int(self._last_write_wall_ms.get(key, 0))
        if last_wall_ms > 0 and (wall_ms - last_wall_ms) < _SR_LOG_MIN_INTERVAL_MS:
            return
        self._last_write_wall_ms[key] = wall_ms

        row = dict(snapshot)
        timestamp_ms = _coerce_int(row.get("timestamp_ms"))
        row["timestamp_ms"] = timestamp_ms
        row["timestamp_text"] = format_timestamp_ms(timestamp_ms) if timestamp_ms is not None else ""
        row["aircraft_id"] = int(aircraft_id)
        row["uav_label"] = f"UAV{max(0, int(aircraft_id) - 3)}"
        row["mission_plan_id"] = _coerce_int(row.get("mission_plan_id"))
        row["current_waypoint_id"] = _coerce_int(row.get("current_waypoint_id"))
        row["individual_mission_id"] = _coerce_int(row.get("individual_mission_id"))
        row["path_id"] = _coerce_int(row.get("path_id"))
        row["sensor_operation_mode"] = _coerce_int(row.get("sensor_operation_mode"))
        row["flying"] = _coerce_int(row.get("flying"))
        row["filming"] = _coerce_int(row.get("filming"))
        row["monitor_source"] = self._source
        _append_sr_csv_row(row)
...
def _is_sweep_measurement_active(
    ...
    *,
    ...
    if waypoint_operation_mode is not None and int(waypoint_operation_mode) != 2:
        return False
    if sensor_operation_mode is None or int(sensor_operation_mode) != 2:
        return False
    if flying_status is None or int(flying_status) != 1:
        return False
    if filming_status is None or int(filming_status) != 1:
        return False
    return True
...
class SpatialResolutionCard(QGroupBox):
    ...
    def update_snapshot(
        ...
        *,
        ...
        have_data = fp_w is not None and fp_h is not None and fp_area is not None
        ...
        if not measurement_enabled:
            ...
            return {
                ...
                "footprint_height_m": fp_h,
                "footprint_area_m2": fp_area,
                "eq_gsd_m_per_px": None,
                "req_gsd_m_per_px": req_gsd,
                "req_footprint_area_m2": req_fp_area,
                "satisfied": None,
                "satisfy_rate_pct": None,
                ...
                "obj_w_m": obj_w_m,
                "obj_h_m": obj_h_m,
                "obj_min_px_x": obj_min_px_x,
                "obj_min_px_y": obj_min_px_y,
            }
        ...
        return {
            ...
            "footprint_height_m": fp_h,
            "footprint_area_m2": fp_area,
            "eq_gsd_m_per_px": eq_gsd,
            "req_gsd_m_per_px": req_gsd,
            "req_footprint_area_m2": req_fp_area,
            "satisfied": satisfied,
            "satisfy_rate_pct": satisfy_rate_pct,
            "sample_count": self._sample_count,
            "pass_count": self._pass_count,
            "img_w_px": img_w_px,
            "img_h_px": img_h_px,
...
class SpatialResolutionMonitorPanel(QWidget):
    ...
    def __init__(self, parent: QWidget | None = None, *, compact: bool = True) -> None:
        ...
        self._sr_csv_logger = SpatialResolutionCsvLogger("dashboard_panel")
    ...
    def set_csv_logging_enabled(self, enabled: bool) -> None:
        self._sr_csv_logger.set_enabled(bool(enabled))
    ...
    def _apply_mission_plan_view(self, mission_plan_id: int | None) -> None:
        ...
        for entry in (self._mission_view or {}).get("uav_entries") or []:
            ...
            for mission in entry.get("missions") or []:
                ...
                if sep_m is not None and sep_m <= 0.0:
                    sep_m = None
                # SEP가 없어도 waypoint meta는 만들어 스윕 게이트(lineSearch 판정)가
                # 열리게 하고, SEP 기반 임계값 계산만 비활성화한다 - SEP 미탑재
                # 임무(예: 재계획 산출물)에서 공간해상도 모니터가 통째로 멈추지
                # 않도록.
                base_meta = {
            ...
            snapshot.update(
                {
                    ...
                    "individual_mission_id": _coerce_int(waypoint_meta.get("individual_mission_id")),
                    "path_id": _coerce_int(waypoint_meta.get("path_id")),
                    "sensor_operation_mode": _coerce_int(state.get("sensor_operation_mode")),
                    "flying": _coerce_int(state.get("flying")),
                    "filming": _coerce_int(state.get("filming")),
                }
            )
            self._sr_csv_logger.write_snapshot(snapshot)
...
class QualityMonitorTab(QWidget):
    def __init__(self, parent: QWidget | None = None) -> None:
        ...
        self._last_chart_redraw_monotonic_ms: int = 0
        self._log_callback: Callable[[str], None] | None = None
        self._sr_csv_logger = SpatialResolutionCsvLogger("quality_monitor_tab")
