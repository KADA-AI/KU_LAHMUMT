...
def _load_path_waypoint_summary(
    path_id: object,
    *,
    ...
    def _load_payload(folder: str, file_id: int | None) -> dict[str, Any]:
        parsed_file_id = _coerce_int(file_id)
        if parsed_file_id is None or parsed_file_id <= 0:
            return {}
        path = db_paths.get_db_subpath(str(folder), f"{int(parsed_file_id)}.json")
        payload = json.loads(path.read_text(encoding="utf-8"))
        return payload if isinstance(payload, dict) else {}

    try:
        cached_loader = getattr(dispatch_context, "cached_db_json", None)
        if callable(cached_loader):
            payload = cached_loader("FlightPath", int(parsed_path_id), _load_payload)
        else:
            payload = _load_payload("FlightPath", int(parsed_path_id))
    except Exception:
        return [], None
...
class PriorMissionReplanCoordinator:
    ...
    def on_agent_states(
        ...
        *,
        ...
        settings_snapshot = (
            dispatch_context.settings_snapshot
            if dispatch_context is not None and isinstance(getattr(dispatch_context, "settings_snapshot", None), dict)
            else None
        )
        close_cfg = get_post_attack_rejoin_settings(settings_snapshot)
