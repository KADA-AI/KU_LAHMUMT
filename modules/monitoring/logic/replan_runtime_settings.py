...
_FALLBACK_SETTINGS: Dict[str, Any] = {
    ...
    "path_deviation": {
        ...
        "watch_angle_deg": 90.0,
        "warning_angle_deg": 150.0,
        "hold_s": 2.0,
    ...
    "next_collab": {
        ...
        "default_entry_strategy": "midpoint_to_next_start",
    },
    "rtb": {
...
def settings_path() -> Path:
    return Path(__file__).resolve().parents[1] / "replan_settings.json"


def defaults_path() -> Path:
    return Path(__file__).resolve().parents[1] / "replan_settings_defaults.json"
...
def _normalize_strategy(value: Any, default: str) -> str:
    ...
    if text in {"turn_projection", "midpoint_to_next_start"}:
        return text
    return str(default)
...
def normalize_replan_settings(
    ...
    *,
    ...
    path_cfg["turn_rate_threshold_dps"] = max(0.1, _as_float(path_cfg.get("turn_rate_threshold_dps"), 1.0))
    ...
    path_cfg["alt_waypoint_lead_time_s"] = max(0.0, _as_float(path_cfg.get("alt_waypoint_lead_time_s"), 8.0))
    path_cfg["next_mission_entry_lead_time_s"] = max(
        0.0, _as_float(path_cfg.get("next_mission_entry_lead_time_s"), 5.0)
    )
    ...
    next_collab_cfg = merged.get("next_collab")
    if not isinstance(next_collab_cfg, dict):
        next_collab_cfg = {}
        merged["next_collab"] = next_collab_cfg
    next_collab_cfg["entry_lead_time_s"] = max(0.0, _as_float(next_collab_cfg.get("entry_lead_time_s"), 5.0))
    next_collab_cfg["default_entry_strategy"] = _normalize_strategy(
        next_collab_cfg.get("default_entry_strategy"),
        "midpoint_to_next_start",
    )
