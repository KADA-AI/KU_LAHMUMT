...
class ReplanQueueManager:
    ...
    def find_target_detection_option_decision_blocker(self) -> dict[str, Any] | None:
        """Return an unresolved normal target-detection option request, if any."""
        now_ms = int(self._now_fn())
        with self._lock:
            candidates: list[_QueueItem] = []
    ...
    def _logs_from_result(self, result: dict[str, Any]) -> list[str]:
        ...
        for event in result.get("events") or []:
            ...
            elif event_type == "stale_notice_ignored":
                signal_name = event.get("signal_name") or "0001"
                logs.append(
                    f"[RQUEUE] stale {signal_name} ignored before dispatch -> "
                    f"#{queue_id} {source_label}: {reason}"
                )
        return logs
    ...
    def _timeout_ms_locked(self) -> int:
        ...
        return max(1000, _coerce_int(cfg.get("active_timeout_ms")) or 120000)
    ...
    def _handle_failure_notice(self, *, payload: object | None, now_ms: int) -> dict[str, Any]:
        ...
        if self._active is None:
            return self._result_locked(now_ms=now_ms)
        if (
            str(self._active.status or "") == "dispatching"
            and self._active.dispatched_ms is None
        ):
            # A completion signal can promote the next item before the old
            # request's trailing 0001 notice arrives.  The promoted item has
            # not sent its own 0902 yet, so the notice cannot belong to it.
            return self._result_locked(
                now_ms=now_ms,
                events=[
                    {
                        "type": "stale_notice_ignored",
                        "signal_name": "0001",
                        "item": self._active.public_dict(now_ms=now_ms),
                    }
                ],
            )
        if is_noop_notice:
    ...
    def _snapshot_locked(self, *, now_ms: int | None = None) -> dict[str, Any]:
        ...
        return {
            ...
            "stats": {
                ...
                "active_timeout_ms": max(1000, _coerce_int(queue_cfg.get("active_timeout_ms")) or 120000),
    ...
    def _deepcopy_locked(self, value: Any, *, label: str) -> Any:
        ...
        metric_label = str(label or "copy")
        replan_perf.add("monitoring.replan_queue.deepcopy", elapsed_ms=elapsed_ms)
        replan_perf.add(f"monitoring.replan_queue.deepcopy.{metric_label}", elapsed_ms=elapsed_ms)
        return copied
