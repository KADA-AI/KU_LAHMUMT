...
def _detect_reexecute_clone_pair(
    plan: dict[str, Any],
    *,
    ...
    for idx in range(len(missions) - 1):
        ...
        if _mission_clone_fingerprint(original) != _mission_clone_fingerprint(cloned):
            continue
        original_done = _mission_done(original)
        cloned_done = _mission_done(cloned)
        if original_done and not cloned_done:
            return int(original_id), int(cloned_id)
        if allow_status_fallback and original_done == cloned_done:
            # The refreshed 0201 can inherit the currently tracked completion
            # state before this coordinator sees it.  In that narrow execute=2
            # wait window, both adjacent rows can therefore have the same
            # isDone value even though the highest ID is the newly inserted
            # reexecute clone.  Geometry/content identity plus adjacency and the
            # highest-ID guard above are stable; status alone is not.
            fallback_pair = (int(original_id), int(cloned_id))
...
class CollabReexecuteCoordinator:
    ...
    def on_input_plan(
        ...
        *,
        ...
        clone_status_fallback = False
        if (
            clone_pair is None
            and self._state.pending
            and not self._state.inflight
        ):
            clone_pair = _detect_reexecute_clone_pair(
                plan,
                allow_status_fallback=True,
            )
            clone_status_fallback = clone_pair is not None
        ...
        if clone_input_id is not None:
            ...
            if clone_status_fallback:
                logs.append(
                    "[REEXEC] equal-status clone recovered in execute=2 wait -> "
                    f"sourceInputMissionID={int(clone_source_input_id)}, "
                    f"cloneInputMissionID={int(clone_input_id)}"
                )
