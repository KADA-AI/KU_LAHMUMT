...
def _centerline_coordinate_lists(mission: dict[str, Any]) -> list[list[dict[str, Any]]]:
    # Execution geometry must come from this individual mission.  Every child
    # mission related to a LINE input also receives ``input_line_list`` as
    # immutable source metadata; using that field as executable geometry made
    # attack, return, and transit artifacts look like full LINE scans.
    coordinate_lists = _line_list_coordinate_lists(mission.get("line_list"))
    if coordinate_lists:
...
def update_line_sweep_state(
    ...
    *,
    ...
    # 촬영 꺼짐뿐 아니라 좌표지향/자동추적 등 비스윕 센서 모드, 원거리 주시,
    # 계획 폭 대비 과대 풋프린트도 적립을 차단한다(선회·원거리 오적립 방지).
    if not evaluate_capture_gate(agent_state, width_hint_m=definition.width_hint_m):
        state.last_line_index = None
        state.last_chainage_m = None
        state.last_xy = None
