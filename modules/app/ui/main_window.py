...
from modules.common.version_info import load_release_info
...
class MainWindow(QMainWindow):
    ...
    def __init__(self):
        ...
        self._mission_status_monitor_url: Optional[str] = None
    ...
    def _build_action_panel(self, *, horizontal: bool = False) -> QWidget:
        ...
        button_stack = QGridLayout() if horizontal else QVBoxLayout()
        ...
        action_buttons.append(self.btn_simulation_run)

        self.btn_mission_status_monitor = QPushButton("임무상태 모니터링 SW 실행", card)
        self.btn_mission_status_monitor.setObjectName("BtnMissionStatusMonitor")
        self.btn_mission_status_monitor.setFixedHeight(36 if horizontal else 40)
        self.btn_mission_status_monitor.setSizePolicy(QSizePolicy.Expanding, QSizePolicy.Fixed)
        self.btn_mission_status_monitor.clicked.connect(self._launch_mission_status_monitor)
        action_buttons.append(self.btn_mission_status_monitor)
        ...
        action_buttons.append(self.btn_log_analyzer_run)
        ...
        for preset in SCENARIO_PRESETS:
            button = QPushButton(str(preset["button_label"]), card)
            button.setObjectName("BtnScenarioPreset")
            button.setProperty("scenarioPresetKey", str(preset["key"]))
            button.setFixedHeight(28)
            button.setSizePolicy(QSizePolicy.Expanding, QSizePolicy.Fixed)
            button.setToolTip(
                f"{preset['display_name']} 0201/0203 파일을 덮어쓰기 원본으로 준비합니다."
            )
            button.clicked.connect(
                lambda _checked=False, key=str(preset["key"]): self._handle_scenario_preset(key)
            )
            preset_buttons.append(button)
        self._scenario_preset_buttons = preset_buttons
        ...
        if horizontal:
            ...
            for button, placement in zip(action_buttons, placements):
                button_stack.addWidget(button, *placement)
            for column in range(6):
                button_stack.setColumnStretch(column, 1)
            ...
            button_stack.addLayout(preset_row, 2, 0, 1, 6)
        else:
            ...
            button_stack.addLayout(preset_grid)
    ...
    def _build_module_service_panel(self) -> QWidget:
        ...
        for role, label in roles:
            ...
            mission_creator_btn = None
            if role == "info":
                mission_creator_btn = QPushButton("임무 생성 GUI", box)
                mission_creator_btn.setObjectName("SecondaryButton")
                mission_creator_btn.setFixedHeight(28)
                mission_creator_btn.setProperty("role", role)
                mission_creator_btn.setProperty("action", "mission_creation_gui")
                lay.addWidget(mission_creator_btn)
            ...
            self._module_service_widgets[role] = {
                ...
                "button": btn,
                "mission_creation_button": mission_creator_btn,
            }
            row.addWidget(box, 1)
    ...
    def mission_creation_gui_button(self):
        widgets = getattr(self, "_module_service_widgets", {}).get("info")
        if isinstance(widgets, dict):
            return widgets.get("mission_creation_button")
        return None
    ...
    def _find_project_root(self) -> Path:
        """Locate the project root, excluding the internal modules/run.py."""
        here = Path(__file__).resolve().parent
        for candidate in [here, *here.parents]:
            if (candidate / "run.py").is_file() and (
                candidate / "modules" / "common" / "db_paths.py"
            ).is_file():
                return candidate
        return here
    ...
    def _launch_gui(self, script_name: str):
        ...
        # Dashboard implementation lives under <root>/modules/app/ui.
        root = db_paths.PROJECT_ROOT
        ds_dir = MODULE_ROOT / "decision_support"
    ...
    def _launch_role(self, role: str, *, schedule_powerup: bool = True, start_hidden: bool = False):
        ...
        if role == "decision":
            candidates = [
                MODULE_ROOT / "decision_support" / "decision_support_gui.py",
            ]
            target_log = self.module_decision

        elif role == "mission":
            candidates = [
                MODULE_ROOT / "mission_planning" / "mission_planning_gui.py",
                MODULE_ROOT / "decision_support" / "assignment_planning_gui.py",
            ]
            target_log = self.module_mission

        elif role == "monitor":
            candidates = [
                MODULE_ROOT / "monitoring" / "monitoring_gui.py",
                MODULE_ROOT / "monitoring_ver2" / "monitoring_gui.py",
            ]
            target_log = self.module_monitor

        elif role == "info":
            candidates = [
                MODULE_ROOT / "info_manage" / "info_manage.py",
            ]
        elif role == "integration":
            candidates = [
                MODULE_ROOT / "integration_module" / "integration_gui.py",
            ]
        else:
            return
        ...
        try:
            ...
            env["PYTHONUNBUFFERED"] = "1"
            # 대시보드에서 개별 재기동해도 run.py 최초 기동과 동일하게
            # MMR/MSM/MOB만 0102를 주기 송신하고 INF는 제외한다.
            env.update(
                {
                    "KU_MMR_0102_PUSH_ENABLED": "1",
                    "KU_MSM_0102_PUSH_ENABLED": "1",
                    "KU_MOB_0102_PUSH_ENABLED": "1",
                    "KU_INF_0102_PUSH_ENABLED": "0",
                }
            )
            title_map = {
    ...
    def _launch_simulation(self):
        ...
        try:
            root = db_paths.PROJECT_ROOT
            script = MODULE_ROOT / "sim_main.py"
            if not script.exists():
    ...
    def _launch_log_analyzer(self):
        ...
        try:
            root = db_paths.PROJECT_ROOT
            script = MODULE_ROOT / "log_main.py"
            if not script.exists():
    ...
    def _launch_mission_status_monitor(self, *, auto_start: bool = False) -> None:
        ...
        if existing is not None and existing.poll() is None:
            if auto_start:
                self._log_simulation(
                    message=(
                        "[RUN] Mission Status Monitoring auto-start skipped; "
                        f"already running (pid={existing.pid})"
                    )
                )
                return
            self._log_simulation(
                message=f"[RUN] Stopping Mission Status Monitoring (pid={existing.pid})"
            )
            self._kill_process_tree(existing)
            self._role_processes.pop("mission_status_monitor", None)
            self._mission_status_monitor_url = None
            if button is not None:
                button.setText("임무상태 모니터링 SW 실행")
            return
        self._role_processes.pop("mission_status_monitor", None)

        if button is not None:
            button.setEnabled(False)
            button.setText("임무상태 모니터링 SW 준비 중...")
            QApplication.processEvents()
        try:
            root = db_paths.PROJECT_ROOT
            script = MODULE_ROOT / "mission_status_monitoring" / "server.py"
            if not script.exists():
                raise FileNotFoundError(f"Mission status monitoring script not found: {script}")
            host = str(os.environ.get("MISSION_STATUS_MONITOR_HOST") or "127.0.0.1").strip()
            try:
                port = int(os.environ.get("MISSION_STATUS_MONITOR_PORT") or 8300)
            except Exception:
                port = 8300
            browse_host = "127.0.0.1" if host in {"0.0.0.0", "::"} else host
            selected_port = None
            for candidate in range(port, port + 24):
                if self._is_tcp_port_available(browse_host, candidate):
                    selected_port = candidate
                    break
                if self._is_mission_status_monitor_service(browse_host, candidate):
                    self._kill_port_occupant(candidate, "Mission Status Monitoring")
                    selected_port = candidate
                    break
            if selected_port is None:
                raise RuntimeError(f"No available monitoring port in {port}-{port + 23}")
            port = selected_port
            url = f"http://{browse_host}:{port}/"

            env = os.environ.copy()
            env["PYTHONUNBUFFERED"] = "1"
            env["KU_CONSOLE_TITLE"] = "KU Mission Status Monitoring Console"
            env["MISSION_STATUS_MONITOR_HOST"] = host
            env["MISSION_STATUS_MONITOR_PORT"] = str(port)
            ui_line = getattr(self, "_db_path_line", None)
            ui_db_root = (
                ui_line.text().strip()
                if ui_line is not None and hasattr(ui_line, "text")
                else ""
            )
            env["KU_MISSION_DB_ROOT"] = (
                ui_db_root or db_paths.get_active_db_root_str()
            )
            env["KU_LAUNCHED_BY_DASHBOARD"] = "1"
            proc = subprocess.Popen(
                [
                    preferred_console_python(sys.executable),
                    str(script),
                    "--host",
                    host,
                    "--port",
            ...
            self._role_processes["mission_status_monitor"] = proc
            self._mission_status_monitor_url = url
            if button is not None:
                button.setText("임무상태 모니터링 SW 종료")
            self._log_simulation(
                message=(
                    "[RUN] Mission Status Monitoring launched "
                    f"({url}, pid={getattr(proc, 'pid', '?')})"
                )
            )
            QTimer.singleShot(900, lambda u=url: QDesktopServices.openUrl(QUrl(u)))
            QTimer.singleShot(
                1400,
                lambda p=proc: self._check_mission_status_monitor_process(p),
            )
        except Exception as exc:
            self._mission_status_monitor_url = None
            message = f"[RUN ERR] Mission Status Monitoring launch failed: {exc}"
            self._log_simulation(message=message)
            if not auto_start:
                QMessageBox.critical(self, "임무상태 모니터링 SW 실행 실패", message)
            if button is not None:
                button.setText("임무상태 모니터링 SW 실행")
        finally:
            if button is not None:
    ...
    def _check_mission_status_monitor_process(self, proc) -> None:
        current = self._role_processes.get("mission_status_monitor")
        ...
        if proc.poll() is not None:
            ...
            if button is not None:
                button.setText("임무상태 모니터링 SW 실행")
            self._log_simulation(
                message=f"[RUN] Mission Status Monitoring stopped (exit={proc.returncode})"
            )
            return
        QTimer.singleShot(
            1400,
    ...
    def _add_left_placeholder(self, grid: QGridLayout) -> None:
        ...
        if body is not None:
            ...
            self.btn_mission_status_monitor = QPushButton("임무상태 모니터링 SW 실행", placeholder)
            self.btn_mission_status_monitor.setObjectName("BtnMissionStatusMonitor")
            self.btn_mission_status_monitor.setFixedHeight(40)
            self.btn_mission_status_monitor.clicked.connect(self._launch_mission_status_monitor)
            body.addWidget(self.btn_mission_status_monitor)
    ...
    def _handle_module_shutdown(self) -> None:
        ...
        try:
            root = db_paths.PROJECT_ROOT
            sim_script = MODULE_ROOT / "sim_main.py"
    ...
    @staticmethod
    def _is_mission_status_monitor_service(host: str, port: int) -> bool:
        try:
            from urllib.request import urlopen

            with urlopen(f"http://{host}:{int(port)}/api/health", timeout=0.5) as response:
                payload = json.loads(response.read().decode("utf-8"))
            return payload.get("service") == "ku-mission-status-monitor"
        except Exception:
            return False
        try:
            from modules.sim.config import resolve_server_binding
    ...
    def _handle_scenario_preset(self, preset_key: str) -> None:
        """Prepare one generated mission pair for the existing overwrite action."""

        try:
            result = stage_scenario_preset(
                preset_key,
                logs_root=db_paths.PROJECT_ROOT / "Logs",
            )
        except Exception as exc:
            message = f"임무 유형 파일 준비에 실패했습니다.\n\n{exc}"
            self._log_to_modules(f"[RUN] scenario preset failed ({preset_key}): {exc}")
            QMessageBox.critical(self, "임무 유형 선택 실패", message)
            return

        display_name = str(result["display_name"])
        package_id = int(result["package_id"])
        self._log_to_modules(
            f"[RUN] scenario preset ready: {display_name} "
            f"(0201/0203 packageID={package_id})"
        )
        QMessageBox.information(
            self,
            "임무 유형 준비 완료",
            f"{display_name}\n\n"
            f"0201: InputMissionPlan\\{package_id}.json\n"
            f"0203: MissionReferenceInfo\\{package_id}.json\n\n"
            "이제 '0201/0203 덮어쓰기' 버튼을 누르면 됩니다.",
        )
    ...
    def _refresh_fov_db_display(self) -> None:
        try:
            ...
            path = MODULE_ROOT / "resource" / "db" / "fov_db.csv"
        text = str(path)
    ...
    def _browse_fov_db(self) -> None:
        try:
            ...
            current = MODULE_ROOT / "resource" / "db" / "fov_db.csv"
        start_dir = (
            current.parent
            if current.parent.exists()
            else MODULE_ROOT / "resource" / "db"
        )
