...
def _latest_json_payload(rows: List[Dict[str, Any]]) -> Dict[str, Any]:
    if not rows:
        return {}
...
def _reference_rows(reference: object, *keys: str) -> List[Dict[str, Any]]:
...
def load_flight_paths(base_path: str, *, project_root: Path) -> Dict[str, Any]:
    ...
    reference_info = _latest_json_payload(_load_json_files(db_root / "MissionReferenceInfo"))
    take_over_list = _reference_rows(reference_info, "takeOverInfoList", "TakeOverInfoList")
    hand_over_list = _reference_rows(reference_info, "handOverInfoList", "HandOverInfoList")
    rtb_coordinate_list = _reference_rows(reference_info, "rtbCoordinateList", "RTBCoordinateList")
