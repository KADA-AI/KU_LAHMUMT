...
def _finite_num_or(value: Any, default: float) -> float:
    parsed = _num(value)
...
def _coordinate_ground_speeds(
    projected: list[tuple[AgentSample, float, float]],
    *,
    window_s: float = 2.0,
) -> list[float | None]:
    """Estimate ground speed with a centered coordinate window.

    A centered window avoids amplifying the roughly 5 Hz coordinate quantization
    present in 0401 telemetry. Boundary samples intentionally remain unavailable
    so they cannot bias the source-selection median.
    """

    if len(projected) < 3:
        return [None] * len(projected)

    times_ms = [item[0].t_ms for item in projected]
    half_window_ms = max(250, int(round(max(0.5, float(window_s)) * 500.0)))
    estimates: list[float | None] = [None] * len(projected)
    for idx, (sample, _, _) in enumerate(projected):
        left = bisect_left(times_ms, sample.t_ms - half_window_ms)
        right = bisect_right(times_ms, sample.t_ms + half_window_ms) - 1
        if left >= idx or right <= idx:
            continue
        dt_s = (times_ms[right] - times_ms[left]) / 1000.0
        if dt_s < 0.5 or dt_s > 10.0:
            continue
        _, x0, y0 = projected[left]
        _, x1, y1 = projected[right]
        speed = math.hypot(x1 - x0, y1 - y0) / dt_s
        if math.isfinite(speed):
            estimates[idx] = max(0.0, float(speed))
    return estimates


def _choose_speed_source(
    *,
    ...
    """Choose coordinate speed only when the disagreement is unambiguous."""

    if reported_median <= 1.0:
        return "coordinate" if coordinate_median > 1.0 else "reported"
    required_coordinate_samples = max(20, int(math.ceil(total_sample_count * 0.5)))
    if coordinate_sample_count < required_coordinate_samples or coordinate_median <= 5.0:
        return "reported"
    ratio = max(reported_median, coordinate_median) / max(
        1e-6,
        min(reported_median, coordinate_median),
    )
    if ratio >= 2.0 and abs(reported_median - coordinate_median) >= 5.0:
        return "coordinate"
    return "reported"
...
def _analyze_agent(samples: list[AgentSample]) -> dict[str, Any]:
    ...
    if len(projected) < 3:
        return {
            ...
            "reported_median_speed_mps": 0.0,
            "coordinate_median_speed_mps": 0.0,
            "usable": False,
        }

    coordinate_ground_speeds = _coordinate_ground_speeds(projected)
    reported_speed_values = [
        float(sample.speed)
        for sample, _, _ in projected
        if math.isfinite(float(sample.speed)) and float(sample.speed) > 1.0
    ]
    coordinate_speed_values = [
        float(speed)
        for speed in coordinate_ground_speeds
        if speed is not None and math.isfinite(float(speed)) and float(speed) >= 0.0
    ]
    reported_median_speed = _median(reported_speed_values, 0.0)
    coordinate_median_speed = _median(coordinate_speed_values, 0.0)
    speed_source = _choose_speed_source(
        reported_median=reported_median_speed,
        coordinate_median=coordinate_median_speed,
        coordinate_sample_count=len(coordinate_speed_values),
        total_sample_count=len(projected),
    )
    ...
    for idx in range(1, len(projected)):
        ...
        reported_speed = max(0.0, (float(prev.speed) + float(curr.speed)) * 0.5)
        coordinate_candidates = [
            value
            for value in (coordinate_ground_speeds[idx - 1], coordinate_ground_speeds[idx])
            if value is not None and math.isfinite(float(value))
        ]
        coordinate_speed = _median(coordinate_candidates, ground_speed)
        if speed_source == "coordinate":
            speed = coordinate_speed if coordinate_speed > 1.0 else ground_speed
            if speed <= 1.0 and reported_speed > 1.0:
        ...
        if abs(yaw_rate) >= 0.25:
            radius = turn_radius_from_rate_m(speed, yaw_rate)
            if radius is None:
                continue
            if 40.0 <= radius <= 8000.0:
                radii.append(radius)
                bank = bank_angle_for_turn_radius_deg(speed, radius)
                if bank is None:
                    continue
                if 0.0 <= bank <= 80.0:
                    banks.append(bank)
    ...
    return {
        ...
        "reported_median_speed_mps": reported_median_speed,
        "coordinate_median_speed_mps": coordinate_median_speed,
    ...
    return (
        {
            ...
            "coordinated_turn_rate_scale": 1.0,
...
def _build_proposal(summary: dict[str, Any], source_path: Path) -> dict[str, Any]:
    ...
    dynamics_params, dynamics_basis = _derive_turn_dynamics(
    ...
    current_ref_radius = dynamics_basis["current_reference_radius_m"]
    radius_scale = dynamics_basis["reference_turn_radius_scale"]

    aircraft_overrides: dict[str, dict[str, Any]] = {}
    for aircraft_id, metrics in sorted((summary.get("agents") or {}).items()):
        if not isinstance(metrics, dict) or not bool(metrics.get("usable")):
            continue
        agent_params, agent_basis = _derive_turn_dynamics(
            median_speed=float(metrics.get("median_speed_mps", 40.0) or 40.0),
            median_radius=float(metrics.get("median_turn_radius_m", 450.0) or 450.0),
            yaw_p95=float(metrics.get("abs_yaw_rate_p95_dps", 4.5) or 4.5),
            bank_p95=float(metrics.get("bank_proxy_p95_deg", 25.0) or 25.0),
            bank_p50=float(metrics.get("bank_proxy_p50_deg", 18.0) or 18.0),
            reversal_gap=float(metrics.get("opposite_turn_gap_p50_s", 0.0) or 0.0),
        )
        aircraft_overrides[str(aircraft_id)] = {
            "params": agent_params,
            "basis": agent_basis,
        }
    ...
    return {
        ...
        "dynamics": {
            ...
            "aircraft_overrides": aircraft_overrides,
        },
        "notes": notes,
        "basis": {
            "median_speed_mps": median_speed,
...
def _sanitize_dynamics(raw: dict[str, Any]) -> dict[str, Any]:
    return {
        "banked_turn_enabled": _bool(raw.get("banked_turn_enabled"), True),
        "bank_yaw_rate_blend": _clamp(
            _finite_num_or(raw.get("bank_yaw_rate_blend"), 0.85),
            0.0,
            1.0,
        ),
        "coordinated_turn_rate_scale": _clamp(
            _finite_num_or(raw.get("coordinated_turn_rate_scale"), 1.0),
            0.25,
            3.0,
        ),
        "reference_turn_radius_scale": _clamp(
            _finite_num_or(raw.get("reference_turn_radius_scale"), 1.0),
            0.25,
            3.0,
        ),
        "max_yaw_rate_dps": _clamp(_finite_num_or(raw.get("max_yaw_rate_dps"), 30.0), 0.5, 90.0),
        "max_roll_rate_dps": _clamp(_finite_num_or(raw.get("max_roll_rate_dps"), 40.0), 1.0, 120.0),
        "turn_bank_limit_deg": _clamp(_finite_num_or(raw.get("turn_bank_limit_deg"), 55.0), 1.0, 80.0),
        "roll_limit_deg": _clamp(_finite_num_or(raw.get("roll_limit_deg"), 60.0), 1.0, 89.0),
        "turn_roll_gain": _clamp(_finite_num_or(raw.get("turn_roll_gain"), 2.2), 0.1, 10.0),
        "use_reference_turn_radius": _bool(raw.get("use_reference_turn_radius"), True),
    }
...
def apply_proposal(proposal: dict[str, Any]) -> dict[str, Any]:
    ...
    aircraft_overrides: dict[str, dict[str, Any]] = {}
    raw_overrides = dynamics.get("aircraft_overrides")
    if isinstance(raw_overrides, dict):
        for aircraft_id, raw_override in raw_overrides.items():
            if not isinstance(raw_override, dict):
                continue
            raw_override_params = (
                raw_override.get("params")
                if isinstance(raw_override.get("params"), dict)
                else raw_override
            )
            try:
                normalized_id = str(int(aircraft_id))
            except Exception:
                continue
            aircraft_overrides[normalized_id] = {
                "params": _sanitize_dynamics(raw_override_params),
                "basis": raw_override.get("basis")
                if isinstance(raw_override.get("basis"), dict)
                else {},
            }
