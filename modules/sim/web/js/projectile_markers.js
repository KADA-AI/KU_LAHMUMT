...
const buildVisualData = (map, projectiles) => {
  ...
  projectiles.forEach((proj) => {
    ...
    const altitude = Number.isFinite(alt) ? alt + ALT_OFFSET_M : terrainElevation(map, lon, lat) + ALT_OFFSET_M;
    const headMerc = toMercator(lon, lat, altitude);
    const groundMerc = toMercator(lon, lat, terrainElevation(map, lon, lat) + GROUND_OFFSET_M);
    ...
    if (altitude - (terrainElevation(map, lon, lat) + GROUND_OFFSET_M) > 35) {
