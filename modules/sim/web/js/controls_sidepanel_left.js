...
const orderQuad = (points) => {
  ...
  const tl = points.reduce((best, point) => (!best || (point.x + point.y) < (best.x + best.y) ? point : best), null);
  const br = points.reduce((best, point) => (!best || (point.x + point.y) > (best.x + best.y) ? point : best), null);
  const tr = points.reduce((best, point) => (!best || (point.x - point.y) > (best.x - best.y) ? point : best), null);
  const bl = points.reduce((best, point) => (!best || (point.x - point.y) < (best.x - best.y) ? point : best), null);
...
const drawCameraFeed = (canvas, view, sourceMap) => {
  ...
  drawBackdrop(ctx, color);
  const quadCss = orderQuad(projectedRing);
  if (!quadCss) {
...
export const initLeftSidePanel = ({
  ...
  const syncDocking = () => {
    const bottoms = [16];
    if (monitoringCard) {
      bottoms.push(monitoringCard.getBoundingClientRect().bottom + 14);
    }
    if (leftControls) {
  ...
  const resizeTargets = [leftControls, monitoringCard, panel].filter(Boolean);
