...
const AUTO_HANDLED_0503_STORAGE_KEY = "sim.autoMissionFlow.lastHandled0503";
...
export const initMissionRecommendPopup = () => {
  ...
  const scheduleAutoAdvance = (payload) => {
    clearAutoAdvance();
    const recVal = toInt(pickValue(payload, ["systemRecommend", "SystemRecommend"]), null);
    if (recVal !== 1 || !isAutoMissionFlowEnabled()) {
      return;
    }
    ...
    const payloadKey = `${timestamp ?? "?"}|${recVal}`;
    try {
      if (window.localStorage.getItem(AUTO_HANDLED_0503_STORAGE_KEY) === payloadKey) {
        return;
      }
    } catch (err) {
      // Storage is optional; the in-page lastKey guard still prevents duplicates.
    }
    autoAdvanceTimer = window.setTimeout(async () => {
      ...
      try {
        window.localStorage.setItem(AUTO_HANDLED_0503_STORAGE_KEY, payloadKey);
      } catch (err) {
        // Storage is optional.
      }
      logStatus("자동 진행: 다음 협업기저임무 수행", { level: "info", ttlMs: 3500 });
      const sent = await handleAction(1);
      if (!sent) {
        try {
          if (window.localStorage.getItem(AUTO_HANDLED_0503_STORAGE_KEY) === payloadKey) {
            window.localStorage.removeItem(AUTO_HANDLED_0503_STORAGE_KEY);
          }
        } catch (err) {
          // Storage is optional.
        }
  ...
  window.addEventListener("sim:auto-mission-flow", (event) => {
    if (event?.detail?.enabled && lastPayload) {
      scheduleAutoAdvance(lastPayload);
    } else {
      clearAutoAdvance();
    }
  });
