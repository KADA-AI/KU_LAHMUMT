...
// Target effects interpolate above the 5 Hz data stream, but do not need to
// force the 3-D terrain to repaint at 24 FPS.
const TARGET_RENDER_FPS = 5;
const TARGET_SAMPLE_DURATION_MS = 0;
...
const TYPE_CONFIG = {
  0: {
    ...
    parts: [
      { shape: "circle", forward: 0, lateral: 0, radius: 5.4, sides: 18, height: 0.8, base: 0.0, color: "body" },
      { shape: "rect", forward: 0, lateral: 0, length: 12.4, width: 1.2, height: 1.2, base: 0.75, color: "accent" },
      { shape: "rect", forward: 0, lateral: 0, length: 1.2, width: 12.4, height: 1.2, base: 0.75, color: "accent" },
    ],
  },
  1: {
    label: TYPE_LABELS[1],
...
const headingToMathRad = (headingDeg) => ((90 - headingDeg) * Math.PI) / 180;
...
const normalizeTarget = (raw) => {
  ...
  const kind = String(raw?.kind || "").toLowerCase() === "roi" ? "roi" : "target";
  const typeId = kind === "roi" ? 0 : Number(raw?.type);
...
const makeTargetMap = (targets) => {
  ...
  targets.forEach((target) => {
    ...
    map.set(target.key || target.id, target);
  });
  return map;
};
...
export const initTargetMarkers = (map) => {
  ...
  let lastFrameMs = 0;
  let lastTargetPayloadSignature = null;
  ...
  const flushScheduledSourceCommit = () => {
    sourceCommitTimer = null;
    if (!mapReady || !pendingFeatureCollection) {
      return;
    }
    ensureLayer();
    ...
    source.setData(pendingFeatureCollection);
    pendingFeatureCollection = null;
  };

  const scheduleSourceCommit = (featureCollection) => {
  ...
  const applyFeatures = (targets, frameTimeMs) => {
    ...
    scheduleSourceCommit({ type: "FeatureCollection", features });
  };
  ...
  const renderFrame = (frameTimeMs) => {
    ...
    const dynamicActive = renderedTargets.some((target) => {
      ...
      return Number.isFinite(fireAge) && fireAge < RECENT_FIRE_WINDOW_S;
    });
  ...
  const loadFromReference = (payload) => {
    ...
    const rawRois = Array.isArray(payload?.rois) ? payload.rois : [];
    let payloadSignature = null;
    try {
      payloadSignature = JSON.stringify([rawTargets, rawRois]);
    } catch (_err) {
      payloadSignature = null;
    }
    if (payloadSignature !== null && payloadSignature === lastTargetPayloadSignature) {
      return;
    }
    lastTargetPayloadSignature = payloadSignature;
    const normalizedTargets = rawTargets.concat(rawRois).map(normalizeTarget).filter(Boolean);
    latestTargets = normalizedTargets;
    notifySubscribers();
    const timestampMs = nowMs();
    const authoritativeTargets = makeTargetMap(normalizedTargets);
    fromTargets = authoritativeTargets;
    toTargets = authoritativeTargets;
