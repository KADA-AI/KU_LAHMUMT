...
const AUTO_HANDLED_0701_STORAGE_KEY = "sim.autoMissionFlow.lastHandled0701";
  ...
  try {
    const response = await fetch(PLAN_LOAD_ENDPOINT, {
      ...
      body: JSON.stringify({
        ...
        skipIfMissionPlanAlreadyLoaded: String(reason || "") === "0903",
      }),
    });
    const data = await response.json();
    if (data?.deferred) {
      logStatus(`Mission ${id} 새 계획 검증 대기 중`, {
        level: "info",
        ttlMs: 2500,
      });
      return false;
...
export const initMissionOptionPopup = () => {
  ...
  const isAutoMissionFlowEnabled = () => {
    const toggle = document.getElementById("auto-mission-flow-toggle");
    if (toggle) {
      return toggle.getAttribute("aria-pressed") === "true";
    }
    return Boolean(window.simAutoMissionFlow || window.simAutoApproveFirstOption);
  };

  const autoApproveFirstOption = (options, optionKey) => {
    if (!isAutoMissionFlowEnabled()) {
      return;
    }
    try {
      if (optionKey && window.localStorage.getItem(AUTO_HANDLED_0701_STORAGE_KEY) === optionKey) {
        return;
      }
    } catch (err) {
      // Storage is optional; the in-page last0701Key guard still prevents duplicates.
    }
    ...
    autoApproveTimer = window.setTimeout(async () => {
      if (pending || !isAutoMissionFlowEnabled()) {
        return;
      }
      pending = true;
      ...
      try {
        if (optionKey) {
          window.localStorage.setItem(AUTO_HANDLED_0701_STORAGE_KEY, optionKey);
        }
      } catch (err) {
        // Storage is optional.
      }
      ...
      if (sent) {
        ...
        try {
          if (
            optionKey &&
            window.localStorage.getItem(AUTO_HANDLED_0701_STORAGE_KEY) === optionKey
          ) {
            window.localStorage.removeItem(AUTO_HANDLED_0701_STORAGE_KEY);
          }
        } catch (err) {
          // Storage is optional.
        }
  ...
  const handle0701 = (payload) => {
    ...
    autoApproveFirstOption(options, key);
  };
  ...
  const handle0903 = async (payload) => {
    ...
    last0903Key = key;
    setOpen(false);
    const loaded = await loadMissionPlan(missionPlanId, "0903", true);
    if (!loaded && last0903Key === key) {
      last0903Key = null;
    }
  };
