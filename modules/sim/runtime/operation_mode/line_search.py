...
class ModeLineSearch(OperationMode):
    ...
    def _geometry_signature(self, line_search: dict) -> tuple[tuple[float, float, float], ...]:
        ...
        for coord in line_search.get("coordinateList") or []:
            ...
            if lon is None or lat is None:
                continue
            try:
                signature.append((float(lon), float(lat), float(alt or 0.0)))
    ...
    def apply(
        ...
        *,
        ...
        geometry_signature = self._geometry_signature(line_search)
        state = prev_state

        if (
            not isinstance(state, dict)
            or state.get("wp_id") != current_wp_id
            or state.get("geometry_signature") != geometry_signature
        ):
        ...
        # A preserve-state mission reload may update speed or FOV while keeping
        # the same scan geometry. Continue from the current segment in that case.
        state["speed"] = float(line_search.get("searchSpeed", 0.0) or 0.0)
        ...
        if seg_len > 1e-6:
            debug = {
                "sweepDirection": [
                    float((p1[0] - p0[0]) / seg_len),
                    float((p1[1] - p0[1]) / seg_len),
                    float((p1[2] - p0[2]) / seg_len),
                ],
                "segmentIndex": int(seg_idx),
                "segmentT": float(seg_t),
            }

        return OperationResult(target=target, state=state, debug=debug)
