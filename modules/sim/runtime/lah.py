...
class LAHOperationalEnvelope:
    ...
    # Must match the planner envelope in
    # modules/mission_planning/MissionPlanner/dynamics/lah_op_envlp.py.
    # A lower value here rejects plans the planner considers valid: firing
    # from concealment needs whatever altitude clears the ridge, measured at
    # 2250-2990 m on this terrain, and 2000 m failed those plans on apply.
    max_altitude_m: float = 3500.0
...
class LAHParams:
    ...
    hover_blend_speed_mps: float = 18.0
    hover_drag_mps2: float = 8.0
...
class LAH:
    ...
    def step(self, dt: float):
        ...
        throttle_cmd = clamp(self.cmd_throttle, -1.0, 1.0)
        a = self.p.accel * throttle_cmd
        self.s.u = clamp(self.s.u + a * dt, self.p.min_speed, self.p.max_speed)
        if self.hover_mode and self.s.u > 0.0:
            self.s.u = max(0.0, self.s.u - self.p.hover_drag_mps2 * dt)
        ...
        vx = self.s.u * cp * cy
        vy = -self.s.u * cp * sy
        pitch_vz = self.s.u * sp
        hover_blend = 0.0
        if self.hover_mode:
            hover_blend = clamp(1.0 - self.s.u / max(1.0, self.p.hover_blend_speed_mps), 0.0, 1.0)
        hover_vz = clamp(self.cmd_vertical_speed, -self.p.max_vertical_speed_mps, self.p.max_vertical_speed_mps)
        vz = pitch_vz * (1.0 - hover_blend) + hover_vz * hover_blend
