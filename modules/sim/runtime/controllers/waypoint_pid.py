...
# Fly-Over(waypointPassType=3) 통과 판정 반경 [m].
# 기준: 최대 속도에서의 최소 선회 반경(560m). 판정은 "waypoint 수직 판정선 통과 +
# waypoint로부터 이 반경 이내"로 이루어진다 (2026-07 실기체 협의 스펙).
FLYOVER_PASS_RADIUS_M = float(interpolate_reference_turn_radius(50.0))
...
class WaypointTarget:
    ...
    individual_mission_type: int | None = None
    pattern_type: int | None = None
    path_id: int | None = None
    pass_type: int | None = None
    sep_m: float | None = None
    # Next-collaboration Area missions may carry two independently managed
    # coverage passes.  Keep their plan metadata on the runtime target so the
    # SIM frame/UI can report whether the aircraft is outbound, returning, or
    # traversing the turn corridor between them.
    area_coverage_pass: str | None = None
    area_coverage_turn_phase: str | None = None
...
class WaypointPIDController:
        ...
        *,
        ...
        for wp in waypoints:
            if isinstance(wp, WaypointTarget):
                if (
                    str(wp.area_coverage_pass or "").strip().lower() == "reverse"
                    or str(wp.area_turn_role or "").strip().lower()
                    == "reciprocal_turn"
                ):
                    continue
                ...
                assignment_completion_seconds = wp.get("assignment_completion_seconds")
                # Legacy logs can still contain the retired AREA return pass.
                # The SIM must execute only the first capture traversal.
                if (
                    str(area_coverage_pass or "").strip().lower() == "reverse"
                    or str(area_turn_role or "").strip().lower()
                    == "reciprocal_turn"
                ):
                    continue
                if isinstance(pos, (list, tuple)) and len(pos) == 3:
                    self.targets.append(
                        WaypointTarget(
                            ...
                            input_mission_type=int(input_mission_type)
                            if input_mission_type is not None
                            else None,
                            ...
                            individual_mission_type=int(individual_mission_type)
                            if individual_mission_type is not None
                            else None,
                            pattern_type=int(pattern_type) if pattern_type is not None else None,
                            ...
                            area_coverage_pass=(
                                str(area_coverage_pass) if area_coverage_pass is not None else None
                            ),
                            area_coverage_turn_phase=(
                                str(area_coverage_turn_phase)
                                if area_coverage_turn_phase is not None
                                else None
                            ),
                            area_turn_role=str(area_turn_role) if area_turn_role is not None else None,
                            complete_loiter_after_assignment=complete_loiter_after_assignment,
                            assignment_completion_seconds=(
                                float(assignment_completion_seconds)
    ...
    def _assignment_loiter_duration_s(self, target: WaypointTarget) -> float | None:
        if not bool(getattr(target, "complete_loiter_after_assignment", False)):
            return None
        try:
            explicit_duration = float(
                getattr(target, "assignment_completion_seconds", None) or 0.0
            )
        except Exception:
        ...
        if not isinstance(loiter_prop, dict):
            return None
        try:
            duration = float(loiter_prop.get("time", 0.0) or 0.0)
        except Exception:
            return None
    ...
    def _assignment_loiter_completed(
        self,
        ...
        *,
        ...
        """Complete a post-attack boundary hold by time since assignment.

        These coordination holds are timers, not navigation achievements.  The
        timer starts as soon as the target becomes current and therefore does
        not wait for the aircraft to cross the loiter center waypoint.
        """

        duration = self._assignment_loiter_duration_s(target)
        if duration is None or self.blocked:
            self._assignment_loiter_idx = None
            self._assignment_loiter_remaining_s = None
            return False
        if self._assignment_loiter_idx != int(self.curr_idx):
            self._assignment_loiter_idx = int(self.curr_idx)
            self._assignment_loiter_remaining_s = float(duration)
        remaining = self._assignment_loiter_remaining_s
        if remaining is None:
            remaining = float(duration)
        remaining = max(0.0, float(remaining) - max(0.0, float(dt)))
        self._assignment_loiter_remaining_s = remaining
        if remaining > 0.0:
            return False
    ...
    def _skip_done_targets(self) -> bool:
        start_idx = int(self.curr_idx)
        while self.curr_idx < len(self.targets):
            target = self.targets[self.curr_idx]
            if not bool(getattr(target, "is_done", False)):
                break
            self.curr_idx += 1
    ...
    def _advance_wp(self):
        ...
        if self.curr_idx >= len(self.targets):
            self.finished = True
            if self.hold_on_complete and self.targets and not skipped_done:
    ...
    def _apply_turn_roll_guidance(self, yaw_rate_limit: float) -> None:
        ...
        if yaw_rate_limit <= 1e-6 or bank_limit <= 0.1:
            ...
            speed_fn = getattr(uav, "horizontal_speed_mps", None)
            if callable(speed_fn):
                speed_xy = float(speed_fn())
            else:
                pitch_rad = math.radians(float(getattr(uav.s, "pitch", 0.0)))
                speed_xy = abs(float(getattr(uav.s, "u", 0.0)) * math.cos(pitch_rad))
            speed_xy = max(1.0, speed_xy)
            predicted_bank = bank_angle_for_turn_rate_deg(speed_xy, float(uav.cmd_yaw_rate))
            roll_target = clamp(float(predicted_bank or 0.0), -bank_limit, bank_limit)
    ...
    def _fixed_wing_guidance_target(
        ...
        turn_radius = turn_radius_from_rate_m(speed_xy, yaw_rate_limit) or float(gains.lookahead_m)
    ...
    def _fixed_wing_capture_radius(self, gains: PIDGains) -> float:
        if self.allow_hover:
        ...
        turn_radius = turn_radius_from_rate_m(speed_xy, yaw_rate_limit) or float(self.pos_tol)
        return max(
            float(self.pos_tol),
    ...
    def _has_line_search_action(self, target: WaypointTarget) -> bool:
        filming = target.filming if isinstance(target.filming, dict) else {}
        line_search = filming.get("lineSearch") or filming.get("LineSearch")
        if not isinstance(line_search, dict):
            return False
        coords = line_search.get("coordinateList") or line_search.get("CoordinateList")
        return isinstance(coords, list) and len(coords) >= 2

    def _has_crossed_stale_first_waypoint(
        self,
        target: WaypointTarget,
        tx: float,
        ty: float,
        ...
        """Detect a first transit WP that was passed while a new plan was in flight."""
        if self.allow_hover or self.is_loitering or self.curr_idx != 0 or len(self.targets) < 2:
            return False
        # A reciprocal Area turn is a deliberate fixed-wing reversal.  When a
        # replacement plan arrives during the outbound/turn transition, the
        # aircraft can already lie beyond the generic first-leg half-plane even
        # though it has not acquired the first compact turn gate.  Skipping that
        # gate destroys the intended turn direction and can send the return leg
        # to the wrong side of the Area, so turn gates are never stale-skipped.
        if str(target.area_turn_role or "").strip().lower() == "reciprocal_turn":
            return False
        if self._target_pass_type(target) not in (1, 3) or target.loiter or target.attack:
            return False
        if self.curr_idx in self.block_indices or self._has_line_search_action(target):
            return False
        ...
        rel_x = float(self.uav.s.x) - float(tx)
        rel_y = float(self.uav.s.y) - float(ty)
        along_m = rel_x * ux + rel_y * uy
        cross_track_m = abs(rel_x * uy - rel_y * ux)
        corridor_m = max(float(capture_radius), float(self.pos_tol) * 2.0)
        if along_m < max(5.0, float(self.pos_tol) * 0.2):
            return False
        if along_m > out_len + corridor_m or cross_track_m > corridor_m:
            return False

        # A point beyond the target plane is not enough by itself: require the
        # aircraft to be moving generally along the new outgoing leg. This
        # prevents an unrelated crossing on the opposite side from skipping a
        # legitimate first waypoint.
        speed_xy = abs(float(getattr(self.uav.s, "u", 0.0)))
        if speed_xy > 2.0:
            yaw_rad = math.radians(float(getattr(self.uav.s, "yaw", 0.0)))
            course_x = math.cos(yaw_rad)
            course_y = -math.sin(yaw_rad)
            if course_x * ux + course_y * uy < 0.20:
                return False

        next_dist_m = math.hypot(
            float(self.uav.s.x) - float(next_target.pos[0]),
            float(self.uav.s.y) - float(next_target.pos[1]),
        )
        return next_dist_m <= out_len + corridor_m

    def _can_bypass_passed_waypoint_altitude(self, target: WaypointTarget) -> bool:
        if self.allow_hover or self.is_loitering or self.curr_idx in self.block_indices:
            return False
        return self._target_pass_type(target) in (1, 3) and not target.loiter and not target.attack
    ...
    def _should_force_waypoint_advance(
        ...
        capture_radius = self._fixed_wing_capture_radius(gains)
        if self._has_crossed_stale_first_waypoint(
            target,
            tx,
            ty,
            capture_radius,
        ):
            self._forced_advance_reason = "stale_first_leg"
            return True

        if self._is_flyover_target(target):
            # Fly-Over: waypoint 수직 판정선(진입각 기준, 첫 WP는 진출각) 통과 +
            # waypoint로부터 560m(최대속도 최소선회반경) 이내면 통과로 판정한다.
            passed = dist_xy <= FLYOVER_PASS_RADIUS_M and self._has_crossed_flyover_plane(tx, ty)
            if passed:
                self._forced_advance_reason = "flyover_plane"
            return passed

        if self._closest_wp_dist_xy > capture_radius:
            return False

        grow_margin = max(4.0, capture_radius * 0.08)
        if dist_xy <= self._closest_wp_dist_xy + grow_margin:
            return False

        passed = self._has_passed_waypoint(tx, ty)
        if passed:
            self._forced_advance_reason = "passed_plane"
        return passed
    ...
    def update(self, dt: float, dem=None, wall_dt: float | None = None) -> bool:
        ...
        if self._assignment_loiter_completed(
            target,
            dt=dt,
            tx=tx,
            ty=ty,
        ...
        if self.is_loitering:
            ...
            ang_rate = turn_rate_for_radius_rad_s(speed, radius) or 0.0
        ...
        forced_advance = self._should_force_waypoint_advance(target, tx, ty, dist_xy, gains)
        reached_target = dist_xy < self.pos_tol or forced_advance
        altitude_reached = abs(dz) < self.pos_tol * 0.6
        if forced_advance and self._can_bypass_passed_waypoint_altitude(target):
            # A fixed-wing aircraft cannot correct altitude by returning to a
            # waypoint whose horizontal pass plane it has already crossed.
        ...
        if not self.allow_hover:
            # In this kinematic UAV model climb is commanded by pitch; a
            # positive altitude throttle bias is neither needed nor physical.
            # Keeping forward speed under the speed PID also avoids a hard
            # on/off acceleration at the target-speed boundary.  Otherwise a
            # 30 m/s Area capture leg can be flown near 40--50 m/s on rising
            # terrain and its lineSearch is truncated on waypoint advance.
            alt_bias = min(float(alt_bias), 0.0)
        else:
