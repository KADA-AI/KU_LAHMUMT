...
# z14 is about 7.5 m/pixel around the Inje operating area, so the 10 m
# operational DEM is preserved instead of being flattened into z8 (~482 m)
# terrain.  MapLibre still requests only the tiles in the current viewport.
DEM_MAX_ZOOM = _env_int("SIM_DEM_MAX_ZOOM", 14)
DEM_TILE_CACHE_MAX = _env_int("SIM_DEM_TILE_CACHE_MAX", 512)
...
# Authoritative browser history is intentionally 5 Hz. The flight model stays
# at 15 Hz so detection probabilities and target motion remain stable.
SIM_HISTORY_SAMPLE_HZ = _env_float("SIM_HISTORY_SAMPLE_HZ", 5.0)
SIM_HISTORY_RESPONSE_MAX = _env_int("SIM_HISTORY_RESPONSE_MAX", 120)
# 루프 1회당 서브스텝 상한 — 고배속(x8~x30)에서 8이면 backlog가 잘려나가
# sim 시간이 명령 배속보다 느리게 흐른다(더딤). CPU 여유 내에서 여유 있게.
SIM_MAX_STEPS_PER_LOOP = _env_int("SIM_MAX_STEPS_PER_LOOP", 32)
