...
from PyQt5.QtWidgets import (
    ...
    QWidget, QLabel, QHBoxLayout, QVBoxLayout, QSlider, QPushButton, QMessageBox
)
...
class MainWindow(QMainWindow):
    ...
    def __init__(self, *args, **kwargs):
        ...
        self.btn_mission_creation_gui = QPushButton("임무 생성 GUI", self)
        self.btn_mission_creation_gui.setMinimumHeight(30)
        self.btn_mission_creation_gui.clicked.connect(self._open_mission_creation_gui)
        action_row.addWidget(self.btn_mission_creation_gui)
    ...
    def _open_mission_creation_gui(self):
        try:
            import subprocess
            ...
            proc = getattr(self, "_mission_creation_process", None)
            if proc is not None and proc.poll() is None:
                webbrowser.open(url)
                self._append_log_line(f"[INFO] 임무 생성 GUI browser opened: {url}")
                return
            try:
            ...
            script = PROJECT_ROOT / "modules" / "info_manage" / "mission_creation_gui.py"
            env = os.environ.copy()
            env["PYTHONUNBUFFERED"] = "1"
            env["KU_MISSION_DB_ROOT"] = db_paths.get_active_db_root_str()
            ...
            self._mission_creation_process = proc
            QTimer.singleShot(800, lambda: webbrowser.open(url))
            self._append_log_line(f"[INFO] 임무 생성 GUI server launched: {url}")
        except Exception as exc:
            self._append_log_line(f"[ERR] 임무 생성 GUI 열기 실패: {exc}")
            try:
                QMessageBox.critical(self, "임무 생성 GUI", f"임무 생성 GUI 열기 실패:\n{exc}")
            except Exception:
                pass
    ...
    def _is_0102_push_enabled(self) -> bool:
        raw = str(os.getenv("KU_INF_0102_PUSH_ENABLED", "0") or "0").strip().lower()
    ...
    def _send_self_check_0102(self, status: int = 1, _retry: int = 0):
        if not self._is_0102_push_enabled():
            self._append_log_line("[0102] self-check push skipped (set KU_INF_0102_PUSH_ENABLED=1 to enable)")
            return
        if not self._power_on:
    ...
    def _ensure_0102(self, on: bool) -> bool:
        if on and not self._is_0102_push_enabled():
            self._append_log_line("[0102] stream disabled (set KU_INF_0102_PUSH_ENABLED=1 to enable)")
            return False
        if not self._power_on:
