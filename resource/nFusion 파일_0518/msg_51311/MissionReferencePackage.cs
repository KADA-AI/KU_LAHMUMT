...
namespace MessageLibrary.msg_51311
{
    public class MissionReferencePackage : nFusion.Model.msg_51311.MissionReferencePackage
    {
        ...
        public byte[] Serialize()
        {
            ...
            foreach (var takeOverInfo in takeOverInfoList ?? Array.Empty<TakeOverInfo>())
            {
            ...
            foreach (var handOverInfo in handOverInfoList ?? Array.Empty<HandOverInfo>())
            {
            ...
            foreach (var rtbCoordinate in rtbCoordinateList ?? Array.Empty<RTBCoordinate>())
            {
            ...
            foreach (var flightArea in flightAreaList ?? Array.Empty<FlightArea>())
            {
                bw.Write(flightArea?.areaLatLonListN ?? 0);
                foreach (var areaLatLon in flightArea?.areaLatLonList ?? Array.Empty<AreaLatLon>())
                {
            ...
            foreach (var prohibitedArea in prohibitedAreaList ?? Array.Empty<ProhibitedArea>())
            {
                bw.Write(prohibitedArea?.areaLatLonListN ?? 0);
                foreach (var areaLatLon in prohibitedArea?.areaLatLonList ?? Array.Empty<AreaLatLon>())
                {
                    bw.Write(areaLatLon?.latitude ?? 0);
                    bw.Write(areaLatLon?.longitude ?? 0);
                }
            ...
            foreach (var regionInfo in regionInfoList ?? Array.Empty<RegionInfo>())
            {
