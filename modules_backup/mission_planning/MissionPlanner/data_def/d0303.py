...
from .filming_altitude_guard import normalize_filming_target_altitudes_in_waypoints
...
try:
    from .dubins_turn_link import Point2D as _DubinsPoint2D, compute_turn_link as _compute_turn_link
except Exception:
    _DubinsPoint2D = None
...
def _ground_required_m_from_sweep_coords(
    ...
    *,
    ...
    samples: list[float] = _sample_ground_profile_along_coords(line_coords)
    if not samples and isinstance(fallback_coord, dict):
        lat = _safe_float_or_none(fallback_coord.get("latitude"))
        lon = _safe_float_or_none(fallback_coord.get("longitude"))
        if lat is not None and lon is not None:
        ...
        if formation_line_leader:
            ...
            if info.get("lineList"):
                ...
                if db_cfg:
                    ...
                    mission_fov_deg = _apply_runtime_adjusted_db_fov(float(db_cfg["fov"]))
            ...
            # (ii) areaList
                ...
                if mission_pattern_type != 3 and runtime_area_auto_fov_from_db:
                    ...
                    if area_db_meta:
                        ...
                        mission_fov_deg = _apply_runtime_adjusted_db_fov(
            ...
            if base and len(base) >= 2:
                ...
                try:
                    line_route_offset_scale = max(float(LINE_ROUTE_OFFSET_SCALE), 0.0)
                except Exception:
                    line_route_offset_scale = 1.0
                for idx in sweep_indices:
                    ...
                    if use_centerline:
                        anchor_xy = _line_wp_xy_from_sweep(
                            sw,
                            offset_m=float(mission_sep_m) * float(line_route_offset_scale),
                        )
    ...
    if formation_followers:
        ...
        for fkey, fpkt in formation_followers:
            ...
            if leader_wplist:
                ...
                _strip_filming_properties(fpkt["wplist"])
            ...
            if first_coords and records and not is_area_pkt:
                ...
                for g_idx, group in enumerate(groups):
                    ...
                    if g_idx == 0 and fold_close_takeover_first_sweep_into_next_group:
                        folded_coords = deepcopy(folded_first_sweep_coords or first_coords)
                        ...
                        initial_group_coords = deepcopy(folded_first_sweep_coords or first_coords)
                ...
                if LINE_FIRST_SWEEP_DENSIFY_ENABLED and _first_sweep_wp is not None:
                    ...
                    if _fs_interp >= 2 and len(_fs_coords) >= 2 * _fs_interp:
                        ...
                        for _li in range(len(_normalized)):
                            _all_normalized.append(deepcopy(_normalized[_li]))
                        ...
                        for _out_idx, _nl in enumerate(_all_normalized):
                            if _out_idx % 2 == 0:
                                _dense_coords_out.extend(deepcopy(_nl))
                            else:
                                _dense_coords_out.extend(deepcopy(list(reversed(_nl))))
        ...
        for fkey, fpkt in formation_followers:
            ...
            if leader_wplist:
                ...
                if follower_aid in (4, 5, 6):
                    ...
                    _align_point_anchor_altitude_with_search_waypoints(fpkt["wplist"])
                    _enforce_waypoint_altitude_rate_limit_inplace(
                        fpkt["wplist"],
                        vertical_rate_mps=_runtime_uav_climb_rate_mps(),
