...
from ._constants import (
    ...
    _MISSION_PLANNER_DIR, _PROJECT_ROOT, _FOV_DB_PATH,
    ...
    get_runtime_area_review_max_segment_m, get_runtime_str,
...
class DivisionPlannerWindow(QMainWindow):
    ...
    def _fov_db_rows(self) -> List[Dict[str, float]]:
        ...
        try:
            ...
            print(f"[FOV_DB] LOAD FAILED: {_db_err} | path={_FOV_DB_PATH}")
            widths = []
            rows = []

        print(f"[FOV_DB] loaded {len(rows)} rows, {len(set(widths))} unique widths, path={_FOV_DB_PATH}")
    ...
    def _mid_line_overlay_bundle(
        ...
        for piece in pieces:
            ...
            overlay_row["widthStartM"] = float(
                geometry.get("maxWidthM", 0.0)
                or geometry.get("widthM", 0.0)
                or 0.0
            )
    ...
    def _build_assignment_path_1_row(
        ...
        *,
        ...
        db_row = self._largest_sep_covering_db_row(part_width_m)
    ...
    def _build_check_mission_row(
        ...
        face_points = self._make_path_face_points(part_overlay, tangent_point_xy)
        ...
        db_row = self._largest_sep_covering_db_row(near_width_m)
                    ...
                    for aid in self.state.uav_ids:
                        if uav_overrides and int(aid) in uav_overrides:
                            ...
                            if uav_state is None:
                                ...
                                continue
                            origin_xy, heading_deg = uav_state
                            override_radius = TURN_PREVIEW_RADIUS_M
            ...
            for candidate in sorted(best_records, key=lambda row: int(row.get("aircraftID", 0) or 0)):
                ...
                if path_row is not None:
                    ...
                    if (
                        ...
                        if sep_cand > db_max_sep and db_max_sep > 0.0:
                            # Need Entry T': advance along ingress (T→FA) until Sep_cand ≈ db_max_sep
                            ...
                            if ingress_len > 1e-6:
                                ...
                                # Binary search for the point where sep_cand ≈ target_sep (with margin)
                                target_sep = db_max_sep * 0.92
                        ...
                        # FOV/vel determination: DB rows where width < part_width AND sep >= sep_cand → max fov
                        # (longer sep needs narrower fov; pick widest fov that still covers the distance)
                        # DB tuple order: (width, sep, vel, fov)
                        resolved_fov = None
                        resolved_vel = None
                        if db_rows and part_width_m > 0.0:
                            matching = [
                                r for r in db_rows
                                if float(r[0]) < part_width_m and float(r[1]) >= sep_cand
                            ]
                            if matching:
                                best = max(matching, key=lambda r: float(r[3]))
                                resolved_fov = float(best[3])
                                resolved_vel = float(best[2])
                            else:
                                # Fallback: no DB row covers sep_cand → use fov of the max sep row (with width filter)
                                width_matching = [r for r in db_rows if float(r[0]) < part_width_m]
                                if width_matching:
                                    max_sep_row = max(width_matching, key=lambda r: float(r[1]))
                                    resolved_fov = float(max_sep_row[3])
                                    resolved_vel = float(max_sep_row[2])
    ...
    def _assignment_next_mission(self) -> None:
        ...
        try:
            ...
            for row in prev_paths:
                ...
                turn_radius_m = _turn_radius_for_speed(vel_mps)
            ...
            for candidate in sorted(best_records, key=lambda r: int(r.get("aircraftID", 0) or 0)):
                ...
                if (
                    ...
                    resolved_vel = None
                    if db_rows and part_width_m > 0.0:
                        matching = [r for r in db_rows if float(r[0]) < part_width_m and float(r[1]) >= sep_cand]
                        if matching:
                            best = max(matching, key=lambda r: float(r[3]))
                            resolved_fov = float(best[3])
                            resolved_vel = float(best[2])
        ...
        try:
            ...
            for overlay in self.state.mid_line_segments:
                ...
                if (
                    ...
                    # FA (t0TargetXY) is the far midline endpoint.
                    # If FA ≈ ml1 (max_s side), near face = corners[0]→corners[3] (min_s side)
                    # If FA ≈ ml0 (min_s side), near face = corners[1]→corners[2] (max_s side)
                    if len(corners) >= 4:
                ...
                # --- FOV / vel determination (same logic as split mode) ---
                resolved_fov: Optional[float] = None
                resolved_vel: Optional[float] = None
                if db_rows and width_m > 0.0:
                    matching = [
                        r for r in db_rows
                        if float(r[0]) < width_m and float(r[1]) >= sep_cand
                    ]
                    if matching:
                        best = max(matching, key=lambda r: float(r[3]))
                        resolved_fov = float(best[3])
                        ...
                        width_matching = [r for r in db_rows if float(r[0]) < width_m]
                        if width_matching:
                            max_sep_row = max(width_matching, key=lambda r: float(r[1]))
                            resolved_fov = float(max_sep_row[3])
                            resolved_vel = float(max_sep_row[2])
    ...
    def _make_sweep(self) -> None:
        ...
        try:
            ...
            for path_row in path_0_rows:
                ...
                # ---- resolve foot from FOV DB (same matching as make_path_0) ----
                part_width_m = float(path_row.get("partWidthM", 0.0) or 0.0)
                ...
                if db_rows and part_width_m > 0.0:
                    matching = [
                        r for r in db_rows
                        if float(r.get("width", 0.0) or 0.0) < part_width_m
                        and float(r.get("sep", 0.0) or 0.0) >= sep_cand_m
                    ]
                    if matching:
                        best = max(matching, key=lambda r: float(r.get("fov", 0.0) or 0.0))
                        foot_m = float(best.get("foot", 0.0) or 0.0)
                    else:
                        width_matching = [
                            r for r in db_rows
                            if float(r.get("width", 0.0) or 0.0) < part_width_m
                        ...
                        if width_matching:
                            best = max(width_matching, key=lambda r: float(r.get("sep", 0.0) or 0.0))
                            foot_m = float(best.get("foot", 0.0) or 0.0)
                if foot_m <= 0.0:
                    lines.append(f"  UAV{aid}/P{piece_idx}: foot 결정 불가, skip")
                    continue

                # ---- build actual piece polygon for clipping ----
                piece_poly: Optional[Polygon] = None
                if self.state.split_result is not None:
                    for piece in self.state.split_result.pieces:
                        if (
                            ...
                            piece_coords_xy = coords_to_xy(
                                (piece.data or {}).get("coordinateList", [])
                            )
                            if len(piece_coords_xy) >= 3:
                                piece_poly = Polygon(piece_coords_xy)
                ...
                # ---- sweep lines: ingress 방향으로 간격, 각 라인은 t축(가로) 방향 ----
                sweep_step_m = foot_m * 0.80  # 20% 오버랩 버퍼

                # take over list 시작인 경우: 첫번째 WP까지 2배 촘촘
                is_takeover = bool(path_row.get("entryTPrimeXY") is not None)
                dense_first_step = sweep_step_m * 0.5 if is_takeover else sweep_step_m
                ...
                lines.append(
                    f"  UAV{aid}/P{piece_idx}: foot={foot_m:.1f}m (step={sweep_step_m:.1f}m, 2%overlap)"
    ...
    def _make_sweep_2(self) -> None:
        ...
        try:
            ...
            for path_row in sweep_targets:
                ...
                # ---- sweep lines: ingress 방향 간격, 20% 오버랩 ----
                sweep_step_m = foot_m * 0.80
                ...
                lines.append(
                    ...
                    f" foot={foot_m:.1f}m (step={sweep_step_m:.1f}m)"
                    f" | L={total_length:.0f}m → {n_lines} sweep lines"
                    f" | clipped={'Y' if piece_poly else 'N'} | overlap=20%"
                )
