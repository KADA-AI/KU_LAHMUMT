...
class DivisionPlannerWindow(QMainWindow):
    ...
    def _turn_radius_scale_value(self) -> float:
        try:
            ...
            scale = 0.0
        if scale <= 0.0:
            scale = float(get_runtime_float("next_collab_turn_radius_scale", 1.40))
    ...
    def _next_collab_area_search_speed_scale(self) -> float:
        value = float(get_runtime_float("area_search_speed_weight", 1.0))
        return max(0.10, min(5.0, value))
    ...
    def _next_collab_area_sweep_spacing_m(
        ...
        footprint_m = self._next_collab_area_footprint_m(sep_m, fov_deg)
        if footprint_m is None or footprint_m <= 0.0:
            return None
        try:
            density = float(get_runtime_float("area_density_scale", 1.2))
        except Exception:
            density = 1.2
    ...
    def _next_collab_resolved_db_row(
        ...
        candidates = [
            dict(row)
            for row in self._fov_db_rows()
            if float(row.get("width", 0.0) or 0.0) < target_m
        ]
        if not candidates:
            fallback_rows = _cached_fov_db_rows()
            candidates = [
            ...
            for candidate in sorted(best_records, key=lambda row: int(row.get("aircraftID", 0) or 0)):
                ...
                if path_row is not None:
                    ...
                    if (
                        ...
                        resolved_vel = None
                        resolved_foot = None
                        resolved_db_row = self._next_collab_resolved_db_row(part_width_m, sep_cand)
                        if isinstance(resolved_db_row, dict):
                            try:
                                resolved_fov = float(resolved_db_row.get("fov", 0.0) or 0.0)
                            except Exception:
                                resolved_fov = None
                            try:
                                resolved_vel = float(resolved_db_row.get("vel", 0.0) or 0.0)
                            except Exception:
                                resolved_vel = None
                            try:
                                resolved_foot = float(resolved_db_row.get("foot", 0.0) or 0.0)
                            except Exception:
                                resolved_foot = None
                            path_row["dbSepM"] = float(resolved_db_row.get("sep", 0.0) or 0.0)
                            path_row["dbWidthM"] = float(resolved_db_row.get("width", 0.0) or 0.0)
                            if resolved_fov is not None and resolved_fov <= 0.0:
                                resolved_fov = None
                            if resolved_vel is not None and resolved_vel <= 0.0:
                                resolved_vel = None
                            if resolved_foot is not None and resolved_foot <= 0.0:
                                resolved_foot = None
                        sep_ref_m = float(sep_cand) if float(sep_cand) > 0.0 else float(path_row.get("dbSepM", 0.0) or 0.0)
            ...
            for overlay in self.state.mid_line_segments:
                ...
                if db_rows and width_m > 0.0:
                    ...
                    if matching:
                        ...
                        resolved_foot = float(best[4]) if len(best) > 4 else None
                        resolved_db_sep = float(best[1])
                        resolved_db_width = float(best[0])
                    else:
                        ...
                        if width_matching:
                            ...
                            resolved_foot = float(max_sep_row[4]) if len(max_sep_row) > 4 else None
                            resolved_db_sep = float(max_sep_row[1])
                            resolved_db_width = float(max_sep_row[0])
                sep_ref_m = float(sep_cand) if float(sep_cand) > 0.0 else float(resolved_db_sep)
                resolved_fov_base = resolved_fov
                resolved_fov, resolved_foot = self._resolve_next_collab_area_runtime_fov_foot(
                    resolved_fov,
                    resolved_foot,
    ...
    def _make_sweep(self) -> None:
        ...
        try:
            ...
            for path_row in path_0_rows:
                ...
                sep_ref_m = sep_cand_m if sep_cand_m > 0.0 else float(path_row.get("dbSepM", 0.0) or 0.0)
                resolved_fov_deg = path_row.get("resolvedFovDeg")
                foot_m = float(self._next_collab_area_footprint_m(sep_ref_m, resolved_fov_deg) or 0.0)
                if foot_m <= 0.0:
                ...
                sweep_step_m = float(self._next_collab_area_sweep_spacing_m(sep_ref_m, resolved_fov_deg) or 0.0)
                if sweep_step_m <= 0.0:
