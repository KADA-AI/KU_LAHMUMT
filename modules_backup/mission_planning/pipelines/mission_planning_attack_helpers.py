    ...
    if (
        ...
        try:
            ...
            if result.returncode == 0:
                ...
                selected = dict(selection.get("coordinate") or fallback)
                _safe_emit(
                    log_cb,
                    f"[variant {variant_no}] attack point selected mode={selection.get('mode')} "
                    f"currentDist={selection.get('current_distance_m')}m "
