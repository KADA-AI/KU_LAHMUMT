...
except Exception:  # pragma: no cover - logAnalyzer should still run standalone.
...
def _analyze_agent(samples: list[FlightSample]) -> dict[str, Any]:
    ...
    for idx in range(1, len(projected)):
        ...
        if speed > 5.0 and abs(yaw_rate) >= 0.2:
            candidate = speed / max(1e-6, math.radians(abs(yaw_rate)))
            if 25.0 <= candidate <= 20_000.0:
                ...
                bank = math.degrees(math.atan((speed * speed) / max(1e-6, G * candidate)))
                if 0.0 <= bank <= 85.0:
