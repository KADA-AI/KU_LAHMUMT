...
def _planner_entry_lead_time_s() -> float:
    try:
        value = float(get_runtime_float("next_collab_entry_lead_time_s", ENTRY_LEAD_TIME_S))
    except Exception:
        value = float(ENTRY_LEAD_TIME_S)
    ...
    def on_execute_next(
        ...
        *,
        ...
        for aircraft_id in context.target_aircraft_ids:
            ...
            sanitized_coord = _sanitize_entry_altitude(coord, altitude_reference)
            if sanitized_coord is not None and sanitized_coord != coord:
                old_alt = coord.get("altitude") if isinstance(coord, dict) else None
                new_alt = sanitized_coord.get("altitude")
                logs.append(
                    f"[0803] execute=1 aircraft {aircraft_id}: entry altitude sanitized "
                    f"{old_alt if old_alt is not None else 'n/a'} -> {new_alt}"
                )
                coord = sanitized_coord
        ...
        detail_payload: dict[str, Any] = {
            ...
            "entryLeadTimeS": float(turn_projection_lead_s),
