...
class ForcedCommandReplanCoordinator:
    ...
    def on_forced_command(
        ...
        *,
        ...
        was_forced_unavailable = int(aircraft_id) in self._state.forced_unavailable_aircraft
        ...
        # mandatoryType=3 -> if the aircraft had already been excluded by a prior
        # forced hold replan, trigger an immediate collaborative rejoin replan.
        if int(mandatory_type) == 3:
            if not was_forced_unavailable:
                logs.append(
                    "[0802] mandatory resume received but aircraft was already available -> skip replan"
                    f" (aircraftID={aircraft_id})"
                )
                return [], logs
            if system_mode not in (3, 4):
                logs.append(f"[0802] resume replan skipped: mode={system_mode} (need 3/4)")
                return [], logs
            logs.append(
                "[0802] mandatory resume received -> immediate collaborative rejoin replan queued"
                f" (aircraftID={aircraft_id})"
            )
            payload_0902 = self._build_replan_payload(
                mandatory_type=int(mandatory_type),
    ...
    def _build_replan_payload(
        ...
        *,
        ...
        if not reason_text:
            reason_text = f"강제명령{int(mandatory_type)} 재계획"
        ...
        payload: dict[str, Any] = {
            ...
            "replanDetail": {
                ...
                "mandatoryType": int(mandatory_type),
                "commandTimestamp": int(command_timestamp) if command_timestamp is not None else None,
