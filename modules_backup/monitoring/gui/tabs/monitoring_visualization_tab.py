...
class MonitoringVisualizationTab(QWidget):
    ...
    def build_execute_next_replan_context(self) -> dict[str, object] | None:
        ...
        for entry in view.get("uav_entries") or []:
            ...
            for mission in entry.get("missions") or []:
                ...
                if mission_input_id_int is None:
                    continue
                if mission_input_id_int == int(current_input_id) and aircraft_id_int not in current_input_aircraft_ids:
        ...
        for aircraft_id_int in current_input_aircraft_ids:
            if aircraft_id_int in target_aircraft_ids:
                continue
    ...
    def _persist_coverage_progress(self, snapshot: dict) -> None:
        ...
        try:
            ...
            return
        if signature == self._coverage_progress_last_signature:
            return
        try:
            base.mkdir(parents=True, exist_ok=True)
            path = base / "coverage_progress.json"
            path.write_text(signature, encoding="utf-8")
            self._coverage_progress_last_signature = signature
        except Exception:
            pass
