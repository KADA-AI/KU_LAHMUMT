...
def simulate_waypoints(
    ...
    while t < total_time and curr_idx < len(wp_list):
        if hasattr(uav, "hover_mode"):
            uav.hover_mode = False
        if hasattr(uav, "cmd_vertical_speed"):
            uav.cmd_vertical_speed = 0.0
        ...
        hover_capture_radius = max(pos_tol * 1.25, 40.0)
        hover_guidance = dist_xy <= hover_capture_radius and abs(dz) > pos_tol * 0.6
        if hover_guidance:
            hold_xy = dist_xy <= hover_capture_radius
            if hasattr(uav, "hover_mode"):
                uav.hover_mode = True
            yaw_int = 0.0
            alt_int = 0.0
            ...
            if hasattr(uav, "cmd_vertical_speed"):
                uav.cmd_pitch_rate = clamp(
                    -uav.s.pitch * max(1.2, pitch_damp + 0.4),
                ...
                if abs(dz) < max(2.0, pos_tol * 0.15):
                    vz_cmd = 0.0
                uav.cmd_vertical_speed = vz_cmd
            else:
                desired_pitch = clamp(
                    dz * alt_pitch,
                ...
                pitch_err = desired_pitch - uav.s.pitch
                uav.cmd_pitch_rate = clamp(pitch_err * pitch_rate_p, -uav.p.max_pitch_rate_dps, uav.p.max_pitch_rate_dps)

            desired_speed = 0.0 if hold_xy else min(max(2.0, dist_xy * 0.08), max(4.0, speed_target * 0.2))
            speed_err = desired_speed - uav.s.u
            throttle_cmd = speed_err * throttle_p
            if hold_xy:
    ...
    if base_wps:
        ...
        suite.append(
            [
                ...
                (float(x0) + 200.0, float(y0), float(z0)),
                (float(x0) + 200.0, float(y0), float(z0) + 120.0),
                (float(x0) + 200.0, float(y0), float(z0) + 20.0),
            ]
        )
    return suite
