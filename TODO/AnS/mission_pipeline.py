...
def run_divide_and_pattern(
    ...
    # env 생성 (areas는 직전에 구성한 sub-area 리스트)
    processed_areas = areas
    env = UnifiedMissionEnvironment(processed_areas)
    env.flight_weight = env.imaging_weight = 0.5
    total_sub = len(processed_areas)

    # 학습 시 사용한 패턴 순서(고정). env에 없으면 mission_patterns 키 순서 사용
    pattern_keys = getattr(env, "pattern_keys", list(mission_patterns.keys()))

    # BestFlightCase 모델 경로 확인 및 로드
    MODEL_PATH = Path(__file__).resolve().parent / "Training" / "SavedModels" / "BestFlightCase_Model.zip"
    if not MODEL_PATH.exists():
        raise FileNotFoundError(f"BestFlightCase 모델이 없습니다: {MODEL_PATH}")
    model = PPO.load(str(MODEL_PATH), env=env, device="cpu")  # MLP면 CPU 권장

    # Gym vs Gymnasium 호환 reset
    reset_out = env.reset()
    if isinstance(reset_out, tuple) and len(reset_out) == 2:   # gymnasium
        obs, _info = reset_out
    else:                                                      # gym
        obs = reset_out
    ...
    def _log_every(n: int) -> int:
        ...
        if n <= 200:  return 5
        return 10
    LOG_EVERY = _log_every(total_sub)

    # ── 성능 측정용 타이머 배열 ─────────────────────────────────
    t_predict, t_step, t_dist = [], [], []
    ...
    while not done:
        ...
        # A) 정책 추론: deterministic 예측 → action_idx (정수)
        action_idx, _ = model.predict(obs, deterministic=True)
        if not (0 <= action_idx < len(pattern_keys)):
            raise ValueError(f"예측 action_idx={action_idx} 가 유효 범위를 벗어남 (0..{len(pattern_keys)-1})")
        t1 = time.perf_counter()

        # B) 환경 진행 (Gym/Gymnasium 호환)
        step_out = env.step(action_idx)
        if isinstance(step_out, tuple) and len(step_out) == 5:  # gymnasium
        ...
        # C) 비행거리 → 예상시간 계산 및 cur_area 업데이트
        cur_idx = getattr(env, "current_mission_index", 0) - 1  # env가 1-based 인덱싱 사용
        if 0 <= cur_idx < len(env.processed_missions):
            cur_area   = env.processed_missions[cur_idx]
            key        = pattern_keys[action_idx]
            sel_pattern= mission_patterns[key]

            dist_m = calculate_flight_distance(
                cur_area["coordinateList"],
                sel_pattern["비행 패턴"],
                sel_pattern["촬영 패턴"]
            )
            cur_area["EstimatedMissionTime"] = dist_m / UAV_VELOCITY
            cur_area["MissionPattern"]       = sel_pattern
            cur_area["patternType"]          = key

        t3 = time.perf_counter()

        # ── 진행 로그 (GUI 실시간 반영) ─────────────────────────
        cur_1based = getattr(env, "current_mission_index", 0)
        if cur_1based == 1 or (cur_1based % LOG_EVERY == 0) or done:
            elapsed = t3 - t_start_all
            log(f"       RL 진행 {cur_1based:>4}/{total_sub}  (소요 {elapsed:4.1f}s)")
            try:
                from PyQt5.QtWidgets import QApplication
                QApplication.processEvents()  # Qt GUI 반응성 확보
            except Exception:
                pass

        # ── 소요시간 누적 ───────────────────────────────────────
        t_predict.append(t1 - t0)   # model.predict 시간
        t_step.append   (t2 - t1)   # env.step 시간
        t_dist.append   (t3 - t2)   # 거리/시간 계산 및 업데이트 시간

    # ── 요약 통계 출력 ─────────────────────────────────────────
    def _ms(arr): return [x * 1000.0 for x in arr]
    log("       ┌─ RL 세부 소요(ms) ─────────────")
