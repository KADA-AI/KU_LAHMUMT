"""Monitoring-specific push implementation for message 0504.

This shim ensures the monitoring package resolves the FuelWarning push helpers
without depending on decision support modules.
"""
from __future__ import annotations

import json
from datetime import datetime, timezone
from typing import Any, Dict

try:
    from nFusion.Model.msg_0504 import FuelWarning  # type: ignore
except ModuleNotFoundError as exc:
    raise ImportError(
        "FuelWarning (msg_0504) type is unavailable. Ensure dll_files.nFusionImports has loaded the nFusion assemblies."
    ) from exc

MSG_ID = "0504"
_EPOCH_2000 = datetime(2000, 1, 1, tzinfo=timezone.utc)


def _now_ms() -> int:
    return int((datetime.now(timezone.utc) - _EPOCH_2000).total_seconds() * 1000)


def _try_set(obj: Any, name: str, value: Any) -> bool:
    for attr in (name, name[:1].upper() + name[1:] if name else name):
        try:
            if hasattr(obj, attr):
                setattr(obj, attr, value)
                return True
        except Exception:
            pass
    return False


def _normalize_body(body_dict: Dict[str, Any] | None) -> Dict[str, Any]:
    data = dict(body_dict or {})

    ts = data.get("timestamp")
    try:
        timestamp = int(ts)
    except Exception:
        timestamp = _now_ms()

    source_val = (
        data.get("source")
        or data.get("Source")
        or data.get("requestModuleName")
        or "MSM"
    )
    source = str(source_val).strip() or "MSM"

    try:
        aircraft_id = int(data.get("aircraftID", data.get("AircraftID", 0)))
    except Exception:
        aircraft_id = 0
    if aircraft_id < 0:
        aircraft_id = 0

    try:
        fuel_level = int(data.get("fuelLevel", data.get("FuelLevel", 1)))
    except Exception:
        fuel_level = 1
    if fuel_level not in (1, 2):
        fuel_level = 1

    return {
        "timestamp": timestamp,
        "source": source,
        "aircraftID": aircraft_id,
        "fuelLevel": fuel_level,
    }


def _dict_to_obj(body_dict: Dict[str, Any]) -> FuelWarning:
    obj = FuelWarning()
    _try_set(obj, "timestamp", int(body_dict.get("timestamp", _now_ms())))
    src = (
        body_dict.get("source")
        or body_dict.get("Source")
        or body_dict.get("requestModuleName")
        or "MSM"
    )
    if src:
        _try_set(obj, "source", str(src))
    _try_set(obj, "aircraftID", int(body_dict.get("aircraftID", 0)))
    _try_set(obj, "fuelLevel", int(body_dict.get("fuelLevel", 1)))
    return obj


def make_and_push(body_dict: Dict[str, Any], node_messenger) -> bytes:
    body = _normalize_body(body_dict)
    msg = _dict_to_obj(body)
    node_messenger.Push(msg)

    body_json = json.dumps(body, ensure_ascii=False)
    log_line = f"[{MSG_ID}] BODY  : {body_json}\n[{MSG_ID}] PUSH OK"

    try:
        from ..receive.receive_center import notify  # type: ignore
    except Exception:
        notify = None  # type: ignore

    if notify:
        try:
            notify(MSG_ID, body_json.encode("utf-8", "ignore"))
        except Exception:
            pass

    try:
        from .. import udp_reporter  # type: ignore
        udp_reporter.notify_tx(MSG_ID)
    except Exception:
        pass

    return log_line.encode("utf-8", "ignore")


def make_random_and_push(node_messenger) -> bytes:
    try:
        from modules.common.generator.message0504_generator import make_msg0504_body
    except ModuleNotFoundError:
        try:
            from generator.message0504_generator import make_msg0504_body  # type: ignore
        except ModuleNotFoundError as exc:
            raise ImportError("message0504 generator is unavailable") from exc

    return make_and_push(make_msg0504_body(), node_messenger)
