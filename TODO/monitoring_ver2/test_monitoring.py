...
try:
    ...
    msg_dll_folder_path = os.path.join(project_root, "modules", "common", "msg_files")
    if msg_dll_folder_path not in sys.path:
        sys.path.append(msg_dll_folder_path)

    for framework_path in (
        os.path.join(project_root, "modules", "common", "dll_files"),
        os.path.join(script_dir, "dll_files"),
    ):
