...
try:
...
except ModuleNotFoundError:  # standalone execution path
    ...
    for _extra in (
        ...
        _PROJECT_ROOT / "modules" / "common",
    ):
        _s = str(_extra)
        if _extra.exists() and _s not in sys.path:
            sys.path.insert(0, _s)

    from modules.common.receive_center import SignalEmitter, set_global_signal_emitter
