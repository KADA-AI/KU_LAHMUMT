    ...
    def execute(self, mode_override=None):
        ...
        if system_mode == 3:
            ...
            if data_401:
                ...
                # test feul Logic
                feul_data = []
                prev_warnings = dict(self.manager.logic_store.get_data("fuel_warning_prev") or {})
                ...
                for agent_state in data_401.agentStateList:
                    if agent_state.isUnmanned == 1:
                        ...
                        feul_data.append({"id": agent_state.aircraftID, "warning": text})

                        if fuel_level in (1, 2):
                            last_state = prev_warnings.get(agent_state.aircraftID)
                            if last_state != text:
                                warning_body = {
                                    "timestamp": int((datetime.now(timezone.utc) - datetime(2000, 1, 1, tzinfo=timezone.utc)).total_seconds() * 1000),
                                    "source": "MSM",
                                    "aircraftID": agent_state.aircraftID,
                                    "fuelLevel": fuel_level,
                                }
                                push_message("0504", self.manager.node_messenger, body_dict=warning_body)
                                self.manager._log(
                                    "MON_LOGIC", "INFO", f"0504 연료 경고 전송 (UAV={agent_state.aircraftID}, level={text})"
                                )
                                try:
                                ...
                                if self.manager.gui_update_callback:
                                    try:
                                        self.manager.gui_update_callback("logic", "0504", warning_body)
                                    except Exception:
                                        pass
                        prev_warnings[agent_state.aircraftID] = text

                self.manager.logic_store.set_data("fuel_warning_prev", prev_warnings)
