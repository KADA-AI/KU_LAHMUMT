...
class BFDBGenerate:
    ...
    def find_largest_fov_with_base_over_1000(self):
        df = self.find_valid_fov_af_pairs_with_geometry()
        df_filtered = df[df['Base Length (m)'] >= 1000]
        if df_filtered.empty:
            raise ValueError("조건을 만족하는 FOV와 Base Length가 없습니다.")
        # FOV 가장 큰 값 찾기
        max_row = df_filtered.loc[df_filtered['FOV (deg)'].idxmax()]
        return max_row['FOV (deg)'], max_row['Base Length (m)']
